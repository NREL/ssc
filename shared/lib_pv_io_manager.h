...
enum modulePowerModelList {
    MODULE_SIMPLE_EFFICIENCY, MODULE_CEC_DATABASE, MODULE_CEC_USER_INPUT, MODULE_SANDIA, MODULE_IEC61853, MODULE_PVYIELD
};
enum inverterTypeList {
    INVERTER_CEC_DATABASE, INVERTER_DATASHEET, INVERTER_PARTLOAD, INVERTER_COEFFICIENT_GEN, INVERTER_PVYIELD
};
...
*/
struct PVSystem_IO {
    PVSystem_IO(compute_module *cm, std::string cmName, Simulation_IO *SimulationIO, Irradiance_IO *IrradianceIO,
                std::vector<Subarray_IO *> Subarrays, Inverter_IO *InverterIO);
...
public:
    ...
    enum self_shading {
        NO_SHADING, NON_LINEAR_SHADING, LINEAR_SHADING
    };
...
public:
    ...
    enum mountingSpecificConfigurationList {
        NONE, RACK_MOUNTING, FLUSH_MOUNTING, INTEGRATED_MOUNTING, GAP_MOUNTING
    };
