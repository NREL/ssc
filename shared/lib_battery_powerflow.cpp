...
void BatteryPowerFlow::initialize(double stateOfCharge) {
    ...
    else if ((m_BatteryPower->powerPV > m_BatteryPower->powerLoad && m_BatteryPower->canPVCharge) ||
             m_BatteryPower->canGridCharge) {
...
void BatteryPowerFlow::calculateACConnected() {
    ...
    if (P_battery_ac <= 0) {
        ...
        if (m_BatteryPower->canFuelCellCharge) {
            P_fuelcell_to_batt_ac = std::fmin(std::fmax(0, fabs(P_battery_ac) - P_pv_to_batt_ac),
                                              P_fuelcell_ac - P_fuelcell_to_load_ac);
        }
    ...
    m_BatteryPower->powerConversionLoss =
            P_batt_to_load_loss_ac + P_batt_to_grid_loss_ac + P_grid_to_batt_loss_ac + P_pv_to_batt_loss_ac;
}
...
void BatteryPowerFlow::calculateDCConnected() {
    ...
    if (P_battery_dc < 0) {
        ...
        m_BatteryPower->sharedInverter->calculateACPower(P_gen_dc_inverter, voltage,
                                                         m_BatteryPower->sharedInverter->Tdry_C);
