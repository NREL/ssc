...
PVIOManager::PVIOManager(compute_module *cm, std::string cmName) {
    ...
    std::unique_ptr<PVSystem_IO> pvSystem(
            new PVSystem_IO(cm, cmName, m_SimulationIO.get(), m_IrradianceIO.get(), getSubarrays(),
                            m_InverterIO.get()));
...
Irradiance_IO::Irradiance_IO(compute_module *cm, std::string cmName) {
    ...
    if (numberOfWeatherFileRecords % 8760 != 0)
        throw exec_error(cmName,
                         util::format("invalid number of data records (%zu): must be an integer multiple of 8760",
                                      numberOfWeatherFileRecords));
    if (stepsPerHour < 1 || stepsPerHour > 60)
        throw exec_error(cmName, util::format("%d timesteps per hour found. Weather data should be single year.",
                                              stepsPerHour));
...
void Irradiance_IO::checkWeatherFile(compute_module *cm, std::string cmName) {
    for (size_t idx = 0; idx < numberOfWeatherFileRecords; idx++) {
        if (!weatherDataProvider->read(&weatherRecord))
            throw exec_error(cmName,
                             "could not read data line " + util::to_string((int) (idx + 1)) + " in weather file");
        ...
        if ((weatherRecord.gh != weatherRecord.gh) &&
            ...
            cm->log(util::format("missing global irradiance %lg W/m2 at time [y:%d m:%d d:%d h:%d], exiting",
                                 weatherRecord.gh, weatherRecord.year, weatherRecord.month, weatherRecord.day,
                                 weatherRecord.hour), SSC_ERROR, (float) idx);
            return;
        }
        if ((weatherRecord.dn != weatherRecord.dn) &&
            ...
            cm->log(util::format("missing beam irradiance %lg W/m2 at time [y:%d m:%d d:%d h:%d], exiting",
                                 weatherRecord.dn, weatherRecord.year, weatherRecord.month, weatherRecord.day,
                                 weatherRecord.hour), SSC_ERROR, (float) idx);
            return;
        }
        if ((weatherRecord.df != weatherRecord.df) &&
            ...
            cm->log(util::format("missing diffuse irradiance %lg W/m2 at time [y:%d m:%d d:%d h:%d], exiting",
                                 weatherRecord.df, weatherRecord.year, weatherRecord.month, weatherRecord.day,
                                 weatherRecord.hour), SSC_ERROR, (float) idx);
            return;
        }
        ...
        if ((weatherRecord.gh < 0 || weatherRecord.gh > irrad::irradiationMax) &&
            ...
            cm->log(util::format("out of range global irradiance %lg W/m2 at time [y:%d m:%d d:%d h:%d], set to zero",
                                 weatherRecord.gh, weatherRecord.year, weatherRecord.month, weatherRecord.day,
                                 weatherRecord.hour), SSC_WARNING, (float) idx);
            weatherRecord.gh = 0;
        }
        ...
        if (useWeatherFileAlbedo && std::isfinite(weatherRecord.alb) && weatherRecord.alb > 0 &&
            weatherRecord.alb < 1) {
            albedoError = false;
        ...
        if (albedoError) {
            throw exec_error(cmName,
                             util::format(
                                     "Error retrieving albedo value: Invalid month in weather file or invalid albedo value in weather file"));
        }
    }
    weatherDataProvider->rewind();
}
...
void Irradiance_IO::AllocateOutputs(compute_module *cm) {
    ...
    if (radiationMode == irrad::DN_DF)
        p_IrradianceCalculated[0] = cm->allocate("gh_calc",
    ...
    if (radiationMode == irrad::DN_GH || radiationMode == irrad::POA_R ||
        ...
        p_IrradianceCalculated[1] = cm->allocate("df_calc", numberOfWeatherFileRecords);
    if (radiationMode == irrad::GH_DF || radiationMode == irrad::POA_R ||
        radiationMode == irrad::POA_P)
        p_IrradianceCalculated[2] = cm->allocate("dn_calc", numberOfWeatherFileRecords);
...
Subarray_IO::Subarray_IO(compute_module *cm, std::string cmName, size_t subarrayNumber) {
    ...
    if (enable) {
        ...
        if (nStrings <=
            0) //subarrays with no strings need to be treated as if they are disabled to avoid divide by zero issues in the subarray setup
        {
            enable = false;
            return;
        }
        ...
        if (trackMode == irrad::FIXED_TILT || trackMode == irrad::SINGLE_AXIS || trackMode == irrad::AZIMUTH_AXIS)
            if (!tiltEqualLatitude && !cm->is_assigned(prefix + "tilt"))
                throw exec_error(cmName, "Subarray " + util::to_string((int) subarrayNumber) +
        ...
        if (monthlySoiling.size() != 12)
            throw exec_error(cmName, "soiling loss array must have 12 values: subarray " +
        ...
        selfShadingInputs.mod_orient = cm->as_integer(prefix +
                                                      "mod_orient"); //although these inputs are stored in self-shading structure, they are also used for snow model and bifacial model, so required for all enabled subarrays
        ...
        if (trackMode == irrad::FIXED_TILT || trackMode == irrad::SEASONAL_TILT || (trackMode == irrad::SINGLE_AXIS)) {
            if (shadeMode != NO_SHADING) {
                ...
                selfShadingInputs.nrows = (int) floor(
                        (nStrings * nModulesPerString) / (selfShadingInputs.nmodx * selfShadingInputs.nmody));
                ...
                if (selfShadingInputs.nrows == 0 && nStrings != 0)
                    throw exec_error(cmName, "Self shading: Number of rows calculated for subarray " +
                                             util::to_string(int(subarrayNumber)) +
                ...
                if ((size_t) (selfShadingInputs.nmodx * selfShadingInputs.nmody * selfShadingInputs.nrows) !=
                    (size_t) (nStrings * nModulesPerString))
                    cm->log(util::format(
                            "The product of number of modules along side and bottom for subarray %d is not equal to the number of modules in the subarray. Check your inputs for self shading.",
        ...
        if (subarrayEnableSnow) {
            if (trackMode == irrad::SEASONAL_TILT)
                throw exec_error(cmName,
                                 "Time-series tilt input may not be used with the snow model at this time: subarray " +
                                 util::to_string((int) (subarrayNumber)));
        ...
        if (nStrings < 0) {
            throw exec_error(cmName,
                             "invalid string allocation between subarrays.  all subarrays must have zero or positive number of strings.");
        }
        ...
        for (size_t nn = 0; nn < Subarrays.size(); nn++) {
            ...
            for (size_t h = 0; h < 8760; h++) {
                for (size_t m = 0; m < Irradiance->stepsPerHour; m++) {
                    ...
                    if (!wdprov->read(&wf)) {
                        throw exec_error("pvsamv1", "could not read data line " + util::to_string((int) (ii + 1)) +
                    ...
                    if (t_sunset >
                        24) //sunset is legitimately the next day, so recalculate sunset from the previous day
                    {
                        ...
                        if (wf.day > 1) //simply decrement day during month
                            solarpos(wf.year, wf.month, wf.day - 1, 12, 0.0, hdr.lat, hdr.lon, hdr.tz, sunanglestemp);
                                 ...
                                 1) //on the 1st of the month, need to switch to the last day of previous month
                            solarpos(wf.year, wf.month - 1, __nday[wf.month - 2], 12, 0.0, hdr.lat, hdr.lon, hdr.tz,
                    ...
                    if (t_sunrise < 0) //sunrise is legitimately the previous day, so recalculate for next day
                    {
                        ...
                        if (wf.day < __nday[wf.month -
                                            1]) //simply increment the day during the month, month is 1-indexed and __nday is 0-indexed
                            solarpos(wf.year, wf.month, wf.day + 1, 12, 0.0, hdr.lat, hdr.lon, hdr.tz, sunanglestemp);
                        else if (wf.month <
                                 12) //on the last day of the month, need to switch to the first day of the next month
                    ...
                    if (t_cur >= t_sunrise - ts_hour / 2.0 && t_cur < t_sunrise + ts_hour / 2.0) {
                        double t_calc =
                                (t_sunrise + (t_cur + ts_hour / 2.0)) / 2.0; // midpoint of sunrise and end of timestep
                    ...
                    else if ((t_sunrise < t_sunset && t_cur >= t_sunrise && t_cur <= t_sunset) ||
                             //this captures normal daylight cases
                    ...
                    if (tms[2] > 0) {
                        incidence(Subarrays[nn]->trackMode, Subarrays[nn]->tiltDegrees, Subarrays[nn]->azimuthDegrees,
                                  Subarrays[nn]->trackerRotationLimitDegrees, sun[1], sun[0],
                                  Subarrays[nn]->backtrackingEnabled, Subarrays[nn]->groundCoverageRatio, false, 0.0,
                                  angle);
                    } else {
                        angle[0] = -999;
                        angle[1] = -999;
...
PVSystem_IO::PVSystem_IO(compute_module *cm, std::string cmName, Simulation_IO *SimulationIO,
                         Irradiance_IO *IrradianceIO, std::vector<Subarray_IO *> SubarraysAll,
    ...
    std::unique_ptr<SharedInverter> tmpSharedInverter(
            new SharedInverter(Inverter->inverterType, numberOfInverters, &Inverter->sandiaInverter,
    ...
    if (Simulation->useLifetimeOutput) {
        ...
        if (enableDCLifetimeLosses) {
            ...
            throw exec_error(cmName,
                             "Length of the lifetime daily DC losses array must be equal to the analysis period * 365 days/year");
        }
        if (enableACLifetimeLosses) {
            ...
            if (acLifetimeLosses.size() != Simulation->numberOfYears * 365)
                throw exec_error(cmName,
                                 "Length of the lifetime daily AC losses array must be equal to the analysis period * 365 days/year");
        }
    }

    // Transformer losses
    transformerLoadLossFraction = cm->as_number("transformer_load_loss") * (ssc_number_t) (util::percent_to_fraction);
    transformerNoLoadLossFraction =
            cm->as_number("transformer_no_load_loss") * (ssc_number_t) (util::percent_to_fraction);
    ...
    if (InverterIO->mpptLowVoltage > 0 && InverterIO->mpptHiVoltage > InverterIO->mpptLowVoltage) {
            ...
            || modulePowerModel == 5) // PVYield single diode
        {
            ...
            cm->log("The simple efficiency and Sandia module models do not allow limiting module voltage to the MPPT tracking range of the inverter.",
                    SSC_NOTICE);
        }
    } else {
    ...
    for (size_t n_subarray = 0; n_subarray < numberOfSubarrays; n_subarray++) {
        if (Subarrays[n_subarray]->enable) {
            if (Subarrays[n_subarray]->mpptInput > (int) Inverter->nMpptInputs)
                throw exec_error(cmName, "Subarray " + util::to_string((int) n_subarray) +
                                         " MPPT input is greater than the number of inverter MPPT inputs.");
        }
    }
    for (size_t mppt = 1;
         mppt <= (size_t) Inverter->nMpptInputs; mppt++) //indexed at 1 to match mppt input numbering convention
    {
        ...
        for (size_t n_subarray = 0; n_subarray <
                                    Subarrays.size(); n_subarray++) //jmf update this so that all subarray markers are consistent, get rid of "enable" check
            if (Subarrays[n_subarray]->enable)
    ...
    if (Inverter->nMpptInputs > 1 && numberOfInverters > 1)
        throw exec_error(cmName,
                         "At this time, only one multiple-MPPT-input inverter may be modeled per system. See help for details.");
    ...
    if (enableMismatchVoltageCalc &&
        ...
        throw exec_error(cmName,
                         "String level subarray mismatch can only be calculated using a single-diode based module model.");
    }
    if (enableMismatchVoltageCalc && numberOfSubarrays <= 1)
        throw exec_error(cmName,
                         "Subarray voltage mismatch calculation requires more than one subarray. Please check your inputs.");
...
void PVSystem_IO::AllocateOutputs(compute_module *cm) {
    ...
    for (size_t subarray = 0; subarray < Subarrays.size(); subarray++) {
        if (Subarrays[subarray]->enable) {
            ...
            p_derateSelfShadingDiffuse.push_back(
                    cm->allocate(prefix + "ss_diffuse_derate", numberOfWeatherFileRecords));
            p_derateSelfShadingReflected.push_back(
                    cm->allocate(prefix + "ss_reflected_derate", numberOfWeatherFileRecords));
            ...
            if (Subarrays[subarray]->enableSelfShadingOutputs) {
                ...
                p_shadeDB_temperatureCell.push_back(
                        cm->allocate("shadedb_" + prefix + "pv_cell_temp", numberOfWeatherFileRecords));
                p_shadeDB_modulesPerString.push_back(
                        cm->allocate("shadedb_" + prefix + "mods_per_str", numberOfWeatherFileRecords));
                p_shadeDB_voltageMaxPowerSTC.push_back(
                        cm->allocate("shadedb_" + prefix + "str_vmp_stc", numberOfWeatherFileRecords));
                p_shadeDB_voltageMPPTLow.push_back(
                        cm->allocate("shadedb_" + prefix + "mppt_lo", numberOfWeatherFileRecords));
                p_shadeDB_voltageMPPTHigh.push_back(
                        cm->allocate("shadedb_" + prefix + "mppt_hi", numberOfWeatherFileRecords));
            }
            p_shadeDBShadeFraction.push_back(
                    cm->allocate("shadedb_" + prefix + "shade_frac", numberOfWeatherFileRecords));
        }
    }

    for (size_t mppt_input = 0; mppt_input < Inverter->nMpptInputs; mppt_input++) {
        p_mpptVoltage.push_back(
                cm->allocate("inverterMppt" + std::to_string(mppt_input + 1) + "_DCVoltage", numberOfLifetimeRecords));
        p_dcPowerNetPerMppt.push_back(
                cm->allocate("inverterMppt" + std::to_string(mppt_input + 1) + "_NetDCPower", numberOfLifetimeRecords));
    }
...
Module_IO::Module_IO(compute_module *cm, std::string cmName, double dcLoss) {
    ...
    if (modulePowerModel == MODULE_SIMPLE_EFFICIENCY) {
        ...
        if (cm->as_integer("cec_temp_corr_mode") == 0) {
            ...
            switch (standoff) { //source for standoff adjustment constants: https://prod-ng.sandia.gov/techlib-noauth/access-control.cgi/1985/850330.pdf page 12
                case 2:
        ...
        if (err != 0)
            throw exec_error(cmName,
                             "CEC 6 parameter model:  Could not solve for normalized coefficients.  Please check your inputs.");
        ...
        selfShadingFillFactor =
                elevenParamSingleDiodeModel.Vmp0 * elevenParamSingleDiodeModel.Imp0 / elevenParamSingleDiodeModel.Voc0 /
                elevenParamSingleDiodeModel.Isc0;
        ...
        selfShadingFillFactor =
                mlModuleModel.V_mp_ref * mlModuleModel.I_mp_ref / mlModuleModel.V_oc_ref / mlModuleModel.I_sc_ref;
...
void Module_IO::setupNOCTModel(compute_module *cm, const std::string &prefix) {
    ...
    if (cm->as_integer(prefix + "_mounting") == 1)
        nominalOperatingCellTemp.ffv_wind = 0.61;  // greater than 22ft high (2 story)
...
void Inverter_IO::setupSharedInverter(compute_module *cm, SharedInverter *a_sharedInverter) {
    ...
    if (err > 1) {
        throw exec_error("pvsamv1",
                         "Inverter temperature derate curve " + util::to_string((int) (-err - 1)) + " is invalid.");
    }
}
