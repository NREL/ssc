...
bool dispatch_resilience::run_outage_step_dc(double crit_load_kwac, double pv_kwdc, double V_pv, double pv_clipped,
                                             double tdry) {
    if (connection != CONNECTION::DC_CONNECTED)
...
void resilience_runner::run_surviving_batteries_by_looping(double *crit_loads_kwac, double *pv_kwac, double *pv_kwdc,
    ...
    while (get_n_surviving_batteries() > 0 && i < steps_lifetime) {
        if (pv_kwdc && V && pv_clipped_kw && tdry_c)
            run_surviving_batteries(crit_loads_kwac[i % nrec], pv_kwac[i], pv_kwdc[i], V[i], pv_clipped_kw[i],
                                    tdry_c[i % nrec]);
        else
...
double resilience_runner::compute_metrics() {
    ...
    return std::accumulate(indices_survived.begin(), indices_survived.end(), 0.0) / batt->step_per_hour /
           (double) indices_survived.size();
}
