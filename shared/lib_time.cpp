            ...
            for (size_t h = 0; h < util::hours_per_year; h++) {
                for (size_t sy = 0; sy < step_per_hour; sy++) {
                    ...
                    singleyear_sampled.push_back(
                            singleyear_vector[(size_t) (sy_idx / interpolation_factor)] / interpolation_factor);
                    sy_idx++;
                }
            }
        }
...
template void
single_year_to_lifetime_interpolated<double>(bool, size_t, size_t, std::vector<double>, std::vector<double> &, size_t &,
                                             double &);

template void
single_year_to_lifetime_interpolated<float>(bool, size_t, size_t, std::vector<float>, std::vector<float> &, size_t &,
                                            double &);
...
template std::vector<double>
extrapolate_timeseries(std::vector<double> input_values, size_t steps_per_hour, double multiplier);
