...
bool iec61853par::tcoeff( util::matrix_t<double> &input, size_t icol, double irr, double *tempc, bool print_table )
{
	*tempc = std::numeric_limits<double>::quiet_NaN();
	...
	if ( Val_stc.size() < 3 )
	{
		PRINTF("insufficient measurements at %lg W/m2, at least 3 required at different temperatures to calculate temperature coefficient of %s.  only %d detected\n", irr, col_names[icol], (int)Val_stc.size() );
		return false;
	}
	...
	if ( print_table )
	{
		for( size_t i=0;i<Tc_stc.size();i++ )
			PRINTF("%d\tTc,%s @ %lg\t%lg\t%lg\n", i, col_names[icol], irr, Tc_stc[i], Val_stc[i] );
	}
	...
	{
		PRINTF("linear regression failed for temperature coefficient of %s calculation\n", col_names[icol]);
		return false;
	}
			...
			double *p_Il, double *p_Io, double *p_Rs, double *p_Rsh )
{
	...
	//PRINTF("iterative solution... max iterations %d, underrelaxation %lg\n", MaxIter, urelax);
	int jj = 0;
	while ( jj < MaxIter )
	{
		...
		//PRINTF("iteration %d:  Il=%lg Io=%lg Rs=%lg Rsh=%lg\n", jj, Il, Io, Rs, Rsh );
		...
		if ( ierr != 0 )
		{
			PRINTF( "singularity in gauss() in solution of four parameter nonlinear equation, iteration %d\n", jj );
			...
			PRINTF("Il=%lg Io=%lg Rs=%lg Rsh=%lg\n", Il, Io, Rs, Rsh );
	...
	if ( jj == MaxIter )
	{
		PRINTF("Reached maximum iterations of %d in solve()\n", MaxIter );
		return false;
	}
	...
	util::matrix_t<double> &par, bool verbose )
{
	if (input.ncols() != COL_MAX) {
		PRINTF( "incorrect number of data columns in input.  %d required\n", COL_MAX );
		return false;
	}

	if ( input.nrows() < NCONDITIONS_MIN ) {
		PRINTF( "insufficient number of test conditions, %d minimum\n", NCONDITIONS_MIN );
		return false;
	}
	...
	if( verbose ) PRINTF("module STC ratings: Pmp=%lg Vmp=%lg Imp=%lg Voc=%lg Isc=%lg\n", Pmp0, Vmp0, Imp0, Voc0, Isc0 );
	...
	if( verbose ) PRINTF("betaVoc=%lg (V/'C)  alphaIsc=%lg (A/'C)   gammaPmp=%lg (%/'C)\n", betaVoc, alphaIsc, gammaPmp);
	...
	for( size_t i=0;i<input.nrows();i++ )
	{
		if ( !std::isfinite( nfac[i] ) )
		{
			if( verbose ) PRINTF(" non-finite diode factor at condition %d (%lg W/m2, %lg C)\n", i+1, input(i,COL_IRR), input(i,COL_TC) );
			nfac[i] = navg;
		}
	}

	if( verbose ) PRINTF("non-finite diode nonideality factors at %d conditions filled with average value.\n", input.nrows() - (int)ncount);
	...
	if( verbose ) PRINTF("reference guess for module resistances @ STC  Rs=%lg Rsh=%lg\n", Rs_ref0, Rsh_ref0 );

	// storage for parameters at each condition
	par.resize_fill( input.nrows(), PARMAX, std::numeric_limits<double>::quiet_NaN() );
	int nsuccess = 0;
	
	if( verbose ) PRINTF("solving for Il, Io, Rs, Rsh at %d conditions...\n", (int)input.nrows() );
	...
	for( size_t i=0;i<input.nrows();i++ )
	{
		...
		{
			PRINTF("solution failed at condition %d\n", i );
		}
	...
	if( verbose ) PRINTF("determined parameter Egref=%lg.  Io_stc=%lg\n", Egref_fit[0], Io_stc );
	...
	for( size_t i=0;i<irrads.size();i++ )
	{
		...
		if( verbose ) PRINTF("Rsh_avg[@ %lg W/m2] = %lg\n", irrads[i], Rsh_avgs[i] );
	}
	...
	if ( verbose ) PRINTF( "determined Rsh equation parameters C1=%lg C2=%lg C3=%lg\n", C[0], C[1], C[2] );
	...
	for( size_t i=0;i<temps.size();i++ )
	{
		...
		if ( Ivec.size() < 3 )
		{
			PRINTF("must have measurements at two different irradiance levels for each unique temperature considered, only %d found at %lg C\n", (int) Ivec.size(), temps[i] );
			return false;
		}
		...
		{
			PRINTF("error in nonlinear least squares fit for Rs equation at %lg C\n", temps[i] );
			return false;
		}
	...
	if( verbose ) PRINTF("determined Rs equation parameters D1=%lg D2=%lg D3=%lg\n", D1, D2, D3 );
