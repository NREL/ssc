//File automatically generated by variable_map_makefile.py 2016/10/13
...
struct var_ambient
{
	...
	spvar< int > atm_model; 		//[none] Atmospheric attenuation model {0=25km Barstow, 1 = 5km Barstow, 2 = user defined}
	...
	spvar< int > insol_type; 		//[none] Model used to determine insolation as a function of time
	...
	spvar< int > sun_type; 		//[none] Sunshape model - {0=point sun, 1=limb darkened sun, 2=square wave sun, 3=user sun}
...
struct var_custom_rec
{
	...
	spvar< int > basis_method; 		//[none] Metric to use as base flux value in determining profile quality
	...
	spvar< int > circ_profile; 		//[none] Method for generating circumferential flux profile based on available data points
	...
	spvar< int > design_location; 		//[none] Flux criteria to determine the tube design point location
	...
	spvar< int > header_domain; 		//[none] Method for dividing a flux panel into header sections
	...
	spvar< int > quality_method; 		//[none] Method for calculating the quality of the flux profile
	...
	spvar< int > tube_material; 		//[] Tube material
...
struct var_fluxsim
{
	spvar< int > aim_method; 		//[] Method for determining the aim point for each heliostat
	spvar< std::string > class_name; 		//[none] Class name
	...
	spvar< int > cloud_shape; 		//[] Shape used to model the cloud shadow
	...
	spvar< int > flux_dist; 		//[] Sampling basis for random positioning. Non-uniform distributions are weighted away from the center.
	...
	spvar< int > flux_model; 		//[none] Desired flux simulation tool. Not all geometries can be simulated using the Hermite approximation.
	...
	spvar< int > flux_time_type; 		//[none] Method for specifying the desired flux simulation time.
...
struct var_heliostat
{
	...
	spvar< int > cant_method; 		//[none] Integer to specify the canting method {0=none, -1=Cant on-axis equal to slant range, 1=user-defined on-axis, 3=user-defined off-axis at hour + day}
	...
	spvar< int > focus_method; 		//[none] The focusing method {0=Flat, 1=Each at slant, 2=Average of group, 3=User defined}
	...
	spvar< bool > is_round; 		//[none] Is the heliostat round (true) or rectangular (false)
	...
	spvar< int > track_method; 		//[none] Specify how often heliostats update their tracking position 
...
struct var_land
{
	...
	spvar< bool > is_land_max_opt; 		//[] Optimize outer land boundary
	spvar< bool > is_land_max_restrict; 		//[] Restrict outer land boundary range
	spvar< double > land_const; 		//[acre] Fixed land area that is added to the area occupied by heliostats
	spvar< double > land_max_opt_max; 		//[tower-ht] Maximum outer boundary value
	spvar< double > land_max_opt_min; 		//[tower-ht] Minimum outer boundary value
...
struct var_optimize
{
	spvar< int > algorithm; 		//[] Optimization algorithm
...
struct var_parametric
{
	...
	spvar< int > fluxmap_format; 		//[] Dimensions of the fluxmap data (rows,cols)
	...
	spvar< int > sam_grid_format; 		//[none] SAM data grid format
...
struct var_plant
{
	...
	spvar< double > power_net_max; 		//[MWe] Maximum net electric power rating considered for optimization - calculated from Layout page
	spvar< double > power_net_min; 		//[MWe] Minimum net electric power rating considered for optimization - calculated from Layout page
...
struct var_receiver
{
	...
	spvar< int > accept_ang_type; 		//[none] Receiver angular acceptance window defines angles about the aperture normal, can be rectangular or elliptical shape
	...
	spvar< int > aperture_type; 		//[] The shape of the receiver aperture
	spvar< double > aspect_opt_max; 		//[m] Maximum receiver aspect ratio during optimization
	spvar< double > aspect_opt_min; 		//[m] Minimum receiver aspect ratio during optimization
	spvar< void* > cbdata; 		//[none] Data pointer for UI page
	spvar< std::string > class_name; 		//[none] Class name
	spvar< double > height_opt_max; 		//[m] Maximum receiver height during optimization
	spvar< double > height_opt_min; 		//[m] Minimum receiver height during optimization
	spvar< int > id; 		//[] Template ID
	spvar< bool > is_aspect_opt; 		//[] Optimize receiver aspect ratio (height / width)
	spvar< bool > is_aspect_restrict; 		//[] Restrict aspect ratio range (height / width)
	spvar< bool > is_enabled; 		//[] Is template enabled?
	spvar< bool > is_height_opt; 		//[] Optimize receiver height
	spvar< bool > is_height_restrict; 		//[] Restrict receiver height range
	...
	spvar< int > rec_type; 		//[none] Receiver geometrical configuration
...
struct var_solarfield
{
	...
	spvar< std::string > class_name; 		//[none] Class name
	spvar< int > des_sim_detail; 		//[none] Simulation detail for placing heliostats (see definitions in options spreadsheet)
	...
	spvar< int > hsort_method; 		//[none] Select the criteria by which heliostats will be included in the solar field layout.
	...
	spvar< bool > is_power_opt; 		//[none] Vary the power output during optimization to identify optimal level?
	spvar< bool > is_power_restrict; 		//[none] Restrict the search range for power output to the indicated limits
	...
	spvar< bool > is_tht_restrict; 		//[none] Restrict the search range for tower height to the indicated limits
	spvar< std::string > layout_data; 		//[] Layout data in string form
	spvar< int > layout_method; 		//[none] Field layout method
	...
	spvar< double > q_des_opt_max; 		//[MWt] Maximum possible solar field design power during optimzation
	spvar< double > q_des_opt_min; 		//[MWt] Minimum possible solar field design power during optimzation
	spvar< int > rad_spacing_method; 		//[none] Method for determining radial spacing during field layout for radial-stagger
	...
	spvar< int > sun_loc_des; 		//[none] Sun location when thermal power rating is achieved
	spvar< int > temp_which; 		//[none] Select the heliostat geometry template that will be used in the layout
	spvar< int > template_rule; 		//[] Method for distributing heliostat geometry templates in the field
	spvar< double > tht; 		//[m] Average height of the tower receiver centerline above the base heliostat pivot point elevation
	spvar< double > tht_opt_max; 		//[m] Maximum allowable tower height during optimization
	spvar< double > tht_opt_min; 		//[m] Minimum allowable tower height during optimization
	...
	spvar< int > xy_field_shape; 		//[] Enforced shape of the heliostat field
