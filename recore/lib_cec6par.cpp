...
static double openvoltage_194( double Voc0, double a, double IL, double IO, double Rsh )
{
/*
...
bool cec6par_module_t::operator() ( pvinput_t &input, double TcellC, double opvoltage, pvoutput_t &out )
{
	...
	air_mass *= exp(-0.0001184 * input.Elev); // 'optional' correction for elevation (m), as applied in Sandia PV model
	...
	if ( Geff_total >= 1.0 )
	{
		...
		double V_oc = openvoltage_194( Voc, A_oper, IL_oper, IO_oper, Rsh_oper );
		...
		if ( opvoltage < 0 )
		{
			...
			refdata.Io = IO_oper;
			refdata.Rs = Rs;
			refdata.Rsh = Rsh_oper;

			int maxiter = 5000;
			
			if (golden( 0, Voc, powerfunc, &refdata, 1e-4, &V, &P, maxiter))
			{
				P = -P;				
				I = 0;
				if (V != 0) I=P/V;
