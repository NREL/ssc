...
TEST_F(LiIon_lib_battery_capacity_test, replaceBatteryTest){
    ...
    compareState(s1, "replaceBatteryTest: init");

    // degrade 100% of battery
    old_cap->updateCapacityForLifetime(0);
    s1 = {0, 0, 1000, 0, 500,0, 100, 0, 2};
    compareState(s1, "replaceBatteryTest: init degradation");

    double percent = 50;
    old_cap->replace_battery(percent);
    s1 = {250, 500, 500, 0, 500,50, 50, 0, 2};
    compareState(s1, "replaceBatteryTest: 1");

    percent = 20;
    old_cap->replace_battery(percent);
    s1 = {350, 700, 700, 0, 500, 50, 50, 0, 2};
    compareState(s1, "replaceBatteryTest: 2");

    percent = 110;
    old_cap->replace_battery(percent);
    s1 = {500, 1000, 1000, 0, 500, 50, 50, 0, 2};
    compareState(s1, "replaceBatteryTest: 4");

    percent = -110;
    old_cap->replace_battery(percent);
    s1 = {500, 1000, 1000, 0, 500, 50, 50, 0, 2};
    compareState(s1, "replaceBatteryTest: 5");
}
...
TEST_F(LiIon_lib_battery_capacity_test, runSequenceTest) {
    ...
    auto s1 = capacity_state({150, 1000, 1000, 350, 0,
                              15, 85, 0, 2});
    compareState(s1, "runSequenceTest: 1");
    I = -400;
    old_cap->updateCapacity(I, dt_hour);
    s1 = capacity_state({550, 1000, 1000, -400, 0,
                              55, 45, 85, 0});
    compareState(s1, "runSequenceTest: 2");

    double percent = 80;
    old_cap->updateCapacityForThermal(percent);
    s1 = capacity_state({550, 1000, 800, -400, 0,
                         68.75, 31.25, 45, 0});
    compareState(s1, "runSequenceTest: 3");

    I = 400;
    old_cap->updateCapacity(I, dt_hour);
    s1 = capacity_state({150, 1000, 800, 400, 0,
                         18.75, 81.25, 31.25, 2});
    compareState(s1, "runSequenceTest: 4");

    I = -400;
    old_cap->updateCapacity(I, dt_hour);
    s1 = capacity_state({550, 1000, 800, -400, 0,
                         68.75, 31.25, 81.25, 0});
    compareState(s1, "runSequenceTest: 5");

    percent = 70;
    old_cap->updateCapacityForLifetime(percent);
    s1 = capacity_state({550, 700, 800, -400, 0,
                         78.6, 21.4, 31.25, 0});
    compareState(s1, "runSequenceTest: 6");

    percent = 20;
    old_cap->replace_battery(percent);
    s1 = {650, 900, 900, -400, 0, 72.2, 27.7, 21.4, 0};
    compareState(s1, "replaceBatteryTest: 7");

    I = 400;
    old_cap->updateCapacity(I, dt_hour);
    s1 = capacity_state({250, 900, 900, 400, 0,
                         27.7, 72.2, 27.7, 2});
    compareState(s1, "replaceBatteryTest: 8");
}
...
TEST_F(KiBam_lib_battery_capacity_test, updateCapacityThermalTest){
    double percent = 80;
    old_cap->updateCapacityForThermal(percent);
    auto s1 = capacity_state({54.07, 108.15, 86.5, 0, 0,
                              62.5, 37.5, 0, 2});
    compareState(s1, "updateCapacityThermalTest: 1");
    ...
    percent = 10;
    old_cap->updateCapacityForThermal(percent);
    s1 = {10.8, 108.15, 10.8, 0, 43.26, 100, 0, 0, 2};
    compareState(s1, "updateCapacityThermalTest: 3");

    percent = 110;
    old_cap->updateCapacityForThermal(percent);
    s1 = {10.8, 108.15, 10.8, 0, 43.26, 10, 90, 0, 2};
    compareState(s1, "updateCapacityThermalTest: 4");
...
TEST_F(KiBam_lib_battery_capacity_test, updateCapacityLifetimeTest){
    double percent = 80;
    old_cap->updateCapacityForLifetime(percent);
    auto s1 = capacity_state({54.07, 86.5, 108.15, 0, 0,62.5, 37.5, 0, 2});
    compareState(s1, "updateCapacityLifetimeTest: 1");
    ...
    percent = 10;
    old_cap->updateCapacityForLifetime(percent);
    s1 = {10.8, 10.8, 108.15, 0, 43.26, 100, 0, 0, 2};
    compareState(s1, "updateCapacityLifetimeTest: 3");
...
TEST_F(KiBam_lib_battery_capacity_test, replaceBatteryTest){
    ...
    double percent = 50;
    old_cap->replace_battery(percent);
    s1 = {27.0, 54.07, 54.07, 0, 54.07,50, 50, 0, 2};
    compareState(s1, "replaceBatteryTest: 1");

    percent = 20;
    old_cap->replace_battery(percent);
    s1 = {37.8, 75.7, 75.7, 0, 54.07, 50, 50, 0, 2};
    compareState(s1, "replaceBatteryTest: 2");
...
TEST_F(KiBam_lib_battery_capacity_test, runSequenceTest) {
    double I = 30;
    old_cap->updateCapacity(I, dt_hour);
    auto s1 = capacity_state({24.07, 108.1, 108.1, 30, 0,
                              22.2, 77.7, 0, 2});
    compareState(s1, "runSequenceTest: 1");
    I = -30;
    old_cap->updateCapacity(I, dt_hour);
    s1 = capacity_state({54.0, 108.1, 108.1, -30, 0,
                         50, 50, 77.7, 0});
    compareState(s1, "runSequenceTest: 2");

    double percent = 80;
    old_cap->updateCapacityForThermal(percent);
    s1 = capacity_state({54.07, 108.1, 86.5, -30, 0,
                         62.5, 37.5, 50, 0});
    compareState(s1, "runSequenceTest: 3");

    I = 40;
    old_cap->updateCapacity(I, dt_hour);
    s1 = capacity_state({20.74, 108.1, 86.5, 33.3, 0,
                         23.9, 76.0, 37.5, 2});
    compareState(s1, "runSequenceTest: 4");

    I = -40;
    old_cap->updateCapacity(I, dt_hour);
    s1 = capacity_state({60.7, 108.1, 86.5, -40, 0,
                         70.1, 29.8, 76.0, 0});
    compareState(s1, "runSequenceTest: 5");

    percent = 70;
    old_cap->updateCapacityForLifetime(percent);
    s1 = capacity_state({60.7, 75.7, 86.5, -40, 0,
                         80.2, 19.7, 29.8, 0});
    compareState(s1, "runSequenceTest: 6");

    percent = 20;
    old_cap->replace_battery(percent);
    s1 = {71.5, 97.3, 97.3, -40, 0, 73.5, 26.4, 19.7, 0};
    compareState(s1, "replaceBatteryTest: 7");

    I = 40;
    old_cap->updateCapacity(I, dt_hour);
    s1 = capacity_state({31.8, 97.3, 97.3, 39.7, 0,
                         32.7, 67.2, 19.7, 2});
    compareState(s1, "replaceBatteryTest: 8");
}