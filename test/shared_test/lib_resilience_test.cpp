                            ...
                            for (auto Qnom : {0.8, 0.9}) {
                                ...
                                for (auto C_rate : {0.05, 0.1, 0.2}) {
                                    for (auto resistance : {0.05, 0.1, 0.2}) {
                                        char buf[300];
                                        sprintf(buf,
                                                "dtHour, %f, Vfull, %f, Vexp, %f, Vnom, %f, Qfull, %f, Qexp, %f, Qnom, %f, C rate, %f, res, %f",
                                        ...
                                        try {
                                            ...
                                            for (auto q_ratio : {0.25, 0.5, 0.75}) {
                                                ...
                                                auto max_1 = voltageModel->calculate_max_discharge_w(q, qmax, 0,
                                                                                                     &current1);
                                                auto power1 = voltageModel->calculate_voltage_for_current(current1, q -
                                                                                                                    current1 *
                                                                                                                    dtHour,
                                                                                                          qmax, 0) *
                                                              current1;
...
//    auto vol = batt->battery_model->voltage_model();
    ...
    auto losses = losses_t(1, batt->lifetime_model, batt->thermal_model, batt->capacity_model,
                           batt_vars->batt_loss_choice,
                           batt_vars->batt_losses_charging, batt_vars->batt_losses_discharging,
                           batt_vars->batt_losses_idle, batt_vars->batt_losses_charging);

    batt->battery_model->initialize(batt->capacity_model, batt->voltage_model, batt->lifetime_model,
                                    batt->thermal_model, &losses);
...
TEST_F(ResilienceTest_lib_resilience, RoundtripEffTable) {
    ...
    auto vol = std::unique_ptr<voltage_table_t>(new voltage_table_t(batt_vars->batt_computed_series,
                                                                    ...
                                                                    batt_vars->batt_Vnom_default, table,
                                                                    batt_vars->batt_resistance, 1));
    auto cap = batt->battery_model->capacity_model();
    cap->change_SOC_limits(0, 100);
...
TEST_F(ResilienceTest_lib_resilience, HourlyVsSubHourly) {
    ...
    auto volt_subhourly = new voltage_dynamic_t(batt_vars->batt_computed_series, batt_vars->batt_computed_strings,
                                                ...
                                                batt_vars->batt_Qnom, batt_vars->batt_C_rate,
    ...
    while (cap_hourly->SOC() > 16) {
        double I_hourly = volt_hourly->calculate_current_for_target_w(discharge_watts, cap_hourly->q0(),
                                                                      cap_hourly->qmax(), 0);
        ...
        double I_subhourly = volt_subhourly->calculate_current_for_target_w(discharge_watts, cap_subhourly->q0(),
                                                                            cap_subhourly->qmax(), 0);
    ...
    for (size_t i = 0; i < 10; i++) {
        ...
        if (i < 5)
            EXPECT_NEAR(batt->outBatteryPower[i],
                        1. * inverter->efficiencyAC / 100. * batt_vars->batt_dc_dc_bms_efficiency / 100., 1e-3)
