    ...
    */
    ...
    for (size_t i = 0; i < latitudes.size(); i++) {
        ...
        EXPECT_NEAR((double) sun_results[4], sunrise_times[i], e)
                            << "sunrise time for lat " << latitudes[i] << " long " << longitudes[i] << " failed\n";
        EXPECT_NEAR((double) sun_results[5], sunset_times[i], e)
                            << "sunset time for lat" << latitudes[i] << " long " << longitudes[i] << "failed\n";
    }
}
...
TEST_F(NightCaseIrradProc, CalcTestRadMode0_lib_irradproc) {
    ...
    irr_hourly_night.get_sun(&sun_p[0], &sun_p[1], &sun_p[2], &sun_p[3], &sun_p[4], &sun_p[5], &sunup, &sun_p[7],
                             &sun_p[8], &sun_p[9]);
    ...
    vector<double> sun_solution = {15.400603, 125.406063, -35.406063, 20.874693, 5.707588, 19.519211, 0, 0.968315,
                                   0.886600, 0};
    for (int i = 0; i < 10; i++) {
    ...
    irr_15m_night.calc();
    irr_15m_night.get_sun(&sun_p[0], &sun_p[1], &sun_p[2], &sun_p[3], &sun_p[4], &sun_p[5], &sunup, &sun_p[7],
                          &sun_p[8], &sun_p[9]);
...
TEST_F(SunriseCaseIrradProc, CalcTestRadMode0_lib_irradproc) {
    ...
    irr_hourly_sunrise.get_sun(&sun_p[0], &sun_p[1], &sun_p[2], &sun_p[3], &sun_p[4], &sun_p[5], &sunup, &sun_p[7],
                               &sun_p[8], &sun_p[9]);
    ...
    sun_p[6] = (double) sunup;
    vector<double> sun_solution = {66.441256, 87.969757, 2.030243, 20.841844, 5.709384, 19.517827, 2.0, 0.968283,
                                   5.242355, 46.902536};
    for (int i = 0; i < 10; i++) {
    ...
    irr_15m_sunrise.get_sun(&sun_p[0], &sun_p[1], &sun_p[2], &sun_p[3], &sun_p[4], &sun_p[5], &sunup, &sun_p[7],
                            &sun_p[8], &sun_p[9]);
...
TEST_F(DayCaseIrradProc, CalcTestRadMode0_lib_irradproc) {
    ...
    irr_hourly_day.calc();
    irr_hourly_day.get_sun(&sun_p[0], &sun_p[1], &sun_p[2], &sun_p[3], &sun_p[4], &sun_p[5], &sunup, &sun_p[7],
                           &sun_p[8], &sun_p[9]);
    ...
    sun_p[6] = (double) sunup;
    vector<double> sun_solution = {171.563258, 10.938523, 79.061477, 20.791368, 5.712128, 19.515691, 1.0, 0.968318,
                                   11.886091, 1299.866650};
    for (int i = 0; i < 10; i++) {
    ...
    /* 15m during the day */
    irr_15m_day.calc();
    irr_15m_day.get_sun(&sun_p[0], &sun_p[1], &sun_p[2], &sun_p[3], &sun_p[4], &sun_p[5], &sunup, &sun_p[7], &sun_p[8],
                        &sun_p[9]);
    ...
    sun_p[6] = (double) sunup;
    sun_solution = {190.054756, 10.986005, 79.013994, 20.789459, 5.712231, 19.515691, 1.000000, 0.968318, 12.136079,
                    1299.658007};
    for (int i = 0; i < 10; i++) {
...
TEST_F(SunsetCaseIrradProc, CalcTestRadMode0_lib_irradproc) {
    ...
    irr_hourly_sunset.get_sun(&sun_p[0], &sun_p[1], &sun_p[2], &sun_p[3], &sun_p[4], &sun_p[5], &sunup, &sun_p[7],
                              &sun_p[8], &sun_p[9]);
    ...
    sun_p[6] = (double) sunup;
    vector<double> sun_solution = {292.600441, 86.774381, 3.225619, 20.739568, 5.714928, 19.513485, 3.0, 0.968355,
                                   18.643720, 74.494280};
    for (int i = 0; i < 10; i++) {
    ...
    /* 15m during sunset */
    irr_15m_sunset.calc();
    irr_15m_sunset.get_sun(&sun_p[0], &sun_p[1], &sun_p[2], &sun_p[3], &sun_p[4], &sun_p[5], &sunup, &sun_p[7],
                           &sun_p[8], &sun_p[9]);
    ...
    sun_p[6] = (double) sunup;
    sun_solution = {292.600441, 86.774381, 3.225619, 20.739568, 5.714928, 19.513485, 3.0, 0.968355, 18.643720,
                    74.494280};
    for (int i = 0; i < 10; i++) {
...
TEST_F(BifacialIrradTest, TestSkyConfigFactors) {
    ...
    irr->getSkyConfigurationFactors(rowToRow, verticalHeight, clearanceGround, distanceBetweenRows, horizontalLength,
...
TEST_F(BifacialIrradTest, TestGroundShadeFactors) {
    for (size_t s = 0; s < numberOfSamples; s++) {
        ...
        irr->getGroundShadeFactors(rowToRow, verticalHeight, clearanceGround, distanceBetweenRows, horizontalLength,
                                   irr->get_sun_component(0), irr->get_sun_component(2), rearGroundShade,
                                   frontGroundShade, maxShadow, pvBackShadeFraction, pvFrontShadeFraction);
...
TEST_F(BifacialIrradTest, TestFrontSurfaceIrradiance) {
    for (size_t s = 0; s < numberOfSamples; s++) {
        ...
        irr->getFrontSurfaceIrradiances(expectedPVFrontShadeFraction[t], rowToRow, verticalHeight, clearanceGround,
                                        distanceBetweenRows, horizontalLength, expectedFrontGroundGHI, frontIrradiance,
...
TEST_F(BifacialIrradTest, TestRearSurfaceIrradiance) {
    for (size_t s = 0; s < numberOfSamples; s++) {
        ...
        irr->getBackSurfaceIrradiances(expectedPVRearShadeFraction[t], rowToRow, verticalHeight, clearanceGround,
                                       distanceBetweenRows, horizontalLength, expectedRearGroundGHI,
                                       expectedFrontGroundGHI, expectedFrontReflected, rearIrradiance,
