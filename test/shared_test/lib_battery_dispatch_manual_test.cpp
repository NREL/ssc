...
TEST_F(ManualTest_lib_battery_dispatch_losses, TestLossesWithDispatch)
{
    ...
    // Test max charge power constraint
    batteryPower->powerPV = 40; batteryPower->voltageSystem = 600;
    dispatchManual->dispatch(year, hour_of_year, step_of_hour);
    EXPECT_NEAR(batteryPower->powerPVToBattery, batteryPower->powerPV - batteryPower->powerSystemLoss, 0.1);

    // Test max discharge power constraint
    batteryPower->powerPV = 0; batteryPower->voltageSystem = 600; batteryPower->powerLoad = 40;
    dispatchManual->dispatch(year, hour_of_year, step_of_hour);
