...
protected:
    ...
    void SetUp() {
        ...
        const double tmpValues[33] = {0, 0, 50, 16, 21, 50, 25, 25, 50, 34, 32, 50, 44, 37, 50, 53, 42, 50, 62, 47, 49,
                                      72, 50, 48, 82, 52, 47, 90, 52, 46, 100, 51, 45};
...
public:
    ...
    void SetUp() {
        ...
        fuelCell = new FuelCell(unitPowerMax_kW, unitPowerMin_kW, startup_hours, is_started, shutdown_hours,
                                ...
                                lowerHeatingValue_BtuPerFt3, higherHeatingValue_BtuPerFt3, availableFuel_Mcf,
                                shutdownOption, dt_hour);
...
*/
        fuelCellSubHourly = new FuelCell(unitPowerMax_kW, unitPowerMin_kW, startup_hours, is_started, shutdown_hours,
                                         dynamicResponseUp_kWperHour, dynamicResponseDown_kWperHour,
                                         degradation_kWperHour, degradationRestart_kW,
                                         replacementOption, replacement_percent, replacementSchedule, shutdownTable,
                                         efficiencyChoice, efficiencyTable, lowerHeatingValue_BtuPerFt3,
                                         higherHeatingValue_BtuPerFt3, availableFuel_Mcf, shutdownOption, dt_subHourly);
...
*/
        fuelCellDispatch = new FuelCellDispatch(fuelCell, numberOfUnits, dispatchOption, shutdownOption, dt_hour,
                                                fixed_percent,
                                                dispatchInput_kW, canCharge, canDischarge, discharge_percent,
                                                discharge_units, scheduleWeekday, scheduleWeekend);

        fuelCellDispatchSubhourly = new FuelCellDispatch(fuelCellSubHourly, numberOfUnits, dispatchOption,
                                                         shutdownOption, dt_subHourly, fixed_percent,
                                                         dispatchInput_kW, canCharge, canDischarge, discharge_percent,
                                                         discharge_units, scheduleWeekday, scheduleWeekend);

        discharge_units[0] = n_multipleFuelCells;
        fuelCellDispatchMultiple = new FuelCellDispatch(fuelCell, n_multipleFuelCells, dispatchOption, shutdownOption,
                                                        dt_hour, fixed_percent,
                                                        dispatchInput_kW, canCharge, canDischarge, discharge_percent,
                                                        discharge_units, scheduleWeekday, scheduleWeekend);
