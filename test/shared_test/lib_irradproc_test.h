...
protected:
	...
	void runIrradCalc(size_t index)
	{
		...
		readDataFromTextFile(weatherFile, index, expectedWeather);
	...
	void readDataFromTextFile(std::string fileName, size_t lineNumber, std::vector<double> & data)
	{
		...
		*/
		...
		if (dataFile.is_open())
		{
			...
			std::vector<std::string> tmpData(begin, end);
			std::copy(tmpData.begin(), tmpData.end(), std::ostream_iterator<std::string>(std::cout, "\n"));
