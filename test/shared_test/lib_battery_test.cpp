...
TEST_F(lib_battery_test, createFromParams) {
    ...
    double current = 10;
    double P_orig = batteryModel->runCurrent(0, current);
    current = 10;
    double P_new = bat.runCurrent(0, current);
...
TEST_F(lib_battery_test, AugmentCapacity) {
    ...
    for (size_t y = 0; y < replacement_schedule.size(); y++) {
        for (size_t t = 0; t < 8760; t++) {
            ...
            batteries[replaceCount]->runCurrent(i, current);
        }
