...
/*	SSC Var Name							Data Type			Test Values				Length,Width */
	...
	{"en_low_temp_cutoff",					NUM,				"0"									},
	{"en_icing_cutoff",						NUM,				"0"									},
	{"low_temp_cutoff",						NUM,				"0"									},
	{"icing_cutoff_temp",					NUM,				"0"									},
	{"icing_cutoff_rh",						NUM,				"0"									}
};
...
/*
 * Test 2
 * Defaults with Leap Year File provide same results as default values
 */  
...
/*
* Test 3
* Case where measurement heights are different from the turbine's hub height
* HubHeight is 200, creates an error
*/
TestInfo hubHeightErrorInfo[] = {
/*	SSC Var Name							Data Type			Test Values				Length,Width */
	{"wind_resource_filename",				STR,				"" },
	{"wind_resource_data",					DAT,				windresourcedata },
	{"wind_turbine_hub_ht",					NUM,				"200"}
};

TestResult hubHeightErrorResult[] = {
/*	SSC Var Name							Test Type			Test Result				Error Bound Ratio */
	{										ERR }
};
...
* Test 4
* Case where measurement heights are different from the turbine's hub height
* HubHeight is 90, use shear exponent interpolation; result should be greater than that of lower height
*/
TestInfo hubHeightInterpolateInfo[] = {
/*	SSC Var Name							Data Type			Test Values				Length,Width */
	{"wind_resource_filename",				STR,				""},
	{"wind_resource_data",					DAT,				windresourcedata },
	{"wind_turbine_hub_ht",					NUM,				"90" }
};

TestResult hubHeightInterpolateResult[] = {
/*	SSC Var Name							Test Type			Test Result				Error Bound Ratio */
...
/*
* Test 5
* Using WAsp Wake model instead of Simple Wake Model (default)
* Different wake model produces different results
*/
TestInfo waspModelInfo[] = {
/*	SSC Var Name							Data Type			Test Values				Length,Width */
	{"wind_farm_wake_model",				NUM,				"1"}
};

TestResult waspModelResult[] = {
	{"annual_energy",						NR,					32346158,				.1 },
	{"monthly_energy[0]",					NR,					2.7472e6,				.1 },
	{"monthly_energy[11]",					NR,					2.7472e6,				.1 }
};

testDeclaration waspModelTest(windPowerTesting, "waspModel", &waspModelInfo[0], 1, &waspModelResult[0], 3);


/*
* Test 6
* Using Eddy Viscosity Wake model instead of Simple Wake Model (default)
* Different wake model produces different results
*/
TestInfo EVModelInfo[] = {
	{"wind_farm_wake_model",				NUM,				"2" }
};

TestResult EVModelResult[] = {
	{"annual_energy",						NR,					31081848,				.1 },
	{"monthly_energy[0]",					NR,					2.6398e6,				.1 },
	{"monthly_energy[11]",					NR,					2.6398e6,				.1 }
};

testDeclaration EVModelTest(windPowerTesting, "waspModel", &EVModelInfo[0], 1, &EVModelResult[0], 3);


/*
* Test 7
* Using test 30min file
* Produces same results as with test hourly file, but gen array should be twice as long
*/
TestInfo thirtyMinInfo[] = {
	{"wind_resource_filename",				STR,				thirtyMin }
};

TestResult thirtyMinResult[] = {
	{"annual_energy",						NR,					33224154,				.1 },
	{"monthly_energy[0]",					NR,					2.8218e6,				.1 },
	{"monthly_energy[11]",					NR,					2.8218e6,				.1 },
	{"gen",									SIZE,				8760 * 2,				0 }
...
testDeclaration thirtyMinTest(windPowerTesting, "waspModel", &thirtyMinInfo[0], 1, &thirtyMinResult[0], 4);
...
*/
TestInfo weibullInfo[] = {
	{"wind_resource_model_choice",			NUM,				"1" }
};

TestResult weibullResult[] = {
	{"annual_energy",						NR,					180453760,				.1 },
	{"monthly_energy[0]",					NR,					15326247,				.1 },
	{"monthly_energy[11]",					NR,					15326247,				.1 }
};

testDeclaration weibullTest(windPowerTesting, "waspModel", &weibullInfo[0], 1, &weibullResult[0], 3);
