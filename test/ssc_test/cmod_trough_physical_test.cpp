...
TEST_F(CMTroughPhysical, DefaultNoFinancialModel_cmod_trough_physical) {
    ...
    if (!test_errors) {
        ...
        EXPECT_NEAR(annual_energy, 369099144.268370, 369099144.268370 * m_error_tolerance_lo)
                            << "Annual Net Thermal Energy Production";

        ssc_number_t annual_thermal_consumption;
        ssc_data_get_number(data, "annual_thermal_consumption", &annual_thermal_consumption);
        EXPECT_NEAR(annual_thermal_consumption, 404528.651833, 404528.651833 * m_error_tolerance_lo)
                            << "Annual Thermal Freeze Protection";
        ...
        EXPECT_NEAR(annual_tes_freeze_protection, 366491.965635, 366491.965635 * m_error_tolerance_lo)
                            << "Annual TES Freeze Protection";
        ...
        EXPECT_NEAR(annual_field_freeze_protection, 38036.686198, 38036.686198 * m_error_tolerance_lo)
                            << "Annual Field Freeze Protection";
        ...
        EXPECT_NEAR(annual_W_cycle_gross, 424232748.142327, 424232748.142327 * m_error_tolerance_lo)
                            << "Power cycle gross electrical output";
        ...
        EXPECT_NEAR(annual_total_water_use, 81059.873491, 81059.873491 * m_error_tolerance_lo)
