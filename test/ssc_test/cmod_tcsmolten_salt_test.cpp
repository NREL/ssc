...
TEST_F(CMTcsMoltenSalt, SCO2_Default_SingleOwner_cmod_tcsmolten_salt) {
	...
	if (!test_errors)
	{
		...
		ssc_number_t N_hel;
		ssc_data_get_number(data, "N_hel", &N_hel);
		EXPECT_NEAR(N_hel, 8790, 8790 * m_error_tolerance_lo) << "Number of Heliostats"; 

		ssc_number_t rec_height;
		ssc_data_get_number(data, "rec_height", &rec_height);
		EXPECT_NEAR(rec_height, 21.6029, 21.6029 * m_error_tolerance_lo) << "Rec Height";  

		ssc_number_t A_sf;
		ssc_data_get_number(data, "A_sf", &A_sf);
		EXPECT_NEAR(A_sf, 1.26905e6, 1.26905e6 * m_error_tolerance_lo) << "Solar Field Area";  

		ssc_number_t D_rec;
		ssc_data_get_number(data, "D_rec", &D_rec);
		EXPECT_NEAR(D_rec, 17.65, 17.65 * m_error_tolerance_lo) << "Receiver Outer Diameter";  

		ssc_number_t annual_total_water_use;
		ssc_data_get_number(data, "annual_total_water_use", &annual_total_water_use);
		EXPECT_NEAR(annual_total_water_use, 55965.3, 55965.3 * m_error_tolerance_lo) << "Annual Total Water Use";  

		ssc_number_t csp_pt_cost_total_land_area;
		ssc_data_get_number(data, "csp.pt.cost.total_land_area", &csp_pt_cost_total_land_area);
		EXPECT_NEAR(csp_pt_cost_total_land_area, 1892.04, 1892.04 * m_error_tolerance_lo) << "Total Land Area";  

		ssc_number_t h_tower;
		ssc_data_get_number(data, "h_tower", &h_tower);
		EXPECT_NEAR(h_tower, 193.458, 193.458 * m_error_tolerance_lo) << "Tower Height";  
	}
}
...
TEST_F(CMTcsMoltenSalt, SCO2_Partial_Cooling_SingleOwner_cmod_tcsmolten_salt) {
	...
	if (!test_errors)
	{
		...
		ssc_number_t land_area_base;
		ssc_data_get_number(data, "land_area_base", &land_area_base);
		EXPECT_NEAR(land_area_base, 1847.04, 1847.04 * m_error_tolerance_lo) << "Land Area Base";  
...
TEST_F(CMTcsMoltenSalt, ACTS_sCO2_recompression) {
	...
	for (std::vector<double>::size_type i = 0; i != annual_energys.size(); i++) {
		...
		if (!test_errors)
		{
			...
			ssc_number_t land_area_base;
			ssc_data_get_number(data, "land_area_base", &land_area_base);
			EXPECT_NEAR(land_area_base, land_area_bases[i], land_area_bases[i] * m_error_tolerance_lo) << "Land Area Base";
			...
			ssc_number_t N_hel;
			ssc_data_get_number(data, "N_hel", &N_hel);
			EXPECT_NEAR(N_hel, N_hels[i], N_hels[i] * m_error_tolerance_lo) << "Number of Heliostats";

			ssc_number_t rec_height;
			ssc_data_get_number(data, "rec_height", &rec_height);
			EXPECT_NEAR(rec_height, rec_heights[i], rec_heights[i] * m_error_tolerance_lo) << "Rec Height";

			ssc_number_t A_sf;
			ssc_data_get_number(data, "A_sf", &A_sf);
			EXPECT_NEAR(A_sf, A_sfs[i], A_sfs[i] * m_error_tolerance_lo) << "Solar Field Area";

			ssc_number_t D_rec;
			ssc_data_get_number(data, "D_rec", &D_rec);
			EXPECT_NEAR(D_rec, D_recs[i], D_recs[i] * m_error_tolerance_lo) << "Receiver Outer Diameter";

			ssc_number_t annual_total_water_use;
			ssc_data_get_number(data, "annual_total_water_use", &annual_total_water_use);
			EXPECT_NEAR(annual_total_water_use, annual_total_water_uses[i], annual_total_water_uses[i] * m_error_tolerance_lo) << "Annual Total Water Use";

			ssc_number_t csp_pt_cost_total_land_area;
			ssc_data_get_number(data, "csp.pt.cost.total_land_area", &csp_pt_cost_total_land_area);
			EXPECT_NEAR(csp_pt_cost_total_land_area, csp_pt_cost_total_land_areas[i], csp_pt_cost_total_land_areas[i] * m_error_tolerance_lo) << "Total Land Area";

			ssc_number_t h_tower;
			ssc_data_get_number(data, "h_tower", &h_tower);
			EXPECT_NEAR(h_tower, h_towers[i], h_towers[i] * m_error_tolerance_lo) << "Tower Height";
