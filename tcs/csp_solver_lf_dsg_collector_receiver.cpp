	...
	C_csp_collector_receiver::S_csp_cr_solved_params & solved_params)
{
	...
	*/
	...
	double q_rec_tot_des = q_rec_tot_loop*(double)m_nLoops;			//[kWt] SYSTEM total design incident thermal power after *optical* losses
	double q_loss_tot_des = q_loss_tot_loop*(double)m_nLoops + q_loss_piping;
	m_q_dot_abs_tot_des = q_rec_tot_des - q_loss_tot_des;			//[kWt]

	double eta_therm_sf_des = 1.0 - q_loss_tot_des / q_rec_tot_des;		//[-] Design solar field efficiency
	...
	const C_csp_solver_sim_info &sim_info)
{
	...
	for (int i = 0; i < n_steps_recirc; i++)
	{
		...
		if (m_T_htf_out_t_end[m_nModTot - 1] < m_T_fp + 10.0)
		{
			...
			double T_htf_guess_lower = (m_Q_field_losses_total / sim_info.ms_ts.m_step)*1.E6 /
				(m_c_htf_ave_ts_ave_temp * m_m_dot_tot) + T_cold_in;	//[K]
	...
	cr_out_solver.m_T_field_out_C = mc_sys_h_out.m_temp - 273.15;		//[C]
	cr_out_solver.m_h_htf_hot = mc_sys_h_out.m_enth;			//[kJ/kg]
	cr_out_solver.m_xb_htf_hot = mc_sys_h_out.m_x;				//[-]
	cr_out_solver.m_P_htf_hot = m_P_turb_des*100.0;				//[kPa]
	...
	cr_out_solver.m_m_dot_tot = m_m_dot_tot*3600.0;		//[kg/hr] Total HTF mass flow rate
	...
	const C_csp_solver_sim_info &sim_info)
{
	...
	for (i_step = 0; i_step < n_steps_recirc; i_step++)
	{
		...
		// Add current temperature so summation
		T_sys_h_t_int_sum += mc_sys_h_out.m_temp;	//[K]
...
int C_csp_lf_dsg_collector_receiver::C_mono_eq_freeze_prot_E_bal::operator()(double T_htf_cold_in /*K*/, double *E_loss_balance /*-*/)
{
	...
	Q_htf_fp = mpc_csp->m_m_dot_tot*
		(mpc_csp->mc_sys_c_in.m_enth - mpc_csp->mc_sys_h_out.m_enth) / 1.E6*(mpc_csp->ms_loop_energy_balance_inputs.ms_sim_info->ms_ts.m_step);	//[MJ]
...
int C_csp_lf_dsg_collector_receiver::C_mono_eq_xb_loop_out::operator()(double m_dot_htf_loop /*kg/s*/, double *xb_loop_out /*-*/)
{
	...
	// Set the outlet temperature at end of timestep
	*xb_loop_out = mpc_csp->mc_sys_h_out.m_x;
	...
	const C_csp_solver_sim_info &sim_info)
{
	...
	double SolarZen = weather.m_solzen*0.0174533;;		//Solar zenith angle [deg]
	m_m_dot_tot = m_dot_htf_loop*float(m_nLoops);
	for (int i = 0; i < m_nModTot; i++)
	...
	m_q_rec_loop = 0.0;			//[kWt] LOOP total thermal power on receiver after *optical* losses and *defocus*
	m_eta_opt_ave = 0.0;		//[-] Weighted average optical efficiency
	for (int i = 0; i < m_nModTot; i++)
	{
		// Calculate the energy on the receiver
		m_q_rec[i] = m_q_inc[i]*m_eta_optical[gset];		//[kWt] Incident thermal power on receiver after *optical* losses
		// Average optical efficiency
		m_eta_opt_ave += m_eta_optical[gset]*m_A_aperture.at(gset, 0)*m_nLoops / m_Ap_tot;	//[-] Weighted average optical efficiency
		m_q_rec_loop += m_q_rec[i];			//[kWt] LOOP total thermal power on receiver after *optical* losses and *defocus*
	}
	...
	double dP_loss = pow(m_m_dot_tot / m_m_dot_des, 2)*m_P_turb_des;
	...
	mc_sys_c_in.m_enth = wp.enth;	//[kJ/kg]
	...
	double dh_guess = (h_b_out_guess - mc_sys_c_in.m_enth) / (double)m_nModTot;
	for (int i = 0; i < m_nModTot; i++)
	{
		m_h_ave.at(i, 0) = mc_sys_c_in.m_enth + dh_guess*(double)(i + 1) - dh_guess / 2.0;
	}
	
	// Set the loop inlet enthalpy
	m_h_in.at(0, 0) = mc_sys_c_in.m_enth;
	...
	for (int i = 0; i < m_nModTot; i++)
	{
		...
		if (m_HLCharType.at(gset, 0) == 1)
		{
			...
			m_q_loss[i] = c_hl*m_L_col.at(gset, 0) / 1000.0;		//[kW] Total thermal loss from this collector
			m_q_abs[i] = m_q_rec[i] - m_q_loss[i];		//[kWt] Thermal power absorbed by steam in each receiver
		}
		else if (m_HLCharType.at(gset, 0) == 2)
		{
		...
		int water_code = water_PH(P_loc * 100.0, m_h_out.at(i,0), &wp);
		if (water_code != 0)
		{
			return -1;
		}
	...
	}   // End step through receivers in flow path:     for (int i = 0; i < m_nModTot; i++)

	// Piping thermal loss
	double q_loss_piping = m_Ap_tot * m_Pipe_hl_coef / 1000.0 * ((T_htf_cold_in + mc_sys_h_out.m_temp) / 2.0 - (T_db - 273.15));	// hl coef is [W/m2-K], use average field temp as driving difference
		
	// Given the piping heat/pressure loss, calculate the temperature at the inlet to the power block
	//double h_field_out = 0.0;
	if (m_m_dot_tot > 0.0)
		mc_sys_h_out.m_enth = m_h_out.at(m_nModTot - 1, 0) - q_loss_piping / m_m_dot_tot;	//[kJ/kg]
	else
		mc_sys_h_out.m_enth = m_h_out.at(m_nModTot - 1, 0);			//[kJ/kg]

	water_PH(P_loop_out*100.0, mc_sys_h_out.m_enth, &wp);
	//m_T_field_out = wp.temp - 273.15;		// [C]
	mc_sys_h_out.m_temp = wp.temp;		//[C]
	mc_sys_h_out.m_x = wp.qual;		//[-]
	...
	for (int i = 0; i < m_nModTot; i++)
	{
		...
		Q_rec_tot += m_q_rec[i];		//[kWt] LOOP total thermal power on receiver after *optical* losses and *defocus*
		Q_abs_tot += m_q_abs[i];		//[kWt] Thermal power absorbed by steam in each receiver
...
//			m_q_inc[i] = I_bn*m_A_aperture.at(gset, 0) / 1000.0;		//[kWt] Incident beam radiation for each receiver in loop
//			// Calculate the energy on the receiver
//			m_q_rec.at(i, 0) = m_q_inc[i]*m_eta_optical.at(gset, 0)*defocus_lim;	//[kWt] Incident thermal power on receiver after *optical* losses and *defocus*
//			// Average optical efficiency
//			eta_opt_ave += m_eta_optical.at(gset, 0)*m_A_aperture.at(gset, 0)*m_nLoops / m_Ap_tot;
//			q_rec_loop += m_q_rec.at(i, 0);
//			q_inc_loop += m_q_inc[i];			//[kWt] LOOP total incident beam radiation
//		}
//
//		// For nighttime conditions, calculate the solar field inlet temperature
//		if (q_rec_loop == 0.0 && m_ffrac[tou_period] < m_cycle_cutoff_frac)
