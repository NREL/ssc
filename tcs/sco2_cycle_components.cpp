...
void calculate_turbomachinery_outlet_1(double T_in /*K*/, double P_in /*kPa*/, double P_out /*kPa*/,
    ...
    calculate_turbomachinery_outlet_1(T_in, P_in, P_out, eta_isen, is_comp, error_code, enth_in, entr_in, dens_in,
                                      temp_out, enth_out, entr_out, dens_out, spec_work);
}
...
int Ph_dome(double P_low /*MPa*/, std::vector<double> &P_data /*MPa*/, std::vector<double> &h_data) {
    ...
    int T_P_target_code = P_x0_solver.solve(T_crit - 10.0, T_crit - 20.0, P_low * 1.E3, T_P_target_solved,
                                            tol_T_P_target_solved, iter_T_P_target);
...
void
C_turbine::off_design_turbine(double T_in, double P_in, double P_out, double N, int &error_code, double &m_dot_cycle,
                              double &T_out) {
    /* 9.4.14: code from John Dyreby, converted to C++ by Ty Neises
    ...
    double eta_0 = (((1.0626 * ms_od_solved.m_nu - 3.0874) * ms_od_solved.m_nu + 1.3668) * ms_od_solved.m_nu + 1.3567) *
                   ms_od_solved.m_nu + 0.179921180;
    ...
    ms_od_solved.m_w_tip_ratio =
            U_tip / ssnd_in;        //[-] Ratio of the tip speed to the local (turbine inlet) speed of sound
    ms_od_solved.m_N = N;
...
void C_turbine::od_turbine_at_N_des(double T_in, double P_in, double P_out, int &error_code, double &m_dot_cycle,
                                    double &T_out) {
...
int C_comp__psi_eta_vs_phi::design_given_shaft_speed(double T_in /*K*/, double P_in /*kPa*/, double m_dot /*kg/s*/,
    ...
    ms_des_solved.m_psi_max_at_N_des = calc_psi_isen(ms_des_solved.m_phi_surge, 1.0, T_in,
                                                     P_in);  //[-] max ideal head coefficient at design shaft speed
...
int C_comp__psi_eta_vs_phi::off_design_given_N(double T_in /*K*/, double P_in /*kPa*/, double m_dot /*kg/s*/,
                                               double N_rpm /*rpm*/,
                                               double &T_out /*K*/, double &P_out /*kPa*/) {
    CO2_state co2_props;
    ...
    ms_od_solved.m_eta = std::max(eta_ND_od * ms_des_solved.m_eta_design,
                                  0.0);        //[-] Actual compressor efficiency, not allowed to go negative
    ...
    double dh_s = psi * pow(U_tip, 2.0) *
                  0.001;            //[kJ/kg] Ideal enthalpy rise in compressor, from definition of head coefficient
...
int C_comp__psi_eta_vs_phi::calc_N_from_phi(double T_in /*K*/, double P_in /*kPa*/, double m_dot /*kg/s*/,
                                            double phi_in /*-*/, double &N_rpm /*rpm*/) {
    CO2_state co2_props;
...
int C_comp__psi_eta_vs_phi::calc_m_dot__phi_des(double T_in /*K*/, double P_in /*kPa*/, double N_rpm /*rpm*/,
                                                double &m_dot /*kg/s*/) {
    CO2_state co2_props;
...
std::unique_ptr<C_comp__psi_eta_vs_phi>
C_comp__psi_eta_vs_phi::construct_derived_C_comp__psi_eta_vs_phi(int comp_model_code) {
    if (comp_model_code == E_snl_radial_via_Dyreby) {
        ...
        throw (C_csp_exception(
                "C_comp__psi_eta_vs_phi::construct_derived_C_comp__psi_eta_vs_phi unrecognized compressor model code"));
    }
}
...
double
C_comp__snl_radial_via_Dyreby::calc_psi_isen(double phi_in /*-*/, double N_des_over_N_od /*-*/, double T_comp_in /*K*/,
    ...
    if (phi >= m_phi_min) {
        psi = ((((-498626.0 * phi) + 53224.0) * phi - 2505.0) * phi + 54.6) * phi +
              0.04049;  // from dimensionless modified head curve(at design - point, psi and modified psi are equal)
    } else {
        double psi_at_surge =
                ((((-498626.0 * m_phi_min) + 53224.0) * m_phi_min - 2505.0) * m_phi_min + 54.6) * m_phi_min +
                0.04049;  // from dimensionless modified head curve(at design - point, psi and modified psi are equal)
        psi = (1 + 0.5 * (m_phi_min - phi) / m_phi_min) *
              psi_at_surge;        //[-] Check for surge after model converges
    }
...
double C_comp__snl_radial_via_Dyreby::calc_eta_normalized(double phi_in /*-*/, double N_des_over_N_od /*-*/,
                                                          double T_comp_in /*K*/, double P_comp_in /*kPa*/) {
    double phi = adjust_phi_for_N(phi_in, N_des_over_N_od);

    double eta_star = ((((-1.638e6 * phi) + 182725.0) * phi - 8089.0) * phi + 168.6) * phi -
                      0.7069;    // from dimensionless modified efficiency curve
...
int C_comp_multi_stage::C_MEQ_eta_isen__h_out::operator()(double eta_isen /*-*/, double *h_comp_out /*kJ/kg*/) {
    ...
    if (N_rpm_code != C_monotonic_eq_solver::CONVERGED) {
        if (!(N_rpm_code > C_monotonic_eq_solver::CONVERGED && fabs(tol_solved) < 0.01)) {
            throw (C_csp_exception(
                    "C_comp_multi_stage::C_MEQ_eta_isen__h_out failed to converge within a reasonable tolerance"));
        }
    }
...
int C_comp_multi_stage::C_MEQ_N_rpm__P_out::operator()(double N_rpm /*rpm*/, double *P_comp_out /*kPa*/) {
    ...
    for (int i = 0; i < n_stages; i++) {
        ...
        mpc_multi_stage->mv_c_stages[i] = C_comp__psi_eta_vs_phi::construct_derived_C_comp__psi_eta_vs_phi(
                mpc_multi_stage->m_compressor_model);
        comp_err_code = mpc_multi_stage->mv_c_stages[i]->design_given_shaft_speed(T_in, P_in, m_m_dot_basis, N_rpm,
                                                                                  m_eta_isen, P_out, T_out, tip_ratio);
...
int C_comp_multi_stage::design_given_outlet_state(int comp_model_code, double T_in /*K*/, double P_in /*kPa*/,
                                                  double m_dot_cycle /*kg/s*/,
                                                  double T_out /*K*/, double P_out /*K*/) {
    ...
    if (mv_c_stages[0]->ms_des_solved.m_tip_ratio > tip_speed_limit) {
        ...
        while (is_add_stages) {
            ...
            try {
                eta_isen_code = c_solver.solve(eta_isen_guess_1, eta_isen_guess_2, h_out, eta_isen_solved, tol_solved,
                                               iter_solved);
            }
            catch (C_csp_exception) {
            ...
            if (eta_isen_code != C_monotonic_eq_solver::CONVERGED) {
                if (!(eta_isen_code > C_monotonic_eq_solver::CONVERGED && fabs(tol_solved) < 0.01)) {
                    throw (C_csp_exception(
                            "C_comp_multi_stage::design_given_outlet_state failed to converge within a reasonable tolerance"));
                }
            }
...
int C_comp_multi_stage::calc_m_dot__N_des__phi_des_first_stage(double T_in /*K*/, double P_in /*kPa*/,
                                                               double &m_dot_cycle /*kg/s*/) {
...
void C_comp_multi_stage::off_design_given_N(double T_in /*K*/, double P_in /*kPa*/, double m_dot_in /*kg/s*/,
                                            double N_rpm /*rpm*/,
                                            int &error_code, double &T_out /*K*/, double &P_out /*kPa*/) {
...
void C_comp_multi_stage::off_design_given_P_out(double T_in /*K*/, double P_in /*kPa*/, double m_dot_cycle /*kg/s*/,
    ...
    if (test_code != 0) {
        for (int i = 1; i < 9; i++) {
            phi_guess_lower =
                    ms_des_solved.m_phi_des * (10 - i) / 10.0 + mv_c_stages[0]->ms_des_solved.m_phi_max * i / 10.0;
