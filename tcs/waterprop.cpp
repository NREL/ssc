    ...
    if (P <= water_max_sat_pres) {
        ...
        if (S > SSat[1]) {
            ...
            if (S < water_vapor_entr_values[firstIndex]) {
                ...
                double sFraction = (S - water_vapor_entr_values[sIndex]) /
                                   (water_vapor_entr_values[sIndex + 1] - water_vapor_entr_values[sIndex]);
            ...
            if (S > water_liquid_entr_values[lastIndex]) {
                double sFraction =
                        (S - water_liquid_entr_values[lastIndex]) / (SSat[0] - water_liquid_entr_values[lastIndex]);
                int lastCoefIndex = water_liquid_entr_index_vector[pIndex + 1] - pIndex - 2;
                LiquidGap(lastCoefIndex, TSat, sFraction, data);
            } else {
                ...
                double sFraction = (S - water_liquid_entr_values[sIndex]) /
                                   (water_liquid_entr_values[sIndex + 1] - water_liquid_entr_values[sIndex]);
...
int VaporNonGap(int pIndex, double pFraction, property_info *data, int edgeInd, double indVar) {
    ...
    double sFraction = cubic_solution(pAdjustedCoefs[0], pAdjustedCoefs[1], pAdjustedCoefs[2], pAdjustedCoefs[3],
                                      indVar);
    if (fabs(sFraction) > 1.) return error_code = 9;

    /// calculate S
    int sIndex = water_vapor_entr_index_vector[pIndex] + rowInd;
    (*data).S = water_vapor_entr_values[sIndex] +
                sFraction * (water_vapor_entr_values[sIndex + 1] - water_vapor_entr_values[sIndex]);
...
void LiquidNonGap(int pIndex, double pFraction, property_info *data, int edgeInd, double indVar) {
    ...
    (*data).S = water_liquid_entr_values[sIndex] +
                sFraction * (water_liquid_entr_values[sIndex + 1] - water_liquid_entr_values[sIndex]);
