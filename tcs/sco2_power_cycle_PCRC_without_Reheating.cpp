#include "stdafx.h"
#include "sco2_power_cycle_PCRC_without_Reheating.h"
//#include "co2props.h"
...
double P_pseudocritical_PCRC_without_Reheating(double T_K)
{
	return (0.191448*T_K + 45.6661)*T_K - 24213.3;
}

double HeatExchanger_PCRC_without_Reheating::hxr_DP(int stream, double m_dot, bool scale_DP /*kPa*/)
{
...
double HeatExchanger_PCRC_without_Reheating::hxr_UA(double m_dot_0, double m_dot_1, bool scale_UA /*kW/K*/)
{
...
void calculate_turbomachinery_outlet_PCRC_without_Reheating(double T_in /*K*/, double P_in /*kPa*/, double P_out /*kPa*/, double eta /*-*/, bool is_comp, int & error_code, double & enth_in /*kJ/kg*/, double & entr_in /*kJ/kg-K*/,
...
void calculate_hxr_UA_PCRC_without_Reheating(int N_hxrs, double Q_dot /*units?*/, double m_dot_c, double m_dot_h, double T_c_in, double T_h_in, double P_c_in, double P_c_out, double P_h_in, double P_h_out,
...
void compressor_PCRC_without_Reheating::solve_compressor(double T_in, double P_in, double m_dot, double N_in, int & error_code, double & T_out, double & P_out)
{
...
void turbine_PCRC_without_Reheating::solve_turbine(double T_in, double P_in, double P_out, double N_in, int & error_code, double & T_out, double & m_dot_out)
{
	m_N = N_in;
...
bool RecompCycle_PCRC_without_Reheating::design()
{
	int max_iter = 100;
	...
	int cpp_offset = 1;
	m_temp_last[1 - cpp_offset] = m_cycle_des_par.m_T_mc2_in;
	m_temp_last[11 - cpp_offset] = m_cycle_des_par.m_T_mc1_in;
	m_pres_last[11 - cpp_offset] = m_cycle_des_par.m_P_mc1_in;
	m_pres_last[12 - cpp_offset] = m_cycle_des_par.m_P_mc1_out;
	double P_mc2_in = m_cycle_des_par.m_P_mc2_out / m_cycle_des_par.m_PR_mc2;
	m_pres_last[1 - cpp_offset] = P_mc2_in;
	m_pres_last[2 - cpp_offset] = m_cycle_des_par.m_P_mc2_out;
	...
	if (m_cycle_des_par.m_DP_PC1[2 - cpp_offset] < 0.0)
		m_pres_last[9 - cpp_offset] = m_pres_last[11 - cpp_offset] / (1.0 - abs(m_cycle_des_par.m_DP_PC1[2 - cpp_offset]));			// relative pressure drop specified for precooler [P1 = P9 - P9*rel_DP => P1 = P9*(1-rel_DP)
	else
		m_pres_last[9 - cpp_offset] = m_pres_last[11 - cpp_offset] + m_cycle_des_par.m_DP_PC1[2 - cpp_offset];										// absolute pressure drop specified for precooler

	if (m_cycle_des_par.m_DP_PC2[2 - cpp_offset] < 0.0)
		m_pres_last[12 - cpp_offset] = m_pres_last[1 - cpp_offset] / (1.0 - abs(m_cycle_des_par.m_DP_PC2[2 - cpp_offset]));			// relative pressure drop specified for precooler [P1 = P9 - P9*rel_DP => P1 = P9*(1-rel_DP)
	else
		m_pres_last[12 - cpp_offset] = m_pres_last[1 - cpp_offset] + m_cycle_des_par.m_DP_PC2[2 - cpp_offset];
	...
	calculate_turbomachinery_outlet_PCRC_without_Reheating(m_temp_last[11 - cpp_offset], m_pres_last[11 - cpp_offset], m_pres_last[12 - cpp_offset], m_cycle_des_par.m_eta_mc1,
		true, sub_error_code_1, m_enth_last[11 - cpp_offset], m_entr_last[11 - cpp_offset], m_dens_last[11 - cpp_offset],
		m_temp_last[12 - cpp_offset], m_enth_last[12 - cpp_offset], m_entr_last[12 - cpp_offset], m_dens_last[12 - cpp_offset], w_mc1);
	...
	calculate_turbomachinery_outlet_PCRC_without_Reheating(m_temp_last[1 - cpp_offset], m_pres_last[1 - cpp_offset], m_pres_last[2 - cpp_offset], m_cycle_des_par.m_eta_mc2,
		true, sub_error_code_2, m_enth_last[1 - cpp_offset], m_entr_last[1 - cpp_offset], m_dens_last[1 - cpp_offset],
		m_temp_last[2 - cpp_offset], m_enth_last[2 - cpp_offset], m_entr_last[2 - cpp_offset], m_dens_last[2 - cpp_offset], w_mc2);
	...
	calculate_turbomachinery_outlet_PCRC_without_Reheating(m_temp_last[6 - cpp_offset], m_pres_last[6 - cpp_offset], m_pres_last[7 - cpp_offset], m_cycle_des_par.m_eta_t,
		false, sub_error_code_3, m_enth_last[6 - cpp_offset], m_entr_last[6 - cpp_offset], m_dens_last[6 - cpp_offset],
	...
	if (m_cycle_des_par.m_recomp_frac >= 1.E-12)
	{
		...
		calculate_turbomachinery_outlet_PCRC_without_Reheating(m_temp_last[2 - cpp_offset], m_pres_last[12 - cpp_offset], m_pres_last[10 - cpp_offset], m_cycle_des_par.m_eta_rc,
			true, sub_error_code_4, dummy[0], dummy[1], dummy[2], dummy[3], dummy[4], dummy[5], dummy[6], w_rc);

		if (sub_error_code_4 != 0)
		{
			m_errors.SetError(24);
			m_errors.SetError(sub_error_code_4);
			return false;
		}
	}
	...
	for (T8_iter = 1; T8_iter <= max_iter; T8_iter++)
	{
		...
		for (T9_iter = 1; T9_iter <= max_iter; T9_iter++)
		{
			...
			if (m_cycle_des_par.m_recomp_frac >= 1.E-12)
			{
				...
				calculate_turbomachinery_outlet_PCRC_without_Reheating(m_temp_last[12 - cpp_offset], m_pres_last[12 - cpp_offset], m_pres_last[10 - cpp_offset], m_cycle_des_par.m_eta_rc,
					true, sub_error_code_5, m_enth_last[12 - cpp_offset], m_entr_last[12 - cpp_offset], m_dens_last[12 - cpp_offset], m_temp_last[10 - cpp_offset],
				...
				if (sub_error_code_5 != 0)
				{
					...
					m_errors.SetError(sub_error_code_5);
					return false;
				}
			}
			else
			{
				...
				property_error_code = CO2_TP(m_temp_last[12 - cpp_offset], m_pres_last[12 - cpp_offset], &co2_props);
				if (property_error_code != 0)
				{
					m_errors.SetError(28);
					return false;
				}
				m_temp_last[10 - cpp_offset] = m_temp_last[12 - cpp_offset];					// Assume state(10) is the same as state(9)
				m_enth_last[10 - cpp_offset] = m_enth_last[12 - cpp_offset] = co2_props.enth;
				m_entr_last[10 - cpp_offset] = m_entr_last[12 - cpp_offset] = co2_props.entr;
				m_dens_last[10 - cpp_offset] = m_dens_last[12 - cpp_offset] = co2_props.dens;
			}

			// Knowing the specific work of the the recompressing compressor, the required mass flow rate can be determined.
			m_dot_t = m_cycle_des_par.m_W_dot_net / (w_mc2 * (1.0 - m_cycle_des_par.m_recomp_frac) + w_rc * m_cycle_des_par.m_recomp_frac + w_t + w_mc1);			// total mass flow rate(through turbine)
			...
			calculate_hxr_UA_PCRC_without_Reheating(m_cycle_des_par.m_N_sub_hxrs, Q_dot_LT, m_dot_mc, m_dot_t, m_temp_last[2 - cpp_offset], m_temp_last[8 - cpp_offset],
				m_pres_last[2 - cpp_offset], m_pres_last[3 - cpp_offset], m_pres_last[8 - cpp_offset], m_pres_last[9 - cpp_offset],
				sub_error_code_6, UA_LT_calc, min_DT_LT);
			...
			if (sub_error_code_6 > 0)
			{
				...
				else
				{
					...
					m_errors.SetError(sub_error_code_6);
					return false;
				}
			}
		...
		calculate_hxr_UA_PCRC_without_Reheating(m_cycle_des_par.m_N_sub_hxrs, Q_dot_HT, m_dot_t, m_dot_t, m_temp_last[4 - cpp_offset], m_temp_last[7 - cpp_offset],
			m_pres_last[4 - cpp_offset], m_pres_last[5 - cpp_offset], m_pres_last[7 - cpp_offset], m_pres_last[8 - cpp_offset],
			sub_error_code_7, UA_HT_calc, min_DT_HT);
		...
		if (sub_error_code_7 > 0)
		{
			...
			else
			{
				...
				m_errors.SetError(sub_error_code_7);
				return false;
			}
		}
	...
	HX_design_parameters_PCRC_without_Reheating LT_des_par;
	...
	HX_design_parameters_PCRC_without_Reheating HT_des_par;
	...
	// PHX 
	HX_design_parameters_PCRC_without_Reheating PHX_des_par;
	...
	// Precooler1
	HX_design_parameters_PCRC_without_Reheating PC1_des_par;
	PC1_des_par.m_N_sub = m_cycle_des_par.m_N_sub_hxrs;
	// Resize?
	PC1_des_par.m_m_dot_design[0] = 0.0;
	PC1_des_par.m_m_dot_design[1] = m_dot_mc;
	// Resize?
	PC1_des_par.m_DP_design[0] = 0.0;
	PC1_des_par.m_DP_design[1] = m_pres_last[9 - cpp_offset] - m_pres_last[11 - cpp_offset];
	// Leave UA set to NaN?
	PC1_des_par.m_Q_dot_design = m_dot_mc*(m_enth_last[9 - cpp_offset] - m_enth_last[11 - cpp_offset]);
	m_PC1.initialize(PC1_des_par);

	// Precooler2
	HX_design_parameters_PCRC_without_Reheating PC2_des_par;
	PC2_des_par.m_N_sub = m_cycle_des_par.m_N_sub_hxrs;
	// Resize?
	PC2_des_par.m_m_dot_design[0] = 0.0;
	PC2_des_par.m_m_dot_design[1] = m_dot_mc;
	// Resize?
	PC2_des_par.m_DP_design[0] = 0.0;
	PC2_des_par.m_DP_design[1] = m_pres_last[12 - cpp_offset] - m_pres_last[1 - cpp_offset];
	// Leave UA set to NaN?
	PC2_des_par.m_Q_dot_design = m_dot_mc*(m_enth_last[12 - cpp_offset] - m_enth_last[1 - cpp_offset]);
	...
	// Recompression Cycle
	m_W_dot_net_last = w_mc2*m_dot_mc + w_rc*m_dot_rc + w_t*m_dot_t + w_mc1*m_dot_t;
	...
	// Main Compressor1 
	compressor_design_parameters_PCRC_without_Reheating mc1_des_par;
	mc1_des_par.m_type = m_cycle_des_par.m_mc1_type;
	mc1_des_par.m_w_design = w_mc1;
	mc1_des_par.m_eta_design = m_cycle_des_par.m_eta_mc1;
	mc1_des_par.m_m_dot_design = m_dot_t;
	mc1_des_par.m_rho_in_design = m_dens_last[11 - cpp_offset];
	mc1_des_par.m_recomp_frac_design = 1.0;
	if (!m_mc1.initialize(mc1_des_par))
	{
	...
	// Main Compressor2
	compressor_design_parameters_PCRC_without_Reheating mc2_des_par;
	mc2_des_par.m_type = m_cycle_des_par.m_mc2_type;
	mc2_des_par.m_w_design = w_mc2;
	mc2_des_par.m_eta_design = m_cycle_des_par.m_eta_mc2;
	mc2_des_par.m_m_dot_design = m_dot_mc;
	mc2_des_par.m_rho_in_design = m_dens_last[1 - cpp_offset];
	mc2_des_par.m_recomp_frac_design = 1.0 - m_cycle_des_par.m_recomp_frac;
	if (!m_mc2.initialize(mc2_des_par))
	{
		m_errors.SetError(12342);		// Need to map error code
		return false;
	}
	...
	// Recompressor
	compressor_design_parameters_PCRC_without_Reheating rc_des_par;
	...
	rc_des_par.m_rho_in_design = m_dens_last[12 - cpp_offset];
	...
	if (m_cycle_des_par.m_N_t < 0.0)
		N_t_in = m_mc2.get_N_design();
	else
		N_t_in = m_cycle_des_par.m_N_t;

	turbine_design_parameters_PCRC_without_Reheating turb_des_par;
...
bool RecompCycle_PCRC_without_Reheating::design_no_opt()
{
	if( design() )
	{
		set_des_data();
...
bool RecompCycle_PCRC_without_Reheating::optimal_design()
{
	...
	if (!m_cycle_des_par.m_fixed_P_mc1_in)
	{
		x.push_back(m_cycle_des_par.m_P_mc1_in_guess);
		lb.push_back(7377.0);
	...
	if( !m_cycle_des_par.m_fixed_P_mc2_out )
	{
		x.push_back(m_cycle_des_par.m_P_mc2_out_guess);
		lb.push_back(100.0);
		ub.push_back(m_cycle_des_par.m_P_high_limit);
		scale.push_back(500.0);
	...
	if( !m_cycle_des_par.m_fixed_PR_mc2 )
	{
		x.push_back(m_cycle_des_par.m_PR_mc2_guess);
		lb.push_back(0.0001);
	...
	if( index > 0 )
	{
		...
		opt_des_cycle.set_max_objective(nlopt_callback_opt_des_PCRC_without_Reheating, this);		// Calls wrapper/callback that calls 'design_point_eta', which optimizes design point eta through repeated calls to 'design'
		...
		if( !m_cycle_des_par.m_fixed_P_mc1_in )
		{
			m_cycle_des_par.m_P_mc1_in = x[index];
			index++;
		}
		
		if( !m_cycle_des_par.m_fixed_P_mc2_out )
		{
			m_cycle_des_par.m_P_mc2_out = x[index];
			index++;
		}

		if( !m_cycle_des_par.m_fixed_PR_mc2 )
		{
			m_cycle_des_par.m_PR_mc2 = x[index];
			index++;
		}
...
double nlopt_callback_opt_des_PCRC_without_Reheating(const std::vector<double> &x, std::vector<double> &grad, void *data)
{
	RecompCycle_PCRC_without_Reheating *frame = static_cast<RecompCycle_PCRC_without_Reheating*>(data);
	if( frame != NULL ) return frame->design_point_eta(x);
}

double RecompCycle_PCRC_without_Reheating::design_point_eta(const std::vector<double> &x)
{
	...
	{
		m_cycle_des_par.m_P_mc1_in = x[index];
		if( m_cycle_des_par.m_P_mc1_in > m_cycle_des_par.m_P_high_limit )
			return 0.0;
		index++;
	}
	...
	{
		m_cycle_des_par.m_P_mc2_out = x[index];
		if( m_cycle_des_par.m_P_mc2_out > m_cycle_des_par.m_P_high_limit )
			return 0.0;
		index++;
	}
	...
	{
		m_cycle_des_par.m_PR_mc2 = x[index];
		if( m_cycle_des_par.m_PR_mc2 > 15.0 )
			return 0.0;
		double P_mc2_in = m_cycle_des_par.m_P_mc2_out/m_cycle_des_par.m_PR_mc2;
		if( P_mc2_in >= m_cycle_des_par.m_P_mc2_out)
			return 0.0;
		if( P_mc2_in <= 7377 )
			return 0.0;
		index++;
...
bool RecompCycle_PCRC_without_Reheating::auto_optimal_design()
{
	...
	m_cycle_des_par.m_fixed_P_mc2_out = true;
	// The Pressure Ratio is always optimized in 'optimal_design'
	m_cycle_des_par.m_fixed_PR_mc2 = false;
	...
	double best_P_high = fminbr(
		m_cycle_des_par.m_P_high_limit*0.2, m_cycle_des_par.m_P_high_limit, &fmin_callback_opt_eta_PCRC_without_Reheating, this, 1.0);
	...
	{	// If the optimized high pressure is not at the limit, check results at the limit
		m_cycle_des_par.m_PR_mc2_guess = m_PR_mc2_autodes;

		// Simple Cycle
		m_cycle_des_par.m_fixed_P_mc1_in = false;
		m_cycle_des_par.m_P_mc1_in_guess = 7377;
		...
		m_cycle_des_par.m_recomp_frac_guess = 0.1;
		m_cycle_des_par.m_fixed_LT_frac = false;
		m_cycle_des_par.m_LT_frac_guess = 0.1;
	...
	if( recomp_local != m_eta_thermal_autodes )
	{
		m_cycle_des_par.m_P_mc2_out = m_P_high_autodes;
		
		m_cycle_des_par.m_fixed_P_mc1_in = true;
		m_cycle_des_par.m_P_mc1_in = m_P_mc1_in_autodes;

		m_cycle_des_par.m_fixed_PR_mc2 = true;
		m_cycle_des_par.m_PR_mc2 = m_PR_mc2_autodes;
...
double RecompCycle_PCRC_without_Reheating::opt_eta(double P_high_opt)
{
	...
	double P_pseudo = P_pseudocritical_PCRC_without_Reheating(m_cycle_des_par.m_T_mc2_in);
	...
	m_cycle_des_par.m_P_mc2_out = P_high_opt;

	// 2) Set pressure ratios
	if( P_high_opt > P_pseudo )
		m_cycle_des_par.m_PR_mc2_guess = P_high_opt / P_pseudo;
	else
		m_cycle_des_par.m_PR_mc2_guess = 1.1;
...
double fmin_callback_opt_eta_PCRC_without_Reheating(double x, void *data)
{
	RecompCycle_PCRC_without_Reheating *frame = static_cast<RecompCycle_PCRC_without_Reheating*>(data);
