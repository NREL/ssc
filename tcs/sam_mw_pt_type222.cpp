...
public:
    ...
    virtual int init() {
        ...
        m_T_htf_hot_des =
                value(P_T_htf_hot_des) + 273.15;     //[K] Design receiver outlet temperature -> convert from K
        ...
        m_m_dot_htf_max = value(P_m_dot_htf_max) /
                          3600.0;    //[kg/s] Maximum mass flow rate through receiver -> convert from kg/hr
    ...
    virtual int call(double time, double step, int ncall) {
        ...
        int night_recirc = (int) value(
                I_night_recirc);    //[-] Night recirculation control 0 = empty receiver, 1 = recirculate
        double hel_stow_deploy = value(
                I_hel_stow_deploy);    //[deg] Solar elevation angle at which heliostats are stowed
        ...
        double T_coolant_prop = (T_salt_hot_target + T_salt_cold_in) /
                                2.0;        //[K] The temperature at which the coolant properties are evaluated. Validated as constant (mjw)
        c_p_coolant =
                field_htfProps.Cp(T_coolant_prop) * 1000.0;                    //[kJ/kg-K] Specific heat of the coolant
        ...
        if (m_is_iscc) {
            ...
            double m_dot_iscc_max =
                    m_q_iscc_max / (c_p_coolant * (T_salt_hot_target - T_salt_cold_in));        // [kg/s]
        ...
        do {
            ...
            double c_guess = field_htfProps.Cp((T_salt_hot_target + T_salt_cold_in) /
            ...
            if (I_bn > 1.E-6) {
                double q_guess =
                        0.5 * q_dot_inc_sum;        //[kW] Estimate the thermal power produced by the receiver
                m_dot_salt_guess = q_guess / (c_guess * (T_salt_hot_target - T_salt_cold_in) *
            ...
            while (abs(err) > tol) {
                ...
                double beta = 1.0 /
                              T_amb;                                                //[1/K] Volumetric expansion coefficient
                double nu_amb =
                        ambient_air.visc(T_amb) / ambient_air.dens(T_amb, P_amb);    //[m^2/s] Kinematic viscosity
                ...
                for (int i = 0; i < m_n_panels; i++) {
                    ...
                    int i_fp = i;
                    // Natural convection
                    double Gr_nat = max(0.0, CSP::grav * beta * (m_T_s.at(i_fp) - T_amb) * pow(m_h_rec, 3) /
                ...
                if (abs(err) > tol) {
                    m_dot_salt_guess = q_abs_sum / (m_n_lines * c_p_coolant *
        ...
        if (!rec_is_off) {
            ...
            if (m_E_su_prev > 0.0 || m_t_su_prev > 0.0) {
                ...
                if (m_E_su + m_t_su > 0.0) {
                    ...
                    m_dot_salt_tot = min((1.0 - m_t_su_prev / (step / 3600.0)) * m_dot_salt_tot, m_dot_salt_tot -
                                                                                                 m_E_su_prev /
                                                                                                 ((step / 3600.0) *
                                                                                                  c_p_coolant *
                                                                                                  (T_salt_hot_guess -
                                                                                                   T_salt_cold_in)));
                    f_rec_timestep = max(0.0, min(1.0 - m_t_su_prev / (step / 3600.0), 1.0 - m_E_su_prev /
                                                                                             (m_dot_salt_tot *
                                                                                              c_p_coolant *
                                                                                              (T_salt_hot_guess -
                                                                                               T_salt_cold_in))));
            ...
            double DELTAP_tube = rho_coolant * (f * m_h_rec / m_id_tube * pow(u_coolant, 2) /
                                                2.0);    //[Pa] Pressure drop across the tube, straight length
            double DELTAP_45 = rho_coolant * (f * L_e_45 * pow(u_coolant, 2) /
                                              2.0);                    //[Pa] Pressure drop across 45 degree bends
            double DELTAP_90 = rho_coolant * (f * L_e_90 * pow(u_coolant, 2) /
                                              2.0);                    //[Pa] Pressure drop across 90 degree bends
            DELTAP = DELTAP_tube + 2 * DELTAP_45 + 4 *
                                                   DELTAP_90;                        //[Pa] Total pressure drop across the tube with (4) 90 degree bends, (2) 45 degree bends
            double DELTAP_h_tower = rho_coolant * m_h_tower *
            ...
            double est_load =
                    max(0.25, m_dot_salt_tot / m_m_dot_htf_des) * 100;        //[%] Relative pump load. Limit to 25%
            double eta_pump_adj = eta_pump * (-2.8825E-9 * pow(est_load, 4) + 6.0231E-7 * pow(est_load, 3) -
