...
public:
	...
	virtual int call(double time, double step, int ncall)
	{
		...
		ms_inputs.m_T_wb = value(I_T_WB);				//Ambient wet bulb temperature [C]
		ms_inputs.m_standby_control = (int)value(I_STANDBY_CONTROL);		//Control signal indicating standby mode [none]
		ms_inputs.m_T_db = value(I_T_DB);				//Ambient dry bulb temperature [C]
		ms_inputs.m_P_amb = value(I_P_AMB);				//Ambient pressure [mbar]
		ms_inputs.m_tou = (int)value(I_TOU);		//Current Time-of-use period [none]
		ms_inputs.m_rh = value(I_RH) / 100.0;			//Relative humidity of the ambient air [none]
		...
		try
		{
			mc_power_cycle.call(ms_inputs, ms_outputs, time, step, ncall);
		}
		catch(C_csp_exception &csp_exception)
		{
			// Report warning before exiting with error
