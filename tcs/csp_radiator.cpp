...
void C_csp_radiator::init() {
    ...
    std::ifstream inputFile(
            "C:/Users/adyreson/OneDrive/Documents/PhD/09_System/Desert_Rock_Weather/DesertRock2015_TS_localhr.txt");
...
void C_csp_radiator::night_cool(double T_db /*K*/, double T_rad_in /*K*/, double u /*m/s*/, double T_s /*K*/,
                                double m_dot_rad /*K*/, double Np, double m_dot_coldstorage /*kg/sec*/,
        //outputs
...
void C_csp_radiator::analytical_panel_calc(double T_db /*K*/, double Tin /*K*/, double Tp_est /*K*/, double u /*m/s*/,
    ...
    double Ra =
            9.81 * (1 / Tf) * abs((Tp_est - T_db)) * pow(L_c_free, 3) / (nu * alpha);  //[-] Rayleigh number estimate.
    double Gr = Ra / Pr;                            //[-] Grashof number

    double Re = rho * u * Lc /
                mu;                    //[-] Reynolds number for forced convection based on given characteristic length.
    double GrRe2 = Gr / (pow(Re, 2) +
                         .00001);            //Ratio of Grashof to Reynolds ^ 2 indicates importance of free vs.forced convection.
    ...
    double Nusselt_forc_t =
            h_forc_t * Lc / k_air;            //Nusselt number related to forced convection correlation h.
    double m_conv = 3.5;                                    //Constant for combining free & forced convection.
    double Nusselt_tot_t = pow((c_free * pow(Nusselt_free_t, m_conv) + c_force * pow(Nusselt_forc_t, m_conv)),
                               (1 / m_conv));  //Combined free & forced convection.
    double h_w = Nusselt_tot_t * k_air / L_c_tot;            //Total Nu number.
    double Nusselt_free_b =
            0.58 * pow(Ra, (.2));            //Correlation for free convection from heated plate(bottom).
    ...
    double cp = mc_coldhtf.cp *
                1000;                        //mc_coldhtf.Cp(273)*1000.;               //[J / kg - K] Specific heat capacity of water
    double rho_water = mc_coldhtf.dens;                    //mc_coldhtf.dens(273,101300.);	//[kg/m^3]
    double mu_water = water_visc(rho_water, Tin) *
                      1e-6;    //Function result  is uPa-s, convert to kg/m-s. nu_water * rho_water;				//mc_coldhtf.visc(273);				//[kg / m - s]
    double nu_water = mu_water /
                      rho_water;            //1.66034557e-6 - 3.53322361e-8*(Tin - 273.15) + 2.50116564e-10*pow((Tin - 273.15), 2); //mc_coldhtf.kin_visc(273,101300.);	//Kinematic viscosity [m^2/s]
    ...
    if (Re_tube < 2300) {
        hfi = 3.66 * k_water /
              D;                                //[W / m ^ 2 - K] For laminar flow assuming uniform wall temperature(conservative)
    } else {
        hfi = 0.023 * pow(Re_tube, (0.8)) * pow(Pr_water, 0.3) * k_water /
              D;                                //[W / m ^ 2 - K] Dittus - Boelter equation for turbulent, internal forced flow.Assumes smooth tubes.
    }


    //Collector model based in adiabatic temperature and using methods of Duffie & Beckman(2013).
    double T_bar = 0.5 * (Tp_est +
                          T_db);                                                                        //An estimate of average of plate and ambient temperatures.
    double Tad = T_db - (Sigma * epsilon * (pow(T_db, 4) - pow(T_s, 4)) +
                         Sigma * (1 / (1 / epsilonb + 1 / epsilong - 1)) * (pow(T_db, 4) - pow(T_g, 4)) +
                         h_g * (T_db - T_g)) /
                        (4 * Sigma * (epsilon + 1 / (1 / epsilonb + 1 / epsilong - 1)) * pow(T_bar, 3) + h_g +
                         h_w);    //Adiab. T
    double ULad = 4 * Sigma * (epsilon + 1 / (1 / epsilonb + 1 / epsilong - 1)) * pow(T_bar, 3) + h_g +
                  h_w;            //Adiabatic loss coefficient.
    double m = sqrt(ULad / (k *
                            th));                                                                                //Fin parameter
    double F = tanh(m * (W - D) / 2) /
               (m * (W - D) / 2);                                                            //Fin efficiency
    double Fprime = 1 / (W * ULad / (3.1415 * D * hfi) + W / (D + (W - D) *
                                                                  F));                                        //Collector efficiency based on roll bond type geometry.
    double FR = (m_dot * cp) / (A_c * ULad) * (1 - exp(-(A_c * ULad * Fprime) / (m_dot *
                                                                                 cp)));                                //Flow direction collector heat removal factor.
    double Qu =
            FR * A_c * ULad * (Tin - Tad);                                                                        //Heat
    T_rad_out = Tin - Qu / (m_dot *
                            cp);                                                                            //Outlet temperature
    Tp = Qu / (ULad * A_c) +
         Tad;                                                                            //Plate temperature
    double Tpa = 0.5 * (Tp +
                        T_db);                                                                                //Updated value for average of plate & ambient temperature.

    //Pumping
    W_radpump = (ms_params.radfield_dp * ms_params.m_dot_panel * ms_params.Np) / (rho_water * 0.75 * 0.85) /
                1000;    //MWe pumping power when radiator field is operating. Isentropic eff = 0.75 and Mechanical pump eff = 0.85.
}// adiabatic calc

void
C_csp_radiator::analytical_panel_calc_HX(double T_db /*K*/, double Tin /*K*/, double Tp_est /*K*/, double u /*m/s*/,
                                         double T_s /*K*/, double m_dot /*K*/, double Np, double m_dot_water /*kg/sec*/,
        //outputs
                                         double &T_rad_out /*K*/, double &Tp /*K*/, double &W_radpump /*MW*/) {
    /*	% Author: Ana Dyreson University of Wisconsin - Madison
    % Summary : This function determines the outlet temperature of a fluid
    ...
    if (Tin <= 274) {
        ...
        water_TP(Tin, 101.3,
                 &mc_coldhtf);                    //Get water state at inlet temperature of fluid - this only approximates water temp
    ...
    int idx_props = static_cast<int>(Tin - 273.15) - T_PG20[0] +
                    1; //Truncate temperature to degree [C] and get index in EG property data as provided based on starting point of that property data.
    ...
    if ((Np * m_dot * cp) < (m_dot_water *
                             cp_water))    //Use the full flow rates at HX to compare																			//Determine minimum capacitance rate of HX
    {
        CMIN = Np * m_dot * cp;
    } else {
    ...
    double Qu = FRprime * A_c * ULad *
                (Tin - Tad);                                                                    //Heat
    //double T_panel_out = Tin - Qu /(epsilon_HX*CMIN);															//Outlet temperature from radiator panel (glycol)
    T_rad_out = Tin - Qu * Np / (m_dot_water *
                                 cp_water);                                                            //Outlet temperature of water side of HX. Because this flow rate is full for all panels through HX, need to multiply Qu by Np.
    Tp = Qu / (ULad * A_c) +
         Tad;                                                                                    //Plate temperature
    double Tpa = 0.5 * (Tp +
                        T_db);                                                                                //Updated value for average of plate & ambient temperature.
    //Pumping
    W_radpump = (ms_params.radfield_dp * ms_params.m_dot_panel * ms_params.Np) / (rho_fluid * 0.75 * 0.85) /
                1000;    //MWe pumping power when radiator field is operating. Isentropic eff = 0.75 and Mechanical pump eff = 0.85.
