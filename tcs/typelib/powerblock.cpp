...
//************************************************************************************************************
//************************************************************************************************************
void C_Indirect_PB::evap_tower(double P_cond_min, int n_pl_inc, double DeltaT_cw_des, double T_approach, double P_cycle, 
							 ...
							 double &W_dot_tot, double &P_cond, double &T_cond, double &f_hrsys)
{
	...
	*/
	...
	c_cw = wp.Cp * 1000.0;		// Convert to J/kg-K
	...
	h_pcw_in = wp.H*1000.0;
	s_pcw_in = wp.S*1000.0;
	rho_cw = wp.dens;	
	...
	double dh_high = wp.H;
	deltah_evap = (dh_high - dh_low)*1000.0;	// [J/kg]
...
//************************************************************************************************************
//************************************************************************************************************
void C_Indirect_PB::ACC(double P_cond_min, int n_pl_inc, double T_ITD_des, double P_cond_ratio, double P_cycle, double eta_ref, 
...
//************************************************************************************************************
//************************************************************************************************************
void C_Indirect_PB::HybridHR(/*double fcall,*/ double P_cond_min, int n_pl_inc, /*double time,*/ double F_wc, double F_wcmax, double F_wcmin, 
				  ...
				  double& W_dot_wctot, double& W_dot_tot, double& P_cond, double& T_cond, double& f_hrsys)
{
	...
	*/
	...
	// local vars
	double h_acfan_in, T_acfan_in_K, T_acfan_out_K, T_acfan_out, h_acfan_out_s,  
	q_ac_rej, q_wc_rej, h_pcw_in, s_pcw_in, rho_cw, h_pcw_out_s, h_pcw_out, m_dot_wcair, 
...
//100	f_hrsys = (f_hrsyswc + f_hrsysair)/2;
	...
	if(q_wc_rej > 0.001)
	{
		...
		water_PQ( P_amb/1000.0, 1.0, &wp );
		double dh_high = wp.H;
		deltaH_evap = (dh_high - dh_low)*1000.0;
