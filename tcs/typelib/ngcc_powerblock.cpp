...
void ngcc_power_cycle::set_cycle1_table_props()
{
	...
	m_P_amb_end = m_P_amb_start*(m_N_P - 1.0);
	m_T_amb_end = m_T_amb_start*(m_N_T - 1.0);
	m_q_sf_end = m_q_sf_start*(m_N_q - 1.0);
...
double ngcc_power_cycle::get_performance_results( util::block_t<double> * p_current_table )
{
	...
	double cube1 = p_current_table->at( x_q_low, y_T_low, z_P_low );
	double cube2 = p_current_table->at( x_q_low, y_T_low, z_P_high );
	double cube3 = p_current_table->at( x_q_low, y_T_high, z_P_low );
	double cube4 = p_current_table->at( x_q_low, y_T_high, z_P_high );
	double cube5 = p_current_table->at( x_q_high, y_T_low, z_P_low );
	double cube6 = p_current_table->at( x_q_high, y_T_low, z_P_high );
	double cube7 = p_current_table->at( x_q_high, y_T_high, z_P_low );
	double cube8 = p_current_table->at( x_q_high, y_T_high, z_P_high );
	
	return (1.0/8.0)*(cube1 + cube2 + cube3 + cube4 + cube5 + cube6 + cube7 + cube8);
}
