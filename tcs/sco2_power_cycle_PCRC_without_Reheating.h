#ifndef __SCO2_PC_PCRC_without_Reheating
#define __SCO2_PC_PCRC_without_Reheating
...
class TrackErrors_PCRC_without_Reheating    
{
private:
	std::vector<int> stored_codes;

public:
	~TrackErrors_PCRC_without_Reheating(){};
...
struct cycle_design_parameters_PCRC_without_Reheating
{
	int m_mc1_type; 
	int m_mc2_type;
	int m_rc_type;
	...
	double m_eta_mc1;					//[-] Isentropic efficiency of main compressor1
	double m_eta_mc2;					//[-] Isentropic efficiency of main compressor2
	...
	bool m_fixed_P_mc2_out;			// Is P_mc2_out fixed or available for optimization?
	double m_P_mc2_out;				//[kPa] Compressor2 outlet pressure - only used if fixed_P_mc2_out = true
	double m_P_high_limit;			//[kPa] Maximum allowable high-side pressure - only used if fixed_P_mc2_out = false
	double m_P_mc2_out_guess;			//[kPa] Initial value for mc2 outlet pressure - only used if fixed_P_mc2_out = false
	
	bool m_fixed_P_mc1_in;			// Is P_mc1_out fixed or available for optimization?
	double m_P_mc1_in;				//[kPa] Compressor1 outlet pressure - only used if fixed_P_mc1_in = true
	double m_P_mc1_in_guess;			//[kPa] Initial value for mc1 outlet pressure - only used if fixed_P_mc1_in = false

	bool m_fixed_PR_mc2;				// Is the Pressure Ratio over the main compressor fixed or available for optimization
	double m_PR_mc2;					//[-] Pressure Ratio - only used if fixed_PR_mc = true
	double m_PR_mc2_guess;				//[-] Initial value for Pressure Ratio - only used if fixed_PR_mc = false
	...
	cycle_design_parameters_PCRC_without_Reheating()
	{
		m_mc1_type = m_mc2_type = m_rc_type = m_N_sub_hxrs - 1;
		
		m_fixed_LT_frac = m_fixed_P_mc2_out = m_fixed_P_mc1_in = m_fixed_PR_mc2 = m_fixed_recomp_frac = false;
		
		m_W_dot_net = m_T_mc1_in = m_T_mc2_in = m_T_t_in = m_N_t = m_eta_mc1 =  m_eta_mc2 = m_eta_rc = m_eta_t = m_tol = m_opt_tol = m_UA_rec_total = m_LT_frac = m_LT_frac_guess =
		m_P_mc2_out = m_P_mc1_in = m_P_high_limit = m_P_mc2_out_guess = m_PR_mc2 = m_PR_mc2_guess = m_recomp_frac = m_recomp_frac_guess = std::numeric_limits<double>::quiet_NaN();
...
struct S_cycle_design_metrics_PCRC_without_Reheating
{
	...
	double m_N_mc1;				//[rpm] Main compressor1 speed
	double m_N_mc2;				//[rpm] Main compressor1 speed
	double m_m_dot_PHX;			//[kg/s] Mass flow rate through primary heat exchanger
	double m_m_dot_PC1;			//[kg/s] Mass flow rate through pre-cooler1
	double m_m_dot_PC2;			//[kg/s] Mass flow rate through pre-cooler2

	std::vector<double> m_T;	//[K] Temperature at cycle state points
	std::vector<double> m_P;	//[kPa] Pressure at cycle state points

	S_cycle_design_metrics_PCRC_without_Reheating()
	{
		m_eta_thermal = m_W_dot_net = m_min_DT_LT = m_min_DT_HT = m_m_dot_PHX = m_m_dot_PC1 =
		m_m_dot_PC2 = std::numeric_limits<double>::quiet_NaN();
		// Size vectors when they are assigned to values from the RecompCycle class?
	}
};

struct cycle_opt_off_des_inputs_PCRC_without_Reheating
{
	double m_T_mc1_in;		//[K] Main compressor1 inlet temperature
	double m_T_mc2_in;		//[K] Main compressor2 inlet temperature
	...
	bool m_fixed_N_mc1;			//[-] Is the main compressor1 speed fixed fixed or available for optimization
	double m_N_mc1;				//[rpm] Main compressor1 speed: only used if it is fixed [use negative value to use design point speed]
	double m_N_mc1_guess;		//[rpm] Guess for main compressor1 speed: only used if it is available for optimization

	bool m_fixed_N_mc2;			//[-] Is the main compressor2 speed fixed fixed or available for optimization
	double m_N_mc2;				//[rpm] Main compressor2 speed: only used if it is fixed [use negative value to use design point speed]
	double m_N_mc2_guess;		//[rpm] Guess for main compressor2 speed: only used if it is available for optimization
	...
	cycle_opt_off_des_inputs_PCRC_without_Reheating()
	{
		m_T_mc1_in = m_T_mc2_in = m_T_t_in = m_W_dot_net_target = m_recomp_frac = m_recomp_frac_guess = m_N_mc1 = m_N_mc1_guess =
		m_N_mc2 = m_N_mc2_guess = m_N_t = m_N_t_guess = m_tol = m_opt_tol = std::numeric_limits<double>::quiet_NaN();
		m_fixed_recomp_frac = m_fixed_N_mc1 = m_fixed_N_mc2 = m_fixed_N_t = false;
	}
};

struct cycle_off_des_inputs_PCRC_without_Reheating
{
	cycle_opt_off_des_inputs_PCRC_without_Reheating m_S;	// Structure for inputs to Cycle Optimum Off-Design

	double m_P_mc1_in;
	double m_P_mc2_in;

	cycle_off_des_inputs_PCRC_without_Reheating()
	{
		m_P_mc1_in = m_P_mc2_in = std::numeric_limits<double>::quiet_NaN();
	}	
};

struct S_off_design_performance_PCRC_without_Reheating
{
	double m_eta_thermal;		//[-] Cycle thermal efficiency
	...
	S_off_design_performance_PCRC_without_Reheating()
	{
		m_eta_thermal = m_W_dot_net = m_q_dot_in = m_min_DT_LT = m_min_DT_HT = m_N_rc = 
		m_m_dot_PHX = m_m_dot_PC1 = m_m_dot_PC2 = std::numeric_limits<double>::quiet_NaN();
		// Size vectors when they are assigned to values from the RecompCycle class?
	}
};

struct compressor_design_parameters_PCRC_without_Reheating
{
	int m_type;
	...
	compressor_design_parameters_PCRC_without_Reheating()
	{
		m_type = -1;
...
class compressor_PCRC_without_Reheating
{
private:

	// Parameter Structure
	compressor_design_parameters_PCRC_without_Reheating m_comp_des_par;
...
public:
	~compressor_PCRC_without_Reheating(){};
	...
	bool initialize(const compressor_design_parameters_PCRC_without_Reheating & comp_des_par_in )
	{
	...
	double calculate_m_dot_guess(double rho_in, double N_mc_local)
	{
		return m_phi_design*rho_in*pow(m_D_rotor, 2)*m_D_rotor*0.5*N_mc_local*0.10471975512;		// Highest possible mass flow rate in main compressor (pressure rise would be zero)		
	}
...
struct turbine_design_parameters_PCRC_without_Reheating
{
	...
	turbine_design_parameters_PCRC_without_Reheating()
	{
		m_w_design = m_eta_design = m_m_dot_design = m_N_design = m_rho_out_design = std::numeric_limits<double>::quiet_NaN();
	}
};

class turbine_PCRC_without_Reheating
{
private:
	// Parameter Structure
	turbine_design_parameters_PCRC_without_Reheating m_turb_des_par;
...
public:
	~turbine_PCRC_without_Reheating(){};
	...
	bool initialize(const turbine_design_parameters_PCRC_without_Reheating & turb_des_par_in )
	{
...
struct HX_design_parameters_PCRC_without_Reheating
{
	...
	HX_design_parameters_PCRC_without_Reheating()
	{
		m_N_sub = -1;
...
struct HX_off_design_outputs_PCRC_without_Reheating
{
	double m_UA_calc;
	...
	HX_off_design_outputs_PCRC_without_Reheating()
	{
...
class HeatExchanger_PCRC_without_Reheating
{
private:
	// Parameter Structure
	HX_design_parameters_PCRC_without_Reheating m_HX_des_par;
	HX_off_design_outputs_PCRC_without_Reheating m_HX_od_out;
...
public:
	~HeatExchanger_PCRC_without_Reheating(){};

	HeatExchanger_PCRC_without_Reheating(){};

	void initialize(const HX_design_parameters_PCRC_without_Reheating & HX_des_par_in)
	{
		m_HX_des_par = HX_des_par_in;
		return;
	}

	const HX_off_design_outputs_PCRC_without_Reheating * get_od_outputs()
	{
		return &m_HX_od_out;
	}

	void set_od_outputs(const HX_off_design_outputs_PCRC_without_Reheating & HX_outputs_in)
	{
		m_HX_od_out = HX_outputs_in;
		return;
	}

	const HX_design_parameters_PCRC_without_Reheating * get_design_parameters()
	{
		return &m_HX_des_par;
	}
...
class RecompCycle_PCRC_without_Reheating
{
public:
	// member classes
	compressor_PCRC_without_Reheating m_mc1, m_mc2, m_rc;                                                // compressor and recompressor
	turbine_PCRC_without_Reheating m_t;													      // turbine
	HeatExchanger_PCRC_without_Reheating m_LT, m_HT, m_PHX, m_PC1, m_PC2;								      // heat exchangers
	TrackErrors_PCRC_without_Reheating m_errors;

	// member structure
	cycle_design_parameters_PCRC_without_Reheating m_cycle_des_par;
	cycle_opt_off_des_inputs_PCRC_without_Reheating m_cycle_opt_off_des_in;
	cycle_off_des_inputs_PCRC_without_Reheating m_cycle_off_des_in;
	S_cycle_design_metrics_PCRC_without_Reheating m_cycle_des_metrics;
	S_off_design_performance_PCRC_without_Reheating m_cycle_od_performance;
	...
	bool off_design_target_power_PCRC_without_Reheating();
	...
	void set_des_data()
	{
		...
		m_cycle_des_metrics.m_N_mc1 = m_mc1.get_N_design();
		m_cycle_des_metrics.m_N_mc2 = m_mc2.get_N_design();
		...
		m_cycle_des_metrics.m_m_dot_PC1 = m_PC1.get_design_parameters()->m_m_dot_design[1];
		m_cycle_des_metrics.m_m_dot_PC2 = m_PC2.get_design_parameters()->m_m_dot_design[1];
	}
	...
	void set_od_data()
	{
		...
		m_cycle_od_performance.m_m_dot_PC1 = m_PC1.get_od_outputs()->m_m_dot_calc[1];
		m_cycle_od_performance.m_m_dot_PC2 = m_PC2.get_od_outputs()->m_m_dot_calc[1];
	...
	void clear_member_data()
	{
		...
		m_temp_des.resize(12);
		m_pres_des.resize(12);
		m_enth_des.resize(12);
		m_entr_des.resize(12);
		m_dens_des.resize(12);
		m_temp_od_last.resize(12);
		m_pres_od_last.resize(12);
		m_enth_od_last.resize(12);
		m_entr_od_last.resize(12);
		m_dens_od_last.resize(12);
		m_temp_od.resize(12);
		m_pres_od.resize(12);
		m_enth_od.resize(12);
		m_entr_od.resize(12);
...
public:
	~RecompCycle_PCRC_without_Reheating(){};

	RecompCycle_PCRC_without_Reheating(const cycle_design_parameters_PCRC_without_Reheating & cycle_des_par_in)
	{
		m_cycle_des_par = cycle_des_par_in;
		clear_member_data();
	}

	void set_design_parameters(const cycle_design_parameters_PCRC_without_Reheating & cycle_des_par_in)
	{
		m_cycle_des_par = cycle_des_par_in;
		clear_member_data();
	}

	const cycle_design_parameters_PCRC_without_Reheating * get_cycle_design_parameters()
	{
		return &m_cycle_des_par;
	}

	const S_cycle_design_metrics_PCRC_without_Reheating * get_cycle_design_metrics()
	{
		return &m_cycle_des_metrics;
	}

	const S_off_design_performance_PCRC_without_Reheating * get_off_design_outputs()
	{
		return &m_cycle_od_performance;
	}

	const cycle_off_des_inputs_PCRC_without_Reheating * get_off_design_inputs()
	{
	...
	bool optimal_off_design(const cycle_opt_off_des_inputs_PCRC_without_Reheating & cycle_opt_off_des_in_in);

	double off_design_target_power_function(const std::vector<double> &x);

	bool off_design(const cycle_off_des_inputs_PCRC_without_Reheating & cycle_off_des_in_in);
...
double nlopt_callback_opt_off_des_PCRC_without_Reheating(const std::vector<double> &x, std::vector<double> &grad, void *data);
