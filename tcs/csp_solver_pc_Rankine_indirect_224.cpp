...
void C_pc_Rankine_indirect_224::init()
{
	...
	else if( ms_params.m_pc_fl == HTFProperties::User_defined )
	{
		...
		if( n_rows > 2 && n_cols == 7 )
		{
			...
			{
				error_msg = util::format(mc_pc_htfProps.UserFluidErrMessage(), n_rows, n_cols);
				throw(C_csp_exception(error_msg, "Rankine Indirect Power Cycle Initialization"));
			}
		}
		else
		{
	...
	{
		error_msg = util::format("The power cycle technology type identifier, %d, must be [1..4]", ms_params.m_tech_type);
		throw(C_csp_exception(error_msg, "Power cycle initialization"));
	}
	...
	for( int i = 0; i<9; i++ )
	{
		...
		if(F_wc_current < 0.0)
		{
			error_msg = util::format("The hybrid dispatch value at TOU period %d was entered as %lg. It was reset to 0", i+1, F_wc_current);
			mc_csp_messages.add_message(C_csp_messages::WARNING, error_msg);
			F_wc_current = 0.0;
		}
		if(F_wc_current > 1.0)
		{
			error_msg = util::format("The hybrid dispatch value at TOU period %d was entered as %lg. It was reset to 1.0", i+1, F_wc_current);
			mc_csp_messages.add_message(C_csp_messages::WARNING, error_msg);
			F_wc_current = 1.0;
		}
	...
	{
		error_msg = util::format("The boiler pressure provided by the user, %lg, requires a supercritical system. The pressure has been reset to 220 bar", ms_params.m_P_boil);
		mc_csp_messages.add_message(C_csp_messages::WARNING, error_msg);
	...
	const C_csp_solver_sim_info &sim_info)
{
	...
	if( P_cycle > 0.0 )
	{
		if( (m_startup_time_remain_prev + m_startup_energy_remain_prev) > 0.0 )
		{
