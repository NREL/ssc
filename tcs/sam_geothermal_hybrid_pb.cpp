...
public:
    ...
    void Set_PB_coefficients() {
        ...
        */
        ...
        const double PB_Power_Nominal_Norm[
                3 * 15 * 6] =   // Power Block Power Nominal: Adding ACC Power Back (-):Normalized at   110.418 MWe
                {   // blocks for constant turbine inlet temperature
                        // rows for constant condensing pressure
                        // columns for constant turbine inlet mass
                        //
    ...
    bool Set_PB_ref_values() {
        ...
        double qtot =
                m_P_ref / m_eta_ref;        //total thermal contribution includes both solar and geothermal sources
        double qndtot = 1. +
                        m_qgeo_frac_ref;    //the total non-dimensional thermal fraction includes the solar fraction (1) plus the fraction of geothermal relative to solar (qgeo/qsolar)
        double qsol_nd_ref = 1. / qndtot;            //The actual fraction of thermal energy supplied by solar at design
        double qgeo_nd_ref =
                m_qgeo_frac_ref / qndtot;    //The actual fraction thermal energy supplied by geothermal at design
        m_q_solar_ref =
                qsol_nd_ref * qtot;        //The dimensional amount of thermal energy supplied by solar at design
        m_q_geo_ref =
                qgeo_nd_ref * qtot;        //The dimensional amount of thermal energy supplied by geothermal at design


        double wnet_nd_adj, qsol_nd_adj, qgeo_nd_adj;
        CycleMap(m_Psat_ref, m_T_hot_ref - m_dT_hx_ref, 1., wnet_nd_adj, qsol_nd_adj,
                 qgeo_nd_adj);    //Look up the performance at the specified design-point condenser condition
        ...
        m_eta_adj = m_eta_ref * qfrac /
                    wnet_nd_adj;  //The reference efficiency, adjusted for the user-specified condenser conditions
    ...
    bool CycleMap(double pcond, double Tin, double mdot_ND, double &wnet_ND, double &qsolar_ND, double &qgeo_ND) {
        ...
        */
        ...
        try {
            ...
            //3D interpolation
            vector<util::block_t<double> *> datas = {&m_PB_Power_Nominal_Norm, &m_Solar_Thermal_Norm,
                                                     &m_GeoThermal_Norm};
    ...
    bool GeoCSP_RankineCycle(double T_db, double T_wb, double P_amb, double T_hot, double m_dot_htf, int mode,
                             double demand_var, double F_wc_tou,
                             double &P_cycle, double &Q_solar, double &Q_geo, double &eta, double &T_cold,
        ...
        switch (m_CT) {
            case 1:
                CSP::evap_tower(1, m_P_cond_min, m_n_pl_inc, m_dT_cw_ref, m_T_approach, m_P_ref * 1000.0, m_eta_adj,
                                T_db, T_wb, P_amb, q_reject_est, m_dot_makeup, W_cool_par, P_cond, T_cond, f_hrsys);
                break;
            case 2:
                CSP::ACC(1, m_P_cond_min, m_n_pl_inc, m_T_ITD_des, m_P_cond_ratio, m_P_ref * 1000.0, m_eta_adj, T_db,
                         P_amb, q_reject_est, m_dot_air, W_cool_par, P_cond, T_cond, f_hrsys);
                m_dot_makeup = 0.0;
                break;
            case 3:
                CSP::HybridHR(1, m_P_cond_min, m_n_pl_inc, F_wc_tou, m_F_wcmax, m_F_wcmin, m_T_ITD_des, m_T_approach,
                              m_dT_cw_ref, m_P_cond_ratio, m_P_ref * 1000.0, m_eta_adj, T_db, T_wb, P_amb, q_reject_est,
                              m_dot_makeup,
        ...
        while (err > 1.E-6 && qq < 100) {
            ...
            if (qq < 10) {
                switch (m_CT) {
                    case 1:
                        CSP::evap_tower(1, m_P_cond_min, m_n_pl_inc, m_dT_cw_ref, m_T_approach, m_P_ref * 1000.0,
                                        m_eta_adj, T_db, T_wb, P_amb, q_reject, m_dot_makeup, W_cool_par, P_cond_guess,
                                        T_cond, f_hrsys);
                        break;
                    case 2:
                        CSP::ACC(1, m_P_cond_min, m_n_pl_inc, m_T_ITD_des, m_P_cond_ratio, m_P_ref * 1000.0, m_eta_adj,
                                 T_db, P_amb, q_reject, m_dot_air, W_cool_par, P_cond_guess, T_cond, f_hrsys);
                        break;
                    case 3:
                        CSP::HybridHR(1, m_P_cond_min, m_n_pl_inc, F_wc_tou, m_F_wcmax, m_F_wcmin, m_T_ITD_des,
                                      m_T_approach, m_dT_cw_ref, m_P_cond_ratio, m_P_ref * 1000.0, m_eta_adj, T_db,
                                      T_wb,
...
TCS_IMPLEMENT_TYPE(sam_geothermal_hybrid_pb, "Geomthermal Hybrid Power Cycle", "Mike Wagner", 1,
                   sam_geothermal_hybrid_pb_vars, NULL, 1);
