SUBROUTINE Superheater(info,XIN,XOUT,N_panels,PAR,Q_inc,h_out_target,sh_exit,checkflux)
...
!    4| H_rec                            | Height of superheater                                             | m                | m                
!    5| D_tube                           | O.D. of SH tubes                                                  | m                | m                
!    6| th_tube                          | Thickness of SH tubes                                             | m                | m                
!    7| eps                              | Emissivity of SH tubes                                            | none             | none             
!    8| absorp                           | Absorptivity of SH tubes                                          | none             | none             
...
!    5| T_in                             | Superheater inlet temperature                                     | K                | K                
!    6| m_dot_in                         | Inlet steam mass flow rate to superheater                         | kg/s             | kg/s             
!    7| P_sh_out_min                     | Minimum allowable outlet pressure of superheater                  | Pa               | Pa               
...
!    2| T_out                            | Exit temperature of superheater                                   | K                | K                
!    3| P_out                            | Exit pressure of superheater                                      | Pa               | Pa               
!    4| Energy_out                       | Energy rate transferred to steam                                  | W                | W                
!    5| h_out_comb                       | Outlet enthalpy                                                   | J/kg             | J/kg             
!    6| h_in                             | Inlet enthalpy                                                    | J/kg             | J/kg             
!    7| T1_max                           | Max SH surface temperature                                        | K                | K                
!    8| P_in - P_Path_out(1)             | Pressure drop through superheater                                 | Pa               | Pa               
!    9| u_n_exit                         | Exit velocity of steam                                            | m/s              | m/s              
!   10| Energy_in                        | Flux * receiverArea                                               | W                | W                
!   11| P_path_out(1)                    | SH outlet pressure                                                | Pa               | Pa               
!   12| rho_out                          | Sh outlet density                                                 | kg/m^3           | kg/m^3           
!   13| Q_conv_SH                        | Total convective loss from superheater                            | MW               | MW               
!   14| Q_rad_SH                         | Total radiative loss from superheater                             | MW               | MW               
!   15| Q_abs_SH                         | Total energy rate absorbed by superheater                         | MW               | MW               
...
real(8):: T_amb,H_rec,L_rec,W_rec,D_tube,th_tube,eps,absorp,Mat_tube,d_in,hl_ffact,m,L_eff_90,L_eff_45,sigma,grav,pi,&
            ...
            DELTAP_TUBE,DELTAP_45,DELTAP_90,P_out,diff_P_ave,m_dot_total,&
            diff_P_path,x_n_ave,&
            mh_sum,h_out_comb,energy_in,energy_out,eta_rec,T1_max,P_n_in,dp,T_out_target,&
            diff_P_bracket,P_lower,P_upper,P_sh_out_min,diff_T_HT,h_sh_max,&
            T_1_upper,y_t_upper,T_1_lower,y_t_lower,Q_inc(N_panels),q_wf_min, T_1_max, diff_T1_g, T_n_in,&
            cp_x1,rho_x1,mu_x1,k_x1,&
            u_n_base,u_n_exit,Q_sum,&
            T_1_min, rho_out, Q_rad_SH, Q_conv_SH, Q_abs_SH, P_out_avg
...
integer*4:: N_fr,Nodes,N_par,T_1_iter,P_ave_iter,diff_path_iter,T1_br_lower,T1_br_upper,P_br_low,Pave_flag,&
            N_comb,k,CombNodes
integer*4:: i,j,dummy,up1,flowtype
...
IF(info(7)==-1)THEN
    ...
    h_sh_max    = 4658519.d0     ![J/kg] Corresponds to maximum possible temperature of lookup tables so steam code doesn't bug out
    ...
    Mat_tube    = PAR(up1(1))    ![-] Superheater material
...
!    L_rec       = 16.5d0            ![m] Length of square superheater
!    W_rec       = 16.5d0            ![m] Width of square superheater

!    D_tube      = 0.01905           ![m] Outer diameter of superheater tubes
...
!    Mat_tube    = 28.d0             ![-] Superheater material
    ...
    !Set as constant inside subroutine
...
!    IF (mod(N_panels,N_fr) > 0)  STOP
    ...
    IF(.not.allocated(Flow_Pat))THEN
        allocate(Flow_Pat(N_fr,Nodes),h_path_out(N_fr),P_path_out(N_fr),m_dot_path(N_fr),q_wf1(CombNodes),q_wf_total(N_fr),Flow_Pat_adj(N_fr,(Nodes/N_comb)),Q_adj(N_fr*CombNodes),Q_rad(N_fr*CombNodes),Q_conv(N_fr*CombNodes),Q_abs(N_fr*CombNodes))
    ENDIF
...
IF(info(8)==-1)THEN
    DEALLOCATE(Flow_Pat,h_path_out,P_path_out,m_dot_path,q_wf1,q_wf_total,Flow_Pat_adj,Q_adj,Q_rad,Q_conv,Q_abs)
...
P_sh_out_min = XIN(7)       ![Pa] Minimum allowable outlet pressure of superheater
...
!*********  REFPROP **********************************************************************
!P(kPa),T(C),enth(kJ/kg),dens(kg/m3),inte(kJ/kg),entr(kJ/kg-K),cp(kJ/kg-K),cond(W/m-K),visc(kg/m-s)
CALL Water_TQ(T_in-273.15,1.d0,Pres=P_in,enth=h_in,dens=rho_x1)
P_in = P_in * 1000.         !Convert kPa to Pa
h_in = h_in * 1000.         !Convert kJ/kg to J/kg      
!*****************************************************************************************
...
!Also check that maximum total available energy is at least 80% (conservative) of total needed to reach superheat target
...
348             P_ave_iter  = P_ave_iter + 1        !Increase iteration counter
            ...
            !IF(P_ave_iter == 125)  Pause "Exceeds SH average pressure iteration"
...
!272         DO WHILE((abs(diff_T_1)>1.d0).AND.(T_1_iter < 50)) 
                ...
                !IF(T_1_iter == 50)  Pause "Exceeds Superheater Energy Balance Iterations"
...
!                ENDIF                                                   
                ...
                !*********  REFPROP **********************************************************************
                !P(kPa),T(C),enth(kJ/kg),dens(kg/m3),inte(kJ/kg),entr(kJ/kg-K),cp(kJ/kg-K),cond(W/m-K),visc(kg/m-s)
706             CALL Water_PH(P_ave/1000.,h_n_ave/1000.,dens=rho_n_ave,Temp=T_n_ave,visc=mu_n_ave,cond=k_n_ave,cp=c_n_ave,qual=x_n_ave)
...
h_out_comb  = min(h_out_comb, h_sh_max)
...
Q_rad_SH    = sum(Q_rad)*dble(N_par)*dble(N_comb)/1.E6              ![MW] Total radiative loss from superheater
Q_conv_SH   = sum(Q_conv)*dble(N_par)*dble(N_comb)/1.E6             ![MW] Total convective loss from superheater
Q_abs_SH    = sum(Q_abs)*dble(N_par)*dble(N_comb)/1.E6              ![MW] Total energy rate absorbed by superheater
