...
struct var_info
{
	int var_type; //  TCS_INVALID, TCS_INPUT, TCS_OUTPUT, TCS_INOUT
	unsigned char data_type; // TCS_INVALID, TCS_STRING, TCS_NUMBER, TCS_ARRAY, TCS_MATRIX
...
public:
	...
	void log( const std::string &msg, int type=TCS_NOTICE, float time=-1.0 );
...
protected:
	...
	tcs_number_t *allocate( const std::string &name, size_t length ) throw( general_error );
	tcs_number_t *allocate( const std::string &name, size_t nrows, size_t ncols ) throw( general_error );
	util::matrix_t<tcs_number_t>& allocate_matrix( const std::string &name, size_t nrows, size_t ncols ) throw( general_error );
	...
	tcs_number_t as_number( const std::string &name ) throw( general_error );
	double as_double( const std::string &name ) throw( general_error );
	const char *as_string( const std::string &name ) throw( general_error );
	tcs_number_t *as_array( const std::string &name, size_t *count ) throw( general_error );
	std::vector<double> as_doublevec( const std::string &name ) throw( general_error );
	tcs_number_t *as_matrix( const std::string &name, size_t *rows, size_t *cols ) throw( general_error );


	size_t check_timestep_seconds( double t_start, double t_end, double t_step ) throw( timestep_error );
	
	tcs_number_t accumulate_annual(const std::string &hourly_var, const std::string &annual_var, double scale = 1.0 ) throw( exec_error );
	tcs_number_t *accumulate_monthly(const std::string &hourly_var, const std::string &annual_var, double scale = 1.0 ) throw( exec_error );
...
private:
	...
	tcs_number_t get_operand_value( const std::string &input, const std::string &cur_var_name ) throw( general_error );
...
//	virtual bool on_exec( const std::string &command, const std::string &workdir ) = 0;
	...
	void on_stdout( const std::string &text )
		{ if (!m_cm) return;
		  if (!m_cm->on_extproc_output(text)) m_cm->log( "stdout(child): " + text, TCS_NOTICE ); }
};
