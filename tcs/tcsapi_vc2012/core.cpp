...
bool compute_module::compute( handler_interface *handler, var_table *data )
{
	...
	if (!handler)
	{
		log("no request handler assigned to computation engine", TCS_ERROR);
		return false;
	}
	m_handler = handler;
	...
	if (!data)
	{
		log("no data object assigned to computation engine", TCS_ERROR);
		return false;
	}
	m_vartab = data;
	...
	if (m_varlist.size() == 0)
	{
		log("no variables defined for computation engine", TCS_ERROR);
		return false;
	}
	
	try { // catch any 'general_error' that can be thrown during precheck, exec, and postcheck

		if (!verify("precheck input", TCS_INPUT)) return false;
		exec();
		if (!verify("postcheck output", TCS_OUTPUT)) return false;

	} catch ( general_error &e )	{
		log( e.err_text, TCS_ERROR, e.time );
		return false;
	}
...
{
	int i=0;
	while ( vi[i].data_type != TCS_INVALID
		&& vi[i].name != NULL )
	{
...
tcs_number_t *compute_module::allocate( const std::string &name, size_t length ) throw( general_error )
{
...
tcs_number_t *compute_module::allocate( const std::string &name, size_t nrows, size_t ncols ) throw( general_error )
{
	var_data *v = assign(name, var_data());
	v->type = TCS_MATRIX;
...
util::matrix_t<tcs_number_t>& compute_module::allocate_matrix( const std::string &name, size_t nrows, size_t ncols ) throw( general_error )
{
	var_data *v = assign(name, var_data());
	v->type = TCS_MATRIX;
	v->num.resize_fill(nrows, ncols, 0.0);
...
int compute_module::as_integer( const std::string &name ) throw( general_error )
{
	...
	if (x.type != TCS_NUMBER) throw cast_error("integer", x, name);
	return (int) x.num;
}
...
{
	var_data &x = value(name);
	if (x.type != TCS_NUMBER) throw cast_error("boolean", x, name);
...
{
	var_data &x = value(name);
	if (x.type != TCS_NUMBER) throw cast_error("float", x, name);
	return (float) x.num;
}

tcs_number_t compute_module::as_number( const std::string &name ) throw( general_error )
{
	var_data &x = value(name);
	if (x.type != TCS_NUMBER) throw cast_error("tcs_number_t", x, name);
	return x.num;
}
...
{
	var_data &x = value(name);
	if (x.type != TCS_NUMBER) throw cast_error("double", x, name);
	return (double) x.num;
}
...
{
	var_data &x = value(name);
	if (x.type != TCS_STRING) throw cast_error("string", x, name);
	return x.str.c_str();
}

tcs_number_t *compute_module::as_array( const std::string &name, size_t *count ) throw( general_error )
{
	var_data &x = value(name);
	if (x.type != TCS_ARRAY) throw cast_error("array", x, name);
...
std::vector<double> compute_module::as_doublevec( const std::string &name ) throw( general_error )
{
	...
	tcs_number_t *p = x.num.data();
	for (size_t k=0;k<len;k++)
		v[k] = (double) p[k];
	return v;
}

tcs_number_t *compute_module::as_matrix( const std::string &name, size_t *rows, size_t *cols ) throw( general_error )
{
	var_data &x = value(name);
	if (x.type != TCS_MATRIX) throw cast_error("matrix", x, name);
	if (rows) *rows = x.num.nrows();
	if (cols) *cols = x.num.ncols();
	return x.num.data();
}

tcs_number_t compute_module::get_operand_value( const std::string &input, const std::string &cur_var_name) throw( general_error )
{	
	...
	if (isalpha(input[0]))
	{
		...
		if (v->type != TCS_NUMBER) throw check_error(cur_var_name, "number type required", input );
		return v->num;
	}
	else
	{
		...
		for ( std::vector< std::string >::iterator it = expr_list.begin(); it != expr_list.end(); ++it )
		{
			...
			else
			{
				...
				if (op == ':')
				{
					...
					{
						var_data *v;
						if ( (v = lookup(rhs)) && v->type == TCS_NUMBER &&  ((int)v->num) != 0)
							return 1;
						else
							return 0;
					}
					...
					else if (lhs == "naof") // check if variable is not assigned OR boolean value is 'false'
					{
						...
						if ( v->type == TCS_NUMBER && ((int)v->num)==0 ) return 1;
				...
				{
					tcs_number_t lhs_val = get_operand_value(lhs,name);
					tcs_number_t rhs_val = get_operand_value(rhs,name);
...
#define fail_constraint( str ) { fail_text = "fail("+name+", "+expr+"): "+std::string(str); return false; }
	...
	for ( std::vector<std::string>::iterator it=exprlist.begin(); it!=exprlist.end(); ++it )
	{
		...
		if (expr == "tmyepw")
		{
			if (dat.type != TCS_STRING || dat.str.length() <= 4)
		...
		else if (expr == "integer")
		{
			...
			if ( ((tcs_number_t)((int)dat.num)) != dat.num )
		...
		{
			if (dat.type != TCS_NUMBER) throw constraint_error(name, "cannot test for positive with non-numeric type", expr);
			if (dat.num <= 0.0)
				fail_constraint( util::to_string( (double)dat.num ) );
		}
		else if (expr == "percent")
		{
			if (dat.type != TCS_NUMBER) throw constraint_error(name, "cannot test for percent (%) constraint with non-numeric type", expr);
			if (dat.num < 0.0 || dat.num > 100.0)
				fail_constraint( util::to_string( (double)dat.num ) );
		}
		else if (expr == "factor")
		{
			if (dat.type != TCS_NUMBER) throw constraint_error(name, "cannot test for factor (0..1) constraint with non-numeric type", expr);
		...
		else if ( (pos=expr.find('=')) != std::string::npos )
		{
			...
			if (test == "min")
			{
				if (dat.type != TCS_NUMBER) throw constraint_error(name, "cannot test for min with non-numeric type", expr);
				double minval = 0;
				if (!util::to_double( rhs, &minval )) throw constraint_error(name, "test for min requires a number value", expr);
				if ( dat.num < (tcs_number_t)minval )
					fail_constraint( util::to_string( (double)dat.num ) );
			}
			else if (test == "max")
			{
				if (dat.type != TCS_NUMBER) throw constraint_error(name, "cannot test for max with non-numeric type", expr);
				double maxval = 0;
				if (!util::to_double( rhs, &maxval )) throw constraint_error(name, "test for max requires a numeric value", expr);
				if (dat.num > (tcs_number_t)maxval )
					fail_constraint( util::to_string( (double)dat.num ) );
			}
			else if (test == "length")
			{
				if (dat.type != TCS_ARRAY) throw constraint_error(name, "cannot test for length with non-array type", expr);
				int lenval = 0;
			...
			{
				if (dat.type != TCS_ARRAY) throw constraint_error(name, "cannot test for length_equal with non-array type", expr);
				var_data *other = lookup( rhs );
				if (!other) throw constraint_error(name, "length_equal cannot find variable to test against", expr);
				if (other->type == TCS_ARRAY)
				{
					if (dat.num.length() != other->num.length())
						fail_constraint( util::to_string( (int) other->num.length() ) );
				}
				else if (other->type == TCS_NUMBER)
				{
					if (dat.num.length() != (size_t)(tcs_number_t)other->num)
			...
			{
				if (dat.type != TCS_ARRAY) throw constraint_error(name, "cannot test for length_multiple_of with non-array type", expr);
				int lenval = 0;
			...
			{
				if (dat.type != TCS_MATRIX) throw constraint_error(name, "cannot test for rows with non-matrix type", expr);
				int nrows = 0;
			...
			{
				if (dat.type != TCS_MATRIX) throw constraint_error(name, "cannot test for cols with non-matrix type", expr);
				int ncols = 0;
...
tcs_number_t *compute_module::accumulate_monthly(const std::string &hourly_var, const std::string &monthly_var, double scale) throw( exec_error )
{
		
	size_t count = 0;
	tcs_number_t *hourly = as_array(hourly_var, &count);

	if (!hourly || count != 8760)
		throw exec_error("generic", "Failed to accumulate hourly: " + hourly_var + " to monthly: " + monthly_var);

	
	tcs_number_t *monthly = allocate( monthly_var, 12 );
...
tcs_number_t compute_module::accumulate_annual(const std::string &hourly_var, const std::string &annual_var, double scale) throw( exec_error )
{
	size_t count = 0;
	tcs_number_t *hourly = as_array(hourly_var, &count);
	...
	assign( annual_var, var_data( (tcs_number_t) (annual*scale) ) );

	return (tcs_number_t)(annual*scale);
}
