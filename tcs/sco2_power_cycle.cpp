...
double HeatExchanger::hxr_UA(double m_dot_0, double m_dot_1, bool scale_UA /*kW/K*/) {
    ...
    if (scale_UA) {
        double m_dot_ratio =
                (m_dot_0 / m_HX_des_par.m_m_dot_design[0] + m_dot_1 / m_HX_des_par.m_m_dot_design[1]) / 2.0;
...
void calculate_turbomachinery_outlet(double T_in /*K*/, double P_in /*kPa*/, double P_out /*kPa*/, double eta /*-*/,
                                     bool is_comp, int &error_code, double &enth_in /*kJ/kg*/,
                                     double &entr_in /*kJ/kg-K*/,
                                     double &dens_in /*kg/m3*/, double &temp_out /*K*/, double &enth_out /*kJ/kg*/,
    ...
    prop_error_code = CO2_PS(P_out, s_in,
    ...
    double w_s = h_in -
                 h_s_out;            // specific work if process is isentropic (negative for compression, positive for expansion)
...
void calculate_hxr_UA(int N_hxrs, double Q_dot /*units?*/, double m_dot_c, double m_dot_h, double T_c_in, double T_h_in,
                      double P_c_in, double P_c_out, double P_h_in, double P_h_out,
    ...
    for (int i = 0; i < N_nodes; i++) {
        ...
        if (i > 0) {
            double C_dot_h =
                    m_dot_h * (h_h_prev - h_h) / (T_h_prev - T_h);            // [kW/K] hot stream capacitance rate
            double C_dot_c =
                    m_dot_c * (h_c_prev - h_c) / (T_c_prev - T_c);            // [kW/K] cold stream capacitance rate
            ...
            double eff = (Q_dot / (double) N_hxrs) /
                         (C_dot_min * (T_h_prev - T_c));    // [-] Effectiveness of each sub-heat exchanger
            double NTU = 0.0;
            if (C_R != 1.0)
...
void compressor::solve_compressor(double T_in, double P_in, double m_dot, double N_in, int &error_code, double &T_out,
                                  double &P_out) {
    ...
    double eta_0 = eta_star * 1.47528 / (pow(m_N_design / m_N, pow(20.0 * phi_star,
                                                                   5.0)));        // Efficiency is normalized so it equals 1.0 at phi_star = 0.0297 (the compressor's design point)
    m_eta = max(eta_0 * m_comp_des_par.m_eta_design,
    ...
    double dh_s = psi * pow(U_tip, 2) *
                  0.001;                // ideal enthalpy rise in compressor, from definition of head coefficient (kJ/kg)
    ...
    if (prop_error_code !=
        0)        // most likely case is that the outlet pressure is above the high pressure limit of the property routine
    {
        error_code = 17;
        return;
    }
    ...
    prop_error_code = CO2_PH(P_out, h_out,
                             &co2_props);        // determines compressor outlet temperature and speed of sound
    if (prop_error_code !=
        0)        // most likely case is that the outlet pressure is above the high pressure limit of the property routine
    {
        error_code = 18;
...
void turbine::solve_turbine(double T_in, double P_in, double P_out, double N_in, int &error_code, double &T_out,
                            double &m_dot_out) {
    m_N = N_in;
    ...
    double C_s = sqrt((h_in - h_s_out) *
                      2000.0);                    // Spouting velocity (m/s) [note: there is a mutiple of 2 in the equation that was folded into the *1000 unit conversion]
...
//	recomp_frac = I_recomp_frac;
    ...
    if (m_cycle_des_par.m_DP_LT[1 - cpp_offset] < 0.0)
        m_pres_last[3 - cpp_offset] = m_pres_last[2 - cpp_offset] - m_pres_last[2 - cpp_offset] *
                                                                    abs(m_cycle_des_par.m_DP_LT[1 -
                                                                                                cpp_offset]);        // Relative pressure drop specified for LT recuperator (cold stream)
    else
        m_pres_last[3 - cpp_offset] = m_pres_last[2 - cpp_offset] - m_cycle_des_par.m_DP_LT[1 -
                                                                                            cpp_offset];                                    // Absolute pressure drop specified for LT recuperator (cold stream)
    ...
    if (UA_LT < 1.E-12)
        m_pres_last[3 - cpp_offset] = m_pres_last[2 -
                                                  cpp_offset];        // if there is no LT recuperator, there is no pressure drop
    ...
    if (m_cycle_des_par.m_DP_HT[1 - cpp_offset] < 0.0)
        m_pres_last[5 - cpp_offset] = m_pres_last[4 - cpp_offset] - m_pres_last[4 - cpp_offset] *
                                                                    abs(m_cycle_des_par.m_DP_HT[1 -
                                                                                                cpp_offset]);    // relative pressure drop specified for HT recuperator (cold stream)
    else
        m_pres_last[5 - cpp_offset] = m_pres_last[4 - cpp_offset] - m_cycle_des_par.m_DP_HT[1 -
                                                                                            cpp_offset];                                // absolute pressure drop specified for HT recuperator (cold stream)

    if (UA_HT < 1.E-12)
        m_pres_last[5 - cpp_offset] = m_pres_last[4 -
                                                  cpp_offset];        // if there is no HT recuperator, there is no pressure drop

    if (m_cycle_des_par.m_DP_PHX[1 - cpp_offset] < 0.0)
        m_pres_last[6 - cpp_offset] = m_pres_last[5 - cpp_offset] - m_pres_last[5 - cpp_offset] *
                                                                    abs(m_cycle_des_par.m_DP_PHX[1 -
                                                                                                 cpp_offset]);    // relative pressure drop specified for PHX
    else
        m_pres_last[6 - cpp_offset] = m_pres_last[5 - cpp_offset] - m_cycle_des_par.m_DP_PHX[1 -
                                                                                             cpp_offset];                                // absolute pressure drop specified for PHX

    if (m_cycle_des_par.m_DP_PC[2 - cpp_offset] < 0.0)
        m_pres_last[9 - cpp_offset] = m_pres_last[1 - cpp_offset] / (1.0 - abs(m_cycle_des_par.m_DP_PC[2 -
                                                                                                       cpp_offset]));            // relative pressure drop specified for precooler [P1 = P9 - P9*rel_DP => P1 = P9*(1-rel_DP)
    else
        m_pres_last[9 - cpp_offset] = m_pres_last[1 - cpp_offset] + m_cycle_des_par.m_DP_PC[2 -
                                                                                            cpp_offset];                                        // absolute pressure drop specified for precooler

    if (m_cycle_des_par.m_DP_LT[2 - cpp_offset] < 0.0)
        m_pres_last[8 - cpp_offset] = m_pres_last[9 - cpp_offset] / (1.0 - abs(m_cycle_des_par.m_DP_LT[2 -
                                                                                                       cpp_offset]));            // relative pressure drop specified for LT recuperator (hot stream)
    else
        m_pres_last[8 - cpp_offset] = m_pres_last[9 - cpp_offset] + m_cycle_des_par.m_DP_LT[2 -
                                                                                            cpp_offset];                        // absolute pressure drop specified for LT recuperator (hot stream)

    if (UA_LT < 1.E-12)
        m_pres_last[8 - cpp_offset] = m_pres_last[9 -
                                                  cpp_offset];        // if there is no LT recup, there is no pressure drop

    if (m_cycle_des_par.m_DP_HT[2 - cpp_offset] < 0.0)
        m_pres_last[7 - cpp_offset] = m_pres_last[8 - cpp_offset] / (1.0 - abs(m_cycle_des_par.m_DP_HT[2 -
                                                                                                       cpp_offset]));            // relative pressure drop specified for HT recup
    else
        m_pres_last[7 - cpp_offset] = m_pres_last[8 - cpp_offset] + m_cycle_des_par.m_DP_HT[2 -
                                                                                            cpp_offset];                        // absolute pressure drop specified for HT recup
    ...
    calculate_turbomachinery_outlet(m_temp_last[1 - cpp_offset], m_pres_last[1 - cpp_offset],
                                    m_pres_last[2 - cpp_offset], m_cycle_des_par.m_eta_mc,
                                    true, sub_error_code, m_enth_last[1 - cpp_offset], m_entr_last[1 - cpp_offset],
                                    m_dens_last[1 - cpp_offset],
                                    m_temp_last[2 - cpp_offset], m_enth_last[2 - cpp_offset],
                                    m_entr_last[2 - cpp_offset], m_dens_last[2 - cpp_offset], w_mc);
    ...
    calculate_turbomachinery_outlet(m_temp_last[6 - cpp_offset], m_pres_last[6 - cpp_offset],
                                    m_pres_last[7 - cpp_offset], m_cycle_des_par.m_eta_t,
                                    false, sub_error_code, m_enth_last[6 - cpp_offset], m_entr_last[6 - cpp_offset],
                                    m_dens_last[6 - cpp_offset],
                                    m_temp_last[7 - cpp_offset], m_enth_last[7 - cpp_offset],
                                    m_entr_last[7 - cpp_offset], m_dens_last[7 - cpp_offset], w_t);
    ...
    if (m_cycle_des_par.m_recomp_frac >= 1.E-12) {
        ...
        calculate_turbomachinery_outlet(m_temp_last[2 - cpp_offset], m_pres_last[9 - cpp_offset],
                                        m_pres_last[10 - cpp_offset], m_cycle_des_par.m_eta_rc,
                                        true, sub_error_code, dummy[0], dummy[1], dummy[2], dummy[3], dummy[4],
    ...
    for (T8_iter = 1; T8_iter <= max_iter; T8_iter++) {
        property_error_code = CO2_TP(m_temp_last[8 - cpp_offset], m_pres_last[8 - cpp_offset],
        ...
        for (T9_iter = 1; T9_iter <= max_iter; T9_iter++) {
            ...
            if (m_cycle_des_par.m_recomp_frac >= 1.E-12) {
                calculate_turbomachinery_outlet(m_temp_last[9 - cpp_offset], m_pres_last[9 - cpp_offset],
                                                m_pres_last[10 - cpp_offset], m_cycle_des_par.m_eta_rc,
                                                true, sub_error_code, m_enth_last[9 - cpp_offset],
                                                m_entr_last[9 - cpp_offset], m_dens_last[9 - cpp_offset],
                                                m_temp_last[10 - cpp_offset],
                                                m_enth_last[10 - cpp_offset], m_entr_last[10 - cpp_offset],
            ...
            m_dot_t = m_cycle_des_par.m_W_dot_net /
                      (w_mc * (1.0 - m_cycle_des_par.m_recomp_frac) + w_rc * m_cycle_des_par.m_recomp_frac +
            ...
            if (UA_LT <
                1.E-12)            // no low-temp recuperator (this check is necessary to prevent pressure drops with UA=0 from causing problems)
                Q_dot_LT = 0.0;
            else
                Q_dot_LT = m_dot_t * (m_enth_last[8 - cpp_offset] - m_enth_last[9 - cpp_offset]);

            calculate_hxr_UA(m_cycle_des_par.m_N_sub_hxrs, Q_dot_LT, m_dot_mc, m_dot_t, m_temp_last[2 - cpp_offset],
                             m_temp_last[8 - cpp_offset],
                             m_pres_last[2 - cpp_offset], m_pres_last[3 - cpp_offset], m_pres_last[8 - cpp_offset],
            ...
            if (sub_error_code > 0) {
                if (sub_error_code == 11)        // second - law violation in hxr, therefore temp(9) is too low
                {
                    ...
                    m_temp_last[9 - cpp_offset] =
                            (T9_lower_bound + T9_upper_bound) * 0.5;        // bisect bounds for next guess
            ...
            double secant_guess = m_temp_last[9 - cpp_offset] -
                                  ...
                                  (last_LT_residual - UA_LT_residual);    // next guess predicted using secant method
        ...
        m_enth_last[3 - cpp_offset] = m_enth_last[2 - cpp_offset] + Q_dot_LT /
        ...
        if (m_cycle_des_par.m_recomp_frac >= 1.E-12) {
            m_enth_last[4 - cpp_offset] = (1.0 - m_cycle_des_par.m_recomp_frac) * m_enth_last[3 - cpp_offset] +
                                          m_cycle_des_par.m_recomp_frac * m_enth_last[10 -
                                                                                      cpp_offset];        // conservation of energy (both sides divided by m_dot_t
        ...
        if (m_temp_last[4 - cpp_offset] >=
            m_temp_last[8 - cpp_offset])        // temp(8) is not valid; it must be higher than it is
        {
        ...
        if (UA_HT <
            1.E-12)        // no high-temp recuperator (this check is necessary to prevent pressure drops with UA=0 from causing problems)
            Q_dot_HT = 0.0;
        else
            Q_dot_HT = m_dot_t * (m_enth_last[7 - cpp_offset] - m_enth_last[8 - cpp_offset]);

        calculate_hxr_UA(m_cycle_des_par.m_N_sub_hxrs, Q_dot_HT, m_dot_t, m_dot_t, m_temp_last[4 - cpp_offset],
                         m_temp_last[7 - cpp_offset],
                         m_pres_last[4 - cpp_offset], m_pres_last[5 - cpp_offset], m_pres_last[7 - cpp_offset],
        ...
        if (sub_error_code > 0) {
            if (sub_error_code == 1)        // 2nd law violation in hxr, therefore temp(8) is too low
            {
                T8_lower_bound = m_temp_last[8 - cpp_offset];
                m_temp_last[8 - cpp_offset] =
                        (T8_lower_bound + T8_upper_bound) * 0.5;    // bisect bounds for next guess
                continue;    // cycle T8_loop
            } else {
        ...
        double secant_guess = m_temp_last[8 - cpp_offset] -
                              ...
                              (last_HT_residual - UA_HT_residual);        // next guess predicted using secant method
    ...
    m_enth_last[5 - cpp_offset] = m_enth_last[4 - cpp_offset] +
                                  Q_dot_HT / m_dot_t;        // Energy balance on cold stream of high-temp recuperator
    ...
    // ****************************************************************
    // LT Recuperator
    property_error_code = CO2_TP(m_temp_last[2 - cpp_offset], m_pres_last[9 - cpp_offset],
    ...
    */
    ...
    // HT recuperator
    property_error_code = CO2_TP(m_temp_last[4 - cpp_offset], m_pres_last[4 - cpp_offset],
...
bool RecompCycle::optimal_design() {
    ...
    if (index > 0) {
        ...
        opt_des_cycle.set_max_objective(nlopt_callback_opt_des,
                                        this);        // Calls wrapper/callback that calls 'design_point_eta', which optimizes design point eta through repeated calls to 'design'
...
bool RecompCycle::auto_optimal_design() {
    ...
    if (m_cycle_des_par.m_P_high_limit - best_P_high >
        1.0) {    // If the optimized high pressure is not at the limit, check results at the limit
...
bool RecompCycle::optimal_off_design(const cycle_opt_off_des_inputs &cycle_opt_off_des_in_in) {
    ...
    if (!off_design_target_power()) {
        ...
        m_q_dot_in_od = 0.0;
        m_errors.SetError(
                9999);        // Need to add this error message "Error at optimal off-design solution - perhaps target is not possible"
        return false;
    }
...
bool RecompCycle::off_design_target_power() {
    ...
    if (!off_design(cycle_off_des_in)) {
        m_errors.SetError(
                63);        // This is going to display orders in opposite order than John's code: shouldn't be a problem?
        return false;
    }
...
bool RecompCycle::off_design(const cycle_off_des_inputs &cycle_off_des_in_in) {
    ...
    double m_dot_mc_max = m_mc.calculate_m_dot_max(rho_in,
                                                   N_mc_local);        // Highest possible mass flow rate in main compressor
    m_dot_mc_max *= 2.0;                                                    // Add a safety factor (helps with convergence)

    double m_dot_upper_bound = m_dot_mc_max / (1.0 -
                                               m_cycle_off_des_in.m_S.m_recomp_frac);            // Largest possible total mass flow rate (through turbine)
    ...
    double m_dot_t_guess = (m_dot_lower_bound + m_dot_upper_bound) *
    ...
    {
        m_dot_rc = m_dot_t_guess *
                   m_cycle_off_des_in.m_S.m_recomp_frac;                // mass flow rate through recompressing compressor
        m_dot_mc = m_dot_t_guess - m_dot_rc;                    // mass flow rate through compressor

        // Calculate the pressure rise through the main compressor
        int sub_error_code = 0;
        m_mc.solve_compressor(m_temp_od_last[1 - cpp_offset], m_pres_od_last[1 - cpp_offset], m_dot_mc, N_mc_local,
                              sub_error_code,
        ...
        if (sub_error_code != 0) {
            if (sub_error_code == 16)        // m_dot is too high because the given shaft speed is not possible
            {
                ...
                m_dot_t_guess =
                        (m_dot_lower_bound + m_dot_upper_bound) * 0.5;    // Use bisection for new mass flow rate guess
                continue;
        ...
        m_pres_od_last[3 - cpp_offset] = m_pres_od_last[2 - cpp_offset] - m_LT.hxr_DP(1 - cpp_offset, m_dot_mc,
        ...
        m_pres_od_last[5 - cpp_offset] = m_pres_od_last[4 - cpp_offset] - m_HT.hxr_DP(1 - cpp_offset, m_dot_t_guess,
                                                                                      true);            // through cold-side of high-temp recuperator
        m_pres_od_last[6 - cpp_offset] = m_pres_od_last[5 - cpp_offset] - m_PHX.hxr_DP(1 - cpp_offset, m_dot_t_guess,
                                                                                       true);        // through CO2 (cold) side of primary heat exchanger
        m_pres_od_last[9 - cpp_offset] = m_pres_od_last[1 - cpp_offset] + m_PC.hxr_DP(2 - cpp_offset, m_dot_mc,
                                                                                      true);                // through CO2 side of precooler (starting from known outlet pressure)
        m_pres_od_last[8 - cpp_offset] = m_pres_od_last[9 - cpp_offset] + m_LT.hxr_DP(2 - cpp_offset, m_dot_t_guess,
                                                                                      true);            // through hot-side of low-temp recuperator (starting from known outlet pressure)
        m_pres_od_last[7 - cpp_offset] = m_pres_od_last[8 - cpp_offset] + m_HT.hxr_DP(2 - cpp_offset, m_dot_t_guess,
                                                                                      true);            // through hot-side of high-temp recuperator (starting from known outlet pressure)

        // Calculate the allowable mass flow rate through the turbine
        m_dot_t = std::numeric_limits<double>::quiet_NaN();
        m_t.solve_turbine(m_temp_od_last[6 - cpp_offset], m_pres_od_last[6 - cpp_offset],
                          m_pres_od_last[7 - cpp_offset], N_t_local, sub_error_code, m_temp_od_last[7 - cpp_offset],
                          m_dot_t);
        if (sub_error_code > 0) {
        ...
        double m_dot_residual = m_dot_t_guess -
                                m_dot_t;        // residual between guessed total mass flow rate and turbine mass flow rates
        double secant_guess = m_dot_t_guess - m_dot_residual * (last_m_dot_guess - m_dot_t_guess) /
                                              (last_m_dot_residual -
                                               m_dot_residual);        // next guess predicted using secant method
        if (m_dot_residual > 0.0) {
            if (m_dot_residual / m_dot_t <
                m_cycle_off_des_in.m_S.m_tol)    // residual is positive; check for convergence
                break;
            m_dot_upper_bound = m_dot_t_guess;        // reset lower bound
        } else {
            if (-m_dot_residual / m_dot_t <
                m_cycle_off_des_in.m_S.m_tol)    // residual is negative; check for convergence
                break;
        ...
        if (secant_guess < m_dot_lower_bound ||
            secant_guess > m_dot_upper_bound)    // secant method overshot, use bisection
    ...
    for (T8_iter = 1; T8_iter <= max_iter; T8_iter++) {
        ...
        prop_error_code = CO2_TP(m_temp_od_last[8 - cpp_offset], m_pres_od_last[8 - cpp_offset],
        ...
        if (UA_LT < 1.E-12)    // no low-temp recuperator
        {
            ...
            m_temp_od_last[9 - cpp_offset] =
                    (T9_lower_bound + T9_upper_bound) * 0.5;        // bisect bounds for first guess
            UA_LT_calc = -1.0;
            last_LT_residual = UA_LT;                    // know a priori that with T9 = T8, UA_calc = 0 therefore residual is UA_LT-0
            last_T9_guess = m_temp_od_last[8 - cpp_offset];
        }
        ...
        for (T9_iter = 1; T9_iter <= max_iter; T9_iter++) {
            ...
            prop_error_code = CO2_TP(m_temp_od_last[9 - cpp_offset], m_pres_od_last[9 - cpp_offset],
            ...
            if (m_cycle_off_des_in.m_S.m_recomp_frac >= 1.0E-12) {
                ...
                m_rc.solve_compressor(m_temp_od_last[9 - cpp_offset], m_pres_od_last[9 - cpp_offset], m_dot_rc,
                                      last_N_rc_guess, sub_error_code, m_temp_od_last[10 - cpp_offset], pres_10_calc);
                if (sub_error_code != 0) {
                ...
                for (N_rc_iter = 1; N_rc_iter <= max_iter; N_rc_iter++) {
                    m_rc.solve_compressor(m_temp_od_last[9 - cpp_offset], m_pres_od_last[9 - cpp_offset], m_dot_rc,
                                          N_rc_guess, sub_error_code, m_temp_od_last[10 - cpp_offset], pres_10_calc);
                    ...
                    double secant_guess = N_rc_guess - pres_10_residual * (last_N_rc_guess - N_rc_guess) /
                                                       (last_pres_10_residual -
                                                        pres_10_residual);    // next guess predicted using secant method
            ...
            // Calculate the UA value of the low-temperature recuperator.
            if (UA_LT <
                1.E-12)                // then no low-temp recuperator (this check is necessary to prevent pressure drops with UA=0 from causing problems)
                Q_dot_LT = 0.0;
            else
            ...
            calculate_hxr_UA(m_cycle_off_des_in.m_S.m_N_sub_hxrs, Q_dot_LT, m_dot_mc, m_dot_t,
            ...
            double secant_guess = m_temp_od_last[9 - cpp_offset] -
            ...
            if (secant_guess <= T9_lower_bound || secant_guess >= T9_upper_bound ||
                secant_guess != secant_guess)        //then  ! secant method overshot (or is NaN), use bisection
        ...
        // State 3 can now be fully defined
        m_enth_od_last[3 - cpp_offset] = m_enth_od_last[2 - cpp_offset] + Q_dot_LT /
                                                                          m_dot_mc;    // Energy balance on cold stream of low-temp recuperator
        ...
        if (m_cycle_off_des_in.m_S.m_recomp_frac >= 1.0E-12) {
            m_enth_od_last[4 - cpp_offset] =
                    (1.0 - m_cycle_off_des_in.m_S.m_recomp_frac) * m_enth_od_last[3 - cpp_offset] +
                    m_cycle_off_des_in.m_S.m_recomp_frac * m_enth_od_last[10 -
                                                                          cpp_offset];            // conservation of energy (both sides divided by m_dot_t)
        ...
        if (m_temp_od_last[4 - cpp_offset] >=
            m_temp_od_last[8 - cpp_offset])        // temp(8) is not valid; it must be higher than it is
        {
        ...
        calculate_hxr_UA(m_cycle_off_des_in.m_S.m_N_sub_hxrs, Q_dot_HT, m_dot_t, m_dot_t,
                         m_temp_od_last[4 - cpp_offset], m_temp_od_last[7 - cpp_offset],
                         m_pres_od_last[4 - cpp_offset], m_pres_od_last[5 - cpp_offset], m_pres_od_last[7 - cpp_offset],
                         m_pres_od_last[8 - cpp_offset], sub_error_code, UA_HT_calc, min_DT_HT);
        ...
        double secant_guess = m_temp_od_last[8 - cpp_offset] -
                              UA_HT_residual * (last_T8_guess - m_temp_od_last[8 - cpp_offset]) / (last_HT_residual -
        ...
        // Check if the secant method overshoots and fall back to bisection if it does.
        if (secant_guess <= T8_lower_bound ||
            secant_guess >= T8_upper_bound)        // then  ! secant method overshot, use bisection
            m_temp_od_last[8 - cpp_offset] = (T8_lower_bound + T8_upper_bound) * 0.5;
        else
        ...
        // State 5 can now be fully defined
        m_enth_od_last[5 - cpp_offset] = m_enth_od_last[4 - cpp_offset] + Q_dot_HT /
    ...
    m_enth_od_last[5 - cpp_offset] =
            m_enth_od_last[4 - cpp_offset] + Q_dot_HT / m_dot_t;    // Energy balance on cold stream of HT recup
    prop_error_code = CO2_PH(m_pres_od_last[5 - cpp_offset], m_enth_od_last[5 - cpp_offset], &co2_props);
    if (prop_error_code != 0) {
        m_errors.SetError(60);
        return false;
