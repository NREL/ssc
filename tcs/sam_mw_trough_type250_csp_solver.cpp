...
public:
    ...
    virtual int init() {
        ...
        */
        ...
        ms_trough.m_T_fp = value(
                P_T_FP);                    //Freeze protection temperature (heat trace activation temperature) [C]
        ...
        ms_trough.m_Pipe_hl_coef = value(
        ...
        ms_trough.m_ColTilt = value(
                P_COLTILT);                //Collector tilt angle (0 is horizontal, 90deg is vertical) [deg]
        ...
        ms_trough.m_accept_init = value(P_ACCEPT_INIT) ==
                                  1;                // In acceptance testing mode - require steady-state startup [none]
        ms_trough.m_accept_loc = (int) value(
        ...
        ms_trough.m_is_using_input_gen = (bool) value(
                P_USING_INPUT_GEN);    // Is model getting inputs from input generator (true) or from other components in physical trough SYSTEM model (false)

        ms_trough.m_solar_mult = value(P_SOLAR_MULT);        //Solar multiple [none]
        ms_trough.m_mc_bal_hot_per_MW = value(
                P_MC_BAL_HOT);        //The heat capacity of the balance of plant on the hot side [kWht/K-MWt]
        ms_trough.m_mc_bal_cold_per_MW = value(
                P_MC_BAL_COLD);        //The heat capacity of the balance of plant on the cold side [kWht/K-MWt]
        ms_trough.m_mc_bal_sca = value(
        ...
        int n_W_aper = -1;
        double *W_aper = value(P_W_APERTURE,
                               &n_W_aper);        //The collector aperture width (Total structural area.. used for shadowing) [m]
        ...
        int n_colper = -1;
        double *colper = value(P_COLPERSCA,
                               &n_colper);        //The number of individual collector sections in an SCA  [none]
    ...
    virtual int call(double time, double step, int ncall) {
        ...
        ms_htf_state_in.m_m_dot =
                value(I_M_DOT_IN) / 3600.0;        //[kg/s] HTF mass flow rate at the inlet, convert from kg/hr
        ...
        value(O_SCA_PAR_TOT,
              ms_out_solver.m_W_dot_col_tracking);        //[MWe] Parasitic electric power consumed by the SC
        ...
        value(O_T_C_IN_CALC,
              0.0);    //[C] Calculated cold HTF inlet temperature - used in freeze protection and for stand-alone model in recirculation
