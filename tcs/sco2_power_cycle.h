...
public:
    ...
    bool initialize(const compressor_design_parameters &comp_des_par_in) {
        ...
        get_compressor_parameters(m_comp_des_par.m_type, m_phi_design, m_phi_min,
                                  m_phi_max);        // Get compressor-specific parameters

        if (m_comp_des_par.m_recomp_frac_design > 1.E-12) {
            double psi_design = std::numeric_limits<double>::quiet_NaN();
            psi_design = compressor_psi_polynomial_fit(m_comp_des_par.m_type,
                                                       m_phi_design);        // Get modified ideal head coefficient at design

            double U_tip = sqrt(1000.0 * (-(m_comp_des_par.m_w_design * m_comp_des_par.m_eta_design)) /
                                psi_design);        // Rearranging definition of head coefficient

            m_D_rotor = sqrt(m_comp_des_par.m_m_dot_design / (m_phi_design * m_comp_des_par.m_rho_in_design *
                                                              U_tip));                // Rearranging definition of flow coefficient
    ...
    double calculate_m_dot_max(double rho_in, double N_mc_local) {
        return m_phi_max * rho_in * pow(m_D_rotor, 2) * m_D_rotor * 0.5 * N_mc_local *
               0.10471975512;        // Highest possible mass flow rate in main compressor (pressure rise would be zero)
    }
    ...
    void solve_compressor(double T_in, double P_in, double m_dot, double N_in, int &error_code, double &T_out,
                          double &P_out);
...
public:
    ...
    bool initialize(const turbine_design_parameters &turb_des_par_in) {
        ...
        double C_s = sqrt(2.0 * (m_turb_des_par.m_w_design / m_turb_des_par.m_eta_design) *
                          1000.0);            // [m/s] Spouting velocity
        ...
        m_A_nozzle = m_turb_des_par.m_m_dot_design / (C_s *
                                                      m_turb_des_par.m_rho_out_design);        // [m2] Turbine effective nozzle area in m2 (uses density at turbine outlet)
    ...
    void
    solve_turbine(double T_in, double P_in, double P_out, double N_in, int &error_code, double &T_out, double &m_dot);
...
private:
    ...
    void set_des_data() {
        m_temp_des = m_temp_last;
        m_pres_des = m_pres_last;
        m_enth_des = m_enth_last;
        m_entr_des = m_entr_last;
        m_dens_des = m_dens_last;
    ...
    void set_od_data() {
        m_temp_od = m_temp_od_last;
        m_pres_od = m_pres_od_last;
        m_enth_od = m_enth_od_last;
        m_entr_od = m_entr_od_last;
        m_dens_od = m_dens_od_last;
