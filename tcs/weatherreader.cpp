        ...
        if (ncall == 0) // only read data values once per timestep
        {
            ...
            for (int i = 0; i < nread; i++) {        //for all calls except the first, nread=1
                ...
                if (!m_wfile.read(&m_rec)) {
                    message(TCS_ERROR, "failed to read from weather file %s at time %lg", m_wfile.filename().c_str(),
                            time);
                    return -1; // error code
                }
...
TCS_IMPLEMENT_TYPE(weatherreader, "Standard Weather File format reader", "Aron Dobos", 1, weatherreader_variables, NULL,
                   0)
