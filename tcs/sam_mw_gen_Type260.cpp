...
public:
    ...
    virtual int init() {
        ...
        */
        ...
        interp_arr = (int) value(
                P_INTERP_ARR);        //Interpolate the array or find nearest neighbor? (1=interp,2=no) [none]
        rad_type = (int) value(
                P_RAD_TYPE);        //Solar resource radiation type (1=DNI,2=horiz.beam,3=tot.horiz) [none]
        ...
        sfhlQ_coefs = value(P_SFHLQ_COEFS,
                            &nval_sfhlQ_coefs);        //Irr-based solar field thermal loss adjustment coefficients [1/MWt]
        sfhlT_coefs = value(P_SFHLT_COEFS,
                            &nval_sfhlT_coefs);        //Temp.-based solar field thermal loss adjustment coefficients [1/C]
        sfhlV_coefs = value(P_SFHLV_COEFS,
                            &nval_sfhlV_coefs);        //Wind-based solar field thermal loss adjustment coefficients [1/(m/s)]
        ...
        etaQ_coefs = value(P_ETAQ_COEFS,
                           &nval_etaQ_coefs);        //Part-load power conversion efficiency adjustment coefficients [1/MWt]
        etaT_coefs = value(P_ETAT_COEFS,
                           &nval_etaT_coefs);        //Temp.-based power conversion efficiency adjustment coefs. [1/C]
        ...
        Wpar_prodQ_coefs = value(P_WPAR_PRODQ_COEFS,
                                 &nval_Wpar_prodQ_coefs);        //Part-load production parasitic adjustment coefs. [1/MWe]
        Wpar_prodT_coefs = value(P_WPAR_PRODT_COEFS,
                                 &nval_Wpar_prodT_coefs);        //Temp.-based production parasitic adjustment coefs. [1/C]
        ...
        f_teshl_ref = value(
                P_F_TESHL_REF);        //Reference heat loss from storage per max stored capacity [kWt/MWhr-stored]
        teshlX_coefs = value(P_TESHLX_COEFS,
                             &nval_teshlX_coefs);        //Charge-based thermal loss adjustment - constant coef. [1/MWhr-stored]
        teshlT_coefs = value(P_TESHLT_COEFS,
                             &nval_teshlT_coefs);        //Temp.-based thermal loss adjustment - constant coef. [1/C]
        ...
        */
        if (!istableunsorted) {
            ...
            */
            ...
            if ((nrow_OpticalTable < 5 && ncol_OpticalTable > 3) || (ncol_OpticalTable == 3 && nrow_OpticalTable > 4))
                message(TCS_WARNING,
                        "The optical efficiency table option flag may not match the specified table format. If running SSC, ensure \"IsTableUnsorted\""
                        " =0 if regularly-spaced azimuth-zenith matrix is used and =1 if azimuth,zenith,efficiency points are specified.");

            if (nrow_OpticalTable <= 0 || ncol_OpticalTable <=
                                          0) // If these were not set correctly, it will create memory allocation crash not caught by error handling.
                return -1;
		    ...
		    */
            ...
            if (ncol_OpticalTable != 3) {
                message(TCS_ERROR,
                        "The heliostat field efficiency file is not formatted correctly. Type expects 3 columns"
            ...
            if (err_fit > 0.01)
                message(TCS_WARNING, "The heliostat field interpolation function fit is poor! (err_fit=%f RMS)",
                        err_fit);
        ...
        etes0 = f_etes_0 *
                etesmax;        //[MW-hr] Initial value in thermal storage. This keeps track of energy in thermal storage, or e_in_tes
    ...
    virtual int call(double time, double step, int ncall) {
        ...
        */
        ...
        double DepHr3 = (tan(pi - theta_dep) < 0. ? -1. : 1.) *
        ...
        double StwHr3 = (tan(pi - theta_stow) < 0. ? -1. : 1.) *
        ...
        solalt = asin(sin(dec) * sin(latitude) + cos(latitude) * cos(dec) * cos(omega));
        solaz = (omega < 0. ? -1. : 1.) * fabs(acos(min(1.0, (cos(pi / 2. - solalt) * sin(latitude) - sin(dec)) /
                                                             (sin(pi / 2. - solalt) * cos(latitude)))));
        ...
        double eta_arr = max(opt_val * Ftrack,
                             0.0);  //mjw 7.25.11 limit zenith to <90, otherwise the interpolation error message gets called during night hours.
        ...
        double f_sfhl = 1.0 - f_sfhl_ref * f_sfhl_qdni * f_sfhl_tamb *
                              f_sfhl_vwind;  //This ratio indicates the sf thermal efficiency
        ...
        if (hrs_tes <= 0.) { // No Storage
            ...
            if (pbmode0 == 0) {
                ...
                if ((q_sf + EtesA >= qdisp[touperiod]) || (q_sf > ptsmax)) {
                    ...
                    if (q_sf > q_to_pb) {             // if solar field output is greater than what the necessary load ?
                        ...
                        if (q_to_tes >
                            ptsmax) {       // if q to thermal storage exceeds thermal storage max rate Added 9-10-02
                    ...
                    e_in_tes = etes0 - q_startup +
                               (q_sf - q_to_pb) * dt;   // thermal storage energy is initial + what was left
                ...
                if ((q_sf + max(0.0, etes0 - dispatch.at(touperiod)) / dt) >
                    qdisp[touperiod]) { // if there is sufficient energy to operate at dispatch target output
                    ...
                    if ((e_in_tes > etesmax) && (q_to_pb < qttmax)) { // qttmax (MWt) - power to turbine max
                        if ((e_in_tes - etesmax) / dt < (qttmax - q_to_pb)) {
                            ...
                            e_in_tes = e_in_tes -
                                       (qttmax - q_to_pb) * dt;  // should this be etes0 instead of e_in_tes on RHS ??
                            q_to_pb = qttmax;
                        }
            ...
            e_in_tes = max(e_in_tes - q_hl_tes * dt,
                           0.0); // Adjust the energy in thermal storage according to TES thermal losses
        ...
        if (q_to_pb <
            fdisp[touperiod]) {    // If the thermal power dispatched to the power cycle is less than the level in the fossil control
            q_fossil = fdisp[touperiod] -
                       q_to_pb;    // then the fossil used is the fossil control value minus what's provided by the solar field
