...
public:
    ...
    virtual int init() {
        ...
        dsg_lf.m_PB_fixed_par = value(
                P_PB_FIXED_PAR); // PB_fixed_par );	dsg_lf.q_max_aux = value("T_startup", value("T_hot"));
        ...
        double *bop_array = value(P_BOP_ARRAY,
                                  &nval_bop_array);        //The collector aperture width (Total structural area.. used for shadowing) [m]
        ...
        set_matrix_t(P_B_OPTICALTABLE,
                     dsg_lf.m_b_OpticalTable); // opt_data); [-] Boiler Optical Table double check oooooooooooo
    ...
    virtual int call(double time, double step, int ncall) {
        ...
        value(O_M_DOT_TO_PB,
              dsg_out_solver.m_m_dot_salt_tot);            //[kg/hr] Flow rate delivered to the power block
        value(O_STANDBY_CONTROL,
              dsg_out_solver.m_standby_control);        //[none] Standby control flag - used by Type224
        value(O_DP_SF_SH,
              dsg_out_solver.m_dP_sf_sh);                    //[bar] Pressure drop across the solar field superheater
        ...
        value(O_Q_AUX_FLUID,
              0.0);                       //[MW] Thermal energy provided to the fluid passing through the aux heater
        value(O_Q_AUX_FUEL,
              0.0);                   //[W] --> [MMBTU] Heat content of fuel required to provide aux firing
