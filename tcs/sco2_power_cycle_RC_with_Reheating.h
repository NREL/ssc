#ifndef __SCO2_PC_RC_with_Reheating
#define __SCO2_PC_RC_with_Reheating
...
struct cycle_design_parameters_RC_with_Reheating
{
	...
	double m_N_mt;						//[rpm] main turbine shaft speed: required for calculating physical turbomachinery parameters
	double m_N_rt;						//[rpm] reheating turbine shaft speed: required for calculating physical turbomachinery parameters
	double m_eta_mc;					//[-] Isentropic efficiency of main compressor
	double m_eta_rc;					//[-] Isentropic efficiency of recompessor
	double m_eta_mt;					//[-] Isentropic efficiency of main turbine
	double m_eta_rt;					//[-] Isentropic efficiency of reheating turbine
	...
	cycle_design_parameters_RC_with_Reheating()
	{
		...
		m_fixed_LT_frac = m_fixed_P_mc_out = m_fixed_PR_mc = m_fixed_recomp_frac = m_fixed_P_rt_in =false;
		
		m_W_dot_net = m_T_mc_in = m_T_mt_in = m_T_rt_in = m_P_rt_in = m_N_mt =  m_N_rt = m_eta_mc = m_eta_rc = m_eta_mt = m_eta_rt = m_tol = m_opt_tol = 
		m_UA_rec_total = m_LT_frac = m_LT_frac_guess = m_P_mc_out = 
		m_P_high_limit = m_P_mc_out_guess = m_PR_mc = m_PR_mc_guess = m_recomp_frac = 
		m_P_rt_in_guess = m_recomp_frac_guess = std::numeric_limits<double>::quiet_NaN();
...
struct S_cycle_design_metrics_RC_with_Reheating
{
	...
	double m_m_dot_RHX;			//[kg/s] Mass flow rate through reheating heat exchanger
...
struct cycle_opt_off_des_inputs_RC_with_Reheating
{
	...
	bool m_fixed_N_mt;			//[-] Is the main turbine speed fixed or available for optimization
	double m_N_mt;				//[rpm] Main Turbine speed [use negative value to couple to main compressor]
	double m_N_mt_guess;			//[rpm] Guess for main turbine speed: only used if it is available for optimization
	
	bool m_fixed_N_rt;			//[-] Is the reheating turbine speed fixed or available for optimization
...
class RecompCycle_with_Reheating
{
public:
	// member classes
	...
	void set_des_data()
	{
		...
		m_cycle_des_metrics.m_m_dot_RHX = m_RHX.get_design_parameters()->m_m_dot_design[0];
	...
	void set_autodes_opts()
	{
		...
		m_P_rt_in_autodes = m_cycle_des_par.m_P_rt_in;
	...
	void set_od_data()
	{
		...
		m_cycle_od_performance.m_m_dot_RHX = m_RHX.get_od_outputs()->m_m_dot_calc[0];
	...
	void clear_member_data()
	{
		...
		m_P_rt_in_autodes = m_PR_mc_autodes = m_recomp_frac_autodes = m_LT_frac_autodes = m_P_high_autodes = std::numeric_limits<double>::quiet_NaN();
	}
