...
void mp_ancillary_services(ssc_data_t data) {
    ...
    try {
        ...
        */
        ...
        ancillary_services_success = (!mp_enable_energy_market_revenue && !mp_enable_ancserv1 && !mp_enable_ancserv2 &&
                                      !mp_enable_ancserv3 && !mp_enable_ancserv4);
        ...
        if (nsteps < (8760 * (size_t) analysis_period))
            nsteps = 8760 *
                     (size_t) analysis_period; // extrapolated timeseries has minimum of hourly values for use in all forecasting
                ...
                if (nsteps > 0) {
                    ...
                    for (size_t iyear = 0; iyear < (size_t) analysis_period; iyear++) {
                            ...
                            for (size_t ic = 0; (ic < current_num_per_year); ic++) {
                                if (system_use_lifetime_output > 0.5) // lifetime "gen" = "system_gen"
                                {
                                    if ((ic + iyear * current_num_per_year) < system_gen.ncols())
                                        current_year_capacity.push_back(
                                                system_gen(0, ic + iyear * current_num_per_year) / 1000.0); // kW to MW
                                } else // single year - adjust with degradation
                                {
                                        ...
                                        1) // adjust single year for lifetime system_generation
                                    {
                                        ...
                                        current_year_capacity.push_back(
                                                (system_gen(0, ic) * degrade_factor) / 1000.0); // kW to MW
                                    }

                                }
                            }
                            extrapolated_current_year_capacity = extrapolate_timeseries(current_year_capacity,
                                                                                        steps_per_hour);
                            for (size_t ic = 0; (ic < extrapolated_current_year_capacity.size()) &&
                                                ((ic + iyear * current_num_per_year) <
                                                 cleared_capacity_sum.size()); ic++)
                                system_generation[ic +
                                                  iyear * nsteps_per_year] = extrapolated_current_year_capacity[ic];
                        } else {
                        ...
                        if (en_mp_energy_market) {
                            ...
                            current_year_capacity.reserve(current_num_per_year);
                            for (size_t ic = 0; (ic < current_num_per_year) && ((ic + iyear * current_num_per_year) <
                                                                                mp_energy_market_revenue.nrows()); ic++)
                                current_year_capacity.push_back(
                                        mp_energy_market_revenue(ic + iyear * current_num_per_year, 0));
                            extrapolated_current_year_capacity = extrapolate_timeseries(current_year_capacity,
                                                                                        steps_per_hour);
                            for (size_t ic = 0; (ic < extrapolated_current_year_capacity.size()) &&
                                                ((ic + iyear * current_num_per_year) <
                                                 cleared_capacity_sum.size()); ic++) {
                                energy_market_capacity[ic + iyear *
                                                            nsteps_per_year] = extrapolated_current_year_capacity[ic];
                                cleared_capacity_sum[ic +
                                                     iyear * nsteps_per_year] += extrapolated_current_year_capacity[ic];
                            }
                            ...
                            if (calculate_revenue) {
                                ...
                                for (size_t ic = 0; (ic < current_num_per_year) &&
                                                    ((ic + iyear * current_num_per_year) <
                                                     mp_energy_market_revenue.nrows()); ic++)
                                    current_year_revenue.push_back(
                                            mp_energy_market_revenue(ic + iyear * current_num_per_year, 1));
                                extrapolated_current_year_revenue = extrapolate_timeseries(current_year_revenue,
                                                                                           steps_per_hour);
                                for (size_t ic = 0; (ic < extrapolated_current_year_revenue.size()) &&
                                                    ((ic + iyear * current_num_per_year) <
                                                     energy_market_revenue.size()); ic++)
                                    energy_market_revenue[ic + iyear *
                                                               nsteps_per_year] = extrapolated_current_year_revenue[ic]; // $/MWh
                            }
                        ...
                        if (en_mp_ancserv1) {
                            ...
                            for (size_t ic = 0; (ic < extrapolated_current_year_capacity.size()) &&
                                                ((ic + iyear * current_num_per_year) <
                                                 cleared_capacity_sum.size()); ic++) {
                                ancillary_services1_capacity[ic + iyear *
                                                                  nsteps_per_year] = extrapolated_current_year_capacity[ic];
                                cleared_capacity_sum[ic +
                                                     iyear * nsteps_per_year] += extrapolated_current_year_capacity[ic];
                            }
                            ...
                            if (calculate_revenue) {
                                ...
                                for (size_t ic = 0; (ic < current_num_per_year) &&
                                                    ((ic + iyear * current_num_per_year) <
                                                     mp_ancserv1_revenue.nrows()); ic++)
                                    current_year_revenue.push_back(
                                            mp_ancserv1_revenue(ic + iyear * current_num_per_year, 1));
                                extrapolated_current_year_revenue = extrapolate_timeseries(current_year_revenue,
                                                                                           steps_per_hour);
                                for (size_t ic = 0; (ic < extrapolated_current_year_revenue.size()) &&
                                                    ((ic + iyear * current_num_per_year) <
                                                     ancillary_services1_revenue.size()); ic++)
                                    ancillary_services1_revenue[ic + iyear *
                                                                     nsteps_per_year] = extrapolated_current_year_revenue[ic]; // $/MWh
                            }
                        ...
                        if (en_mp_ancserv2) {
                            ...
                            for (size_t ic = 0; (ic < extrapolated_current_year_capacity.size()) &&
                                                ((ic + iyear * current_num_per_year) <
                                                 cleared_capacity_sum.size()); ic++) {
                                ancillary_services2_capacity[ic + iyear *
                                                                  nsteps_per_year] = extrapolated_current_year_capacity[ic];
                                cleared_capacity_sum[ic +
                                                     iyear * nsteps_per_year] += extrapolated_current_year_capacity[ic];
                            }
                            ...
                            if (calculate_revenue) {
                                ...
                                for (size_t ic = 0; (ic < current_num_per_year) &&
                                                    ((ic + iyear * current_num_per_year) <
                                                     mp_ancserv2_revenue.nrows()); ic++)
                                    current_year_revenue.push_back(
                                            mp_ancserv2_revenue(ic + iyear * current_num_per_year, 1));
                                extrapolated_current_year_revenue = extrapolate_timeseries(current_year_revenue,
                                                                                           steps_per_hour);
                                for (size_t ic = 0; (ic < extrapolated_current_year_revenue.size()) &&
                                                    ((ic + iyear * current_num_per_year) <
                                                     ancillary_services2_revenue.size()); ic++)
                                    ancillary_services2_revenue[ic + iyear *
                                                                     nsteps_per_year] = extrapolated_current_year_revenue[ic]; // $/MWh
                            }
                        ...
                        if (en_mp_ancserv3) {
                            ...
                            for (size_t ic = 0; (ic < extrapolated_current_year_capacity.size()) &&
                                                ((ic + iyear * current_num_per_year) <
                                                 cleared_capacity_sum.size()); ic++) {
                                ancillary_services3_capacity[ic + iyear *
                                                                  nsteps_per_year] = extrapolated_current_year_capacity[ic];
                                cleared_capacity_sum[ic +
                                                     iyear * nsteps_per_year] += extrapolated_current_year_capacity[ic];
                            }
                            ...
                            if (calculate_revenue) {
                                ...
                                for (size_t ic = 0; (ic < current_num_per_year) &&
                                                    ((ic + iyear * current_num_per_year) <
                                                     mp_ancserv3_revenue.nrows()); ic++)
                                    current_year_revenue.push_back(
                                            mp_ancserv3_revenue(ic + iyear * current_num_per_year, 1));
                                extrapolated_current_year_revenue = extrapolate_timeseries(current_year_revenue,
                                                                                           steps_per_hour);
                                for (size_t ic = 0; (ic < extrapolated_current_year_revenue.size()) &&
                                                    ((ic + iyear * current_num_per_year) <
                                                     ancillary_services3_revenue.size()); ic++)
                                    ancillary_services3_revenue[ic + iyear *
                                                                     nsteps_per_year] = extrapolated_current_year_revenue[ic]; // $/MWh
                            }
                        ...
                        if (en_mp_ancserv4) {
                            ...
                            for (size_t ic = 0; (ic < extrapolated_current_year_capacity.size()) &&
                                                ((ic + iyear * current_num_per_year) <
                                                 cleared_capacity_sum.size()); ic++) {
                                ancillary_services4_capacity[ic + iyear *
                                                                  nsteps_per_year] = extrapolated_current_year_capacity[ic];
                                cleared_capacity_sum[ic +
                                                     iyear * nsteps_per_year] += extrapolated_current_year_capacity[ic];
                            }
                            ...
                            if (calculate_revenue) {
                                ...
                                for (size_t ic = 0; (ic < current_num_per_year) &&
                                                    ((ic + iyear * current_num_per_year) <
                                                     mp_ancserv4_revenue.nrows()); ic++)
                                    current_year_revenue.push_back(
                                            mp_ancserv4_revenue(ic + iyear * current_num_per_year, 1));
                                extrapolated_current_year_revenue = extrapolate_timeseries(current_year_revenue,
                                                                                           steps_per_hour);
                                for (size_t ic = 0; (ic < extrapolated_current_year_revenue.size()) &&
                                                    ((ic + iyear * current_num_per_year) <
                                                     ancillary_services4_revenue.size()); ic++)
                                    ancillary_services4_revenue[ic + iyear *
                                                                     nsteps_per_year] = extrapolated_current_year_revenue[ic]; // $/MWh
                            }
                    ...
                    if (cleared_capacity_sum.size() != system_generation.size())
                        error = util::format("cleared capacity size %d and capacity check size %d do not match",
                                             int(cleared_capacity_sum.size()), int(system_generation.size()));
                    else {
                        for (size_t i = 0; (i < cleared_capacity_sum.size()) && (i < system_generation.size()); i++) {
                            if (energy_market_capacity[i] < 0) {
                                error = util::format(
                                        "energy market cleared capacity %g is less than zerod at timestep %d",
                                        energy_market_capacity[i], int(i));
                                break;
                            } else if (ancillary_services1_capacity[i] < 0) {
                                error = util::format(
                                        "ancillary services 1 market cleared capacity %g is less than zerod at timestep %d",
                                        ancillary_services1_capacity[i], int(i));
                                break;
                            } else if (ancillary_services2_capacity[i] < 0) {
                                error = util::format(
                                        "ancillary services 2 market cleared capacity %g is less than zerod at timestep %d",
                                        ancillary_services2_capacity[i], int(i));
                                break;
                            } else if (ancillary_services3_capacity[i] < 0) {
                                error = util::format(
                                        "ancillary services 3 market cleared capacity %g is less than zerod at timestep %d",
                                        ancillary_services3_capacity[i], int(i));
                                break;
                            } else if (ancillary_services4_capacity[i] < 0) {
                                error = util::format(
                                        "ancillary services 4 market cleared capacity %g is less than zerod at timestep %d",
                                        ancillary_services4_capacity[i], int(i));
                                break;
                            } else if ((cleared_capacity_sum[i] > 0) &&
                                       (cleared_capacity_sum[i] > system_generation[i])) {
                                error = util::format(
                                        "sum of cleared capacity %g MW exceeds system capacity %g MW at timestep %d",
                                        cleared_capacity_sum[i], system_generation[i], int(i));
                                break;
                            }
                        }
                    }

                    if (calculate_revenue) { // all user specified capacities are greater than zero and sum of all less than system generation at timestep i
                        vt->assign("mp_energy_market_cleared_capacity",
                                   var_data(energy_market_capacity.data(), energy_market_capacity.size()));
                        vt->assign("mp_ancillary_services1_cleared_capacity",
                                   var_data(ancillary_services1_capacity.data(), ancillary_services1_capacity.size()));
                        vt->assign("mp_ancillary_services2_cleared_capacity",
                                   var_data(ancillary_services2_capacity.data(), ancillary_services2_capacity.size()));
                        vt->assign("mp_ancillary_services3_cleared_capacity",
                                   var_data(ancillary_services3_capacity.data(), ancillary_services3_capacity.size()));
                        vt->assign("mp_ancillary_services4_cleared_capacity",
                                   var_data(ancillary_services4_capacity.data(), ancillary_services4_capacity.size()));
                        ...
                        vt->assign("mp_ancillary_services1_price",
                                   var_data(ancillary_services1_revenue.data(), ancillary_services1_revenue.size()));
                        vt->assign("mp_ancillary_services2_price",
                                   var_data(ancillary_services2_revenue.data(), ancillary_services2_revenue.size()));
                        vt->assign("mp_ancillary_services3_price",
                                   var_data(ancillary_services3_revenue.data(), ancillary_services3_revenue.size()));
                        vt->assign("mp_ancillary_services4_price",
                                   var_data(ancillary_services4_revenue.data(), ancillary_services4_revenue.size()));
                        // total cleared capacity
                        vt->assign("mp_total_cleared_capacity",
                                   var_data(cleared_capacity_sum.data(), cleared_capacity_sum.size()));
                        for (size_t i = 0; (i < system_generation.size()) && (i < energy_market_capacity.size()) &&
                                           (i < energy_market_revenue.size()); i++) {
                            //									if (fabs(cleared_capacity_sum[i]) < 1e-5) // override, compensate generation at first enabled market if greater than zero.
                            {
                                if (en_mp_energy_market)
                                    energy_market_revenue[i] *= energy_market_capacity[i] /
                                                                steps_per_hour; // [MW] * [$/MWh] / fraction per hour [1/h]
                                else
                                    energy_market_revenue[i] = 0.0;
                                if (en_mp_ancserv1)
                                    ancillary_services1_revenue[i] *= ancillary_services1_capacity[i] /
                                                                      steps_per_hour; // [MW] * [$/MWh] / fraction per hour [1/h]
                                else
                                    ancillary_services1_revenue[i] = 0.0;
                                if (en_mp_ancserv2)
                                    ancillary_services2_revenue[i] *= ancillary_services2_capacity[i] /
                                                                      steps_per_hour; // [MW] * [$/MWh] / fraction per hour [1/h]
                                else
                                    ancillary_services2_revenue[i] = 0.0;
                                if (en_mp_ancserv3)
                                    ancillary_services3_revenue[i] *= ancillary_services3_capacity[i] /
                                                                      steps_per_hour; // [MW] * [$/MWh] / fraction per hour [1/h]
                                else
                                    ancillary_services3_revenue[i] = 0.0;
                                if (en_mp_ancserv4)
                                    ancillary_services4_revenue[i] *= ancillary_services4_capacity[i] /
                                                                      steps_per_hour; // [MW] * [$/MWh] / fraction per hour [1/h]
                                else
        ...
        vt->assign("mp_energy_market_generated_revenue",
                   var_data(energy_market_revenue.data(), energy_market_revenue.size()));
