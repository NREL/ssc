...
class cm_sp_ty : public compute_module
{
	...
	void exec( ) throw( general_error )
	{
		...
		std::string file_dir = "C:/Users/tneises/Documents/2015 LPDP/csp pv/rotate vs static cavity/Analysis";
		...
		for( int index_nh = 0; index_nh < n_heliostats_field; index_nh++ )
		{
			// Status update!
			if( index_nh % (n_heliostats_field/100) == 0 )
			{
				float percent = 100.0f * ((float)index_nh + 1) / ((float)n_heliostats_field);
			...
			for( int i = 0; i < n_sol_pos; i++ )
			{
				...
				data[i_local + (n_azimuth)*j_local] = combined_eta_opt(i, index_nh);
			}
		...
		// Write outputs
		std::string hel_performance_processed = file_dir + "/hel_performance_processed.txt";

		ofstream hel_output_file(hel_performance_processed.c_str());

		std::string out_line = "heliostat id,Q_deliver_hel (kWh_per_m2),CSP_avail_relval (kWeh),CSP_produced_relval (kWeh),PV_avail_relval (kWeh),PV_produced_relval (kWeh),bin0-11,bin11-20,bin21-30,bin31-40,bin41-50,bin51-60,bin61-70,bin71-80,bin81-90,bin91-100\n";

		hel_output_file << out_line;

		for( int i = 0; i < n_heliostats_field; i++ )
		{
			hel_output_file << to_string(i) + "," + to_string(hel_csp_to_rec_annual[i]) + 
