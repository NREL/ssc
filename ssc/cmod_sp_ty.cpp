...
static var_info _cm_vtab_sp_ty[] = {
        ...
        {SSC_INPUT, SSC_STRING, "solar_resource_file", "Weather file in TMY2, TMY3, EPW, or SMW.", "", "", "sp_ty", "*",
         "LOCAL_FILE", ""},
...
public:
    ...
    double
    powerout(double dc_nameplate, double ac_nameplate, double inv_eff_percent, double loss_percent, double ts_hour,
        ...
        for (int i = 0; i < n_eta_files; i++) {
            ...
            while (true) {
                if (!lk::read_line(fp, opt_line)) {
                    ...
                    if (i ==
                        0)        // heliostat positions are the same in each optical efficiency file, so only need to do this once...
                    {
        ...
        for (int index_rec_rot = 18; index_rec_rot < 19; index_rec_rot++) {
            ...
            for (int index_nh = 0; index_nh < n_heliostats_test_radius; index_nh++) {
                ...
                if (step_per_hour < 1 || step_per_hour > 60 || step_per_hour * 8760 != nrec)
                    throw exec_error("sp_ty", util::format(
                            "invalid number of data records (%d): must be an integer multiple of 8760", (int) nrec));
                ...
                while (hour < 8760) {
                    ...
                    for (size_t jj = 0; jj < step_per_hour; jj++) {
                        if (!wf.read())
                            throw exec_error("sp_ty", "could not read data line " + util::to_string((int) (idx + 1)) +
                                                      " in weather file");

                        if (wf.dn < 0 || wf.dn > 1500.0) {
                            log(util::format(
                                    "invalid beam irradiance %lg W/m2 at time [y:%d m:%d d:%d h:%d], set to zero",
                        ...
                        if (code != 0)
                            throw exec_error("pvsamv1",
                                             util::format(
                                                     "failed to process irradiation on surface %d (code: %d) [y:%d m:%d d:%d h:%d]",
                        ...
                        if (sunup == 1) {
                            ...
                            pv_available_relval_ts =
                                    powerout(pv_watts_nameplate, ac_nameplate, inv_eff_percent, loss_percent, ts_hour,
                                             poa_pv, wf.wspd, wf.tdry) / 1000.0;    // kWe
                            ...
                            if (wf.dn > DNI_cutoff) {
                                ...
                                csp_available_relval_ts =
                                        opt_eta_point * wf.dn * 0.001 * eta_rec_therm * eta_cycle * eta_csp_parasitics *
                                        dispatch_price_multiplier * A_hel * A_hel_refl_frac;        // kWe
                            ...
                            opt_eta_point = opt_eta_dni_product / (950.0 *
                                                                   0.001);        //[-] Normalized output compared to perfect optical efficiency at design DNI
            ...
            std::string hel_performance_processed =
                    file_dir + "/hel_performance_processed_rotate_" + to_string(rec_rotate_span) + ".txt";
            ...
            for (int i = 0; i < n_heliostats_test_radius; i++) {
                hel_output_file << to_string(hel_id_test_radius_az_sorted[i]) + "," +
                                   ...
                                   "," + to_string(
                        availability_csp * (pv_produced_relval_annual[i] - pv_available_relval_annual[i]) +
