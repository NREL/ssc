...
void Turbine_calculate_powercurve(ssc_data_t data) {
    ...
    double omegaT =
            -(omegaT_b / (2.0 * omegaT_a)) - (sqrt(pow(omegaT_b, 2) - (4.0 * omegaT_a * omegaT_c)) / (2.0 * omegaT_a));
    ...
    double rated_wind_speed = 0.33 * pow(2.0 * rated_hub_power * 1000.0 / (air_density * M_PI * pow(rotor_diameter, 2)
                                                                           / 4.0 * max_cp), (1.0 / 3.0)) +
                              0.67 * (((1.0 / (1.5 * air_density * M_PI * pow(rotor_diameter, 2) * 0.25
                                               * max_cp * pow(wind_at_omegaT, 2))) * 1000.0 *
                                       (rated_hub_power - power_at_omegaT)) + wind_at_omegaT);

    if (omegaT > omega_m) {
        sprintf(errmsg, "Turbine inputs are not valid, please adjust the inputs. omegaT: %f, omegaM: %f", omegaT,
                omega_m);
    ...
    for (size_t i = 0; i < array_size; i += 1) {
        ...
        if (ws <= cut_in || ws >= cut_out) {
            ...
            hub_power =
                    (rated_hub_power - power_at_omegaT) / (rated_wind_speed - wind_at_omegaT) * (ws - wind_at_omegaT)
        ...
        if (hub_power == 0.) {
            ...
            powercurve_hub_efficiency[i] = ((hub_power / rated_hub_power) - (a + b * (hub_power / rated_hub_power)
                                                                             +
                                                                             c * pow(hub_power / rated_hub_power, 2))) /
                                           (hub_power / rated_hub_power);
