...
// 2 Mode Model
// Time Marching Methods: Implicit Euler
		...
		********************************************************************** */	
		for ( i=0; i < 8760; i++ )
		{
			...
			if (Q_useful > 0)	
			{
				...
				T_tank = ((T_tank_prev_hour*m_tank*Cp_water) + dT*((Q_useful) + (UA_tank*T_room) + (mdot_mix*Cp_water*T_mains[i])))/((m_tank*Cp_water) + dT*(UA_tank + mdot_mix*Cp_water));
			...
			else
			{
				...
				else
				{
					...
					double A_hot = tank_cross_section + 2*M_PI*tank_radius*h_hot;
					...
					{	
						...
						double A_cold = tank_cross_section + 2*M_PI*tank_radius*h_cold;
						T_cold = ((T_cold_prev_hour*m_cold*Cp_water) + (dT*U_tank*A_cold*T_room) + (dT*mdot_mix*Cp_water*T_mains[i]) )/((m_cold*Cp_water) + (dT*A_cold*U_tank) + (mdot_mix*dT*Cp_water) ); // IMPLICIT NON-STEADY
					}
				}
