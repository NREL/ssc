...
// developer returns
	...
	{ SSC_OUTPUT,        SSC_ARRAY,      "cf_sponsor_statax_income", "Developer state taxable income",                   "$",            "",                      "DHF",      "*",                     "LENGTH_EQUAL=cf_length",                "" },
	...
	{ SSC_OUTPUT,        SSC_ARRAY,      "cf_sponsor_fedtax_income", "Developer federal tax income",                   "$",            "",                      "DHF",      "*",                     "LENGTH_EQUAL=cf_length",                "" },
...
// partner returns
		...
		if (ppa_mode == 1)
		{
			...
			solved = ( ((residual >=0) && ( residual  < ppa_soln_tolerance )) || ( fabs(ppa_min-ppa_max) < ppa_soln_tolerance) ) ;
			if (!solved)
			{
				double itnpv = cf.at(CF_tax_investor_aftertax_npv, flip_target_year);
				double itnpvplusdelta = npv(CF_tax_investor_aftertax,i,nom_discount_rate+0.01) +  cf.at(CF_tax_investor_aftertax,0) ;
